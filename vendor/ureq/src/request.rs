...
pub struct Request {
    abort_handle: Option<crate::AbortHandle>,
    agent: Agent,
    method: String,
    url: String,
...
impl Request {
    ...
    /// Attach an explicit cancellation handle. This request uses a dedicated
    /// connection so an abort can never affect an unrelated pooled request.
    pub fn with_abort_handle(mut self, handle: crate::AbortHandle) -> Self {
    ...
    fn do_call(mut self, payload: Payload) -> Result<Response> {
        ...
        let request_fn = |req: Request| {
            ...
            unit.abort_handle = req.abort_handle;
