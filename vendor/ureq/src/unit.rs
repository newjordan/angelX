...
pub(crate) struct Unit {
    pub abort_handle: Option<crate::AbortHandle>,
    pub agent: Agent,
    pub method: String,
    pub url: Url,
    is_chunked: bool,
...
pub(crate) fn connect(
    ...
    let mut resp = loop {
        ...
        let abort_handle = unit.abort_handle.clone();
        ...
        unit.abort_handle = abort_handle;
    };
...
fn connect_inner(
    ...
    if unit.abort_handle.is_some() {
        stream.set_unpoolable();
    }
...
fn connect_socket(unit: &Unit, hostname: &str, use_pooled: bool) -> Result<(Stream, bool), Error> {
    ...
    if use_pooled && unit.abort_handle.is_none() {
