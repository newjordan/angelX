...
const pagePlates = () => {
  ...
  return new Set(
    [...html.matchAll(/<img[^>]*src=["']assets\/frames\/([^"']+)\.png["']/g)].map((m) => m[1]),
  )
}
...
const [cellW, cellH] = cellArg >= 0 ? rest[cellArg + 1].split('x').map(Number) : [9, 19]
...
const main = async () => {
  ...
  const explicit =
    onlyArg >= 0
      ? rest[onlyArg + 1]
          ...
          .filter(Boolean)
      : null
  const wanted = explicit ? new Set(explicit) : rest.includes('--all') ? null : pagePlates()
  const files = wanted ? captures.filter((f) => wanted.has(basename(f, '.json'))) : captures
  if (!files.length) {
    console.error(
  ...
  if (explicit) {
    const missing = explicit.filter((n) => !captures.some((f) => basename(f, '.json') === n))
    if (missing.length) {
      console.error(`note: no capture json for ${missing.join(', ')} — run the capture test first`)
    }
  }
  const skipped = captures.filter((f) => !files.includes(f))
  if (skipped.length) {
    console.log(
      `skipping ${skipped.length} capture(s) ${
        explicit ? 'not named by --only' : 'index.html does not show'
      }: ${skipped.map((f) => basename(f, '.json')).join(', ')}`,
    )
    console.log('  (--all rasterizes every capture; --only a,b names them explicitly)')
  }
