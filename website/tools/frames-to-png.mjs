...
if (!srcDir || !outDir) {
  console.error('usage: frames-to-png.mjs <frames-dir> <out-dir> [--scale N] [--cell WxH]')
  process.exit(2)
}
...
// ── colour tables ──────────────────────────────────────────────────────────
const ANSI16 = {
  Black: '#1c1c22', Red: '#c14a52', Green: '#5aa469', Yellow: '#c8a24a',
  Blue: '#5a7fc1', Magenta: '#a06bb8', Cyan: '#4fa3a3', Gray: '#c3c3c8',
  DarkGray: '#5b5b66', LightRed: '#e0736f', LightGreen: '#8ecf8e',
  LightYellow: '#e8dd8a', LightBlue: '#8fb3e8', LightMagenta: '#d4a0e0',
  LightCyan: '#8fd8d8', White: '#f4f4f7',
}
...
const xterm = (n) => {
  ...
  if (n < 232) {
    ...
    const r = steps[Math.floor(i / 36)], g = steps[Math.floor((i % 36) / 6)], b = steps[i % 6]
    return `rgb(${r},${g},${b})`
  }
  const v = 8 + (n - 232) * 10
  return `rgb(${v},${v},${v})`
...
const colour = (spec, fallback) => {
  ...
  const bare = s.replace(/Modifier::/g, '').replace(/\s*\|\s*/g, '+').trim()
...
const page = (frame) => {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const style = [
        `left:${x * cellW}px`, `top:${y * cellH}px`,
        `color:${fg}`, `font-weight:${weight(modSpec)}`,
