...
const plates = imgs.filter(
  (i) =>
    i.src &&
    (i.src.includes("assets/frames/") || i.src.includes("background_lion")),
);
console.log(
  `images on page: ${imgs.length} · plates: ${plates.length} · failed requests: ${failed.length}`,
);
for (const i of plates)
  console.log(`  ${i.w > 0 ? "OK " : "BROKEN"} ${i.src} ${i.w}x${i.h}`);
...
console.log(
  ok
    ? `GATE: PASS — ${EXPECTED_PLATES} plates loaded as real pixels`
    : "GATE: FAIL",
);
process.exit(ok ? 0 : 1);
