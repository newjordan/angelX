...
if (!srcArg) { console.error('usage: teaser-to-mp4.mjs <frames-dir> [out.mp4]'); process.exit(2); }
...
function load(prefix) {
  ...
  return names.map(n => ({
    ...
    cells: (() => { try { return readFileSync(join(src, n.replace('.txt', '.cells')), 'utf8').split('\n'); } catch { return null; } })(),
  }));
}
...
function xterm256(i) {
  ...
  if (i < 232) {
    const c = i - 16, v = [0, 95, 135, 175, 215, 255];
...
const browser = await chromium.launch({ executablePath: '/usr/bin/chromium', args: ['--no-sandbox', '--force-device-scale-factor=2'] });
...
function frameDom(text, cells) {
  ...
  for (let y = 0; y < Math.min(lines.length, ROWS); y++) {
    ...
    for (let i = 0, x = 0; i < cps.length && x < COLS; i++, x++) {
      ...
      } else if (ch === ' ') { rowHtml += `<span class="cell"> </span>`; }
      else rowHtml += `<span class="cell"><span class="g" style="color:${fg}">${ch === '<' ? '&lt;' : ch === '&' ? '&amp;' : ch}</span></span>`;
    }
...
async function shotPngs(prefix) {
  ...
  for (const f of frames) {
    await page.evaluate(html => { document.getElementById('t').innerHTML = html; }, frameDom(f.text, f.cells));
    // dedupe: skip if text identical to previous (already-rendered png reused)
    const buf = await page.screenshot({ type: 'png', clip: { x: 0, y: 0, width: W, height: H } });
    pngs.push(buf);
    if (f.text === frames[frames.indexOf(f) - 1]?.text) pngs[pngs.length - 1] = pngs[pngs.length - 2];
  }
  return pngs;
}
...
console.log(`OK ${out} (${(statSync(out).size / 1e6).toFixed(1)} MB, ${total.toFixed(1)}s @${FPS}fps)`);
