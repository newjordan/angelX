...
#[test]
fn retard_mode_arms_and_injects_directive() {
    let mut app = seed_preview_app();
    app.input = "/retard".to_string();
    app.submit();
    assert!(app.retard_mode);
    assert!(
        app.messages
            .iter()
            .any(|m| m.text.contains("retard mode ON"))
    );
    ...
    let context = app
        ...
        .expect("Harness-role cockpit controls");
    assert!(
        context.content.contains("literally retarded"),
        "literal retard contract missing: {}",
        context.content
    );
    assert!(
    ...
    app.input = "/retard off".to_string();
    app.submit();
    assert!(!app.retard_mode);
    assert!(
        app.messages
            .iter()
            .any(|m| m.text.contains("retard mode OFF"))
    );
}
