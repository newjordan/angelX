...
test("a copyleft license on its own is refused, not merely unrecognized", () => {
  for (const expression of [
    ...
    assert.deepEqual(
      result.unknown,
      [],
      `${expression} should be a known refusal`,
    );
...
test("file-level copyleft is refused by policy, not by accident", () => {
  for (const expression of ["MPL-2.0", "CDDL-1.0", "EPL-2.0"]) {
    ...
    assert.deepEqual(
      result.unknown,
      [],
      `${expression} must be a deliberate refusal`,
    );
...
test("WITH passes only for an allowlisted exception", () => {
  ...
  assert.equal(
    allowed("Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT"),
    true,
  );
  assert.ok(ALLOWED_EXCEPTIONS.has("llvm-exception"));

  const classpath = evaluateExpression(
  ...
  assert.deepEqual(classpath.unknown, [
    "apache-2.0 WITH classpath-exception-2.0",
  ]);
});
...
test("a missing license is its own violation kind", () => {
  const report = auditLicenses([
    { name: "bare", version: "0.1.0", license: "" },
  ]);
...
test("parseExpression rejects trailing tokens instead of ignoring them", () => {
  assert.throws(
    () => parseExpression(tokenize("MIT APACHE-2.0")),
    /trailing tokens/,
  );
  assert.throws(
    () => parseExpression(tokenize("Apache-2.0 WITH")),
    /WITH without an exception/,
  );
});
