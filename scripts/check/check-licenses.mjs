...
export function parseExpression(tokens) {
  ...
  function parseFactor() {
    const token = peek();
    if (token === undefined)
      throw new Error("unexpected end of SPDX expression");
    if (token === "(") {
      take();
      const inner = parseOr();
      if (take() !== ")")
        throw new Error("unbalanced parenthesis in SPDX expression");
      return inner;
    }
    const id = take();
    if (peek() && peek().toUpperCase() === "WITH") {
      ...
      if (exception === undefined)
        throw new Error(`WITH without an exception after ${id}`);
  ...
  if (at !== tokens.length)
    throw new Error(
      `trailing tokens in SPDX expression: ${tokens.slice(at).join(" ")}`,
    );
  return tree;
}
    ...
    switch (node.kind) {
      ...
      case "with": {
        ...
        if (!ALLOWED_EXCEPTIONS.has(exception)) {
          if (REFUSED_LICENSES.has(exception))
            denied.add(`${id} WITH ${exception}`);
...
function run(repoRoot) {
  ...
  try {
    ...
    console.error(
      `${SCRIPT}: could not collect dependency licenses: ${error.message}`,
    );
    return 2;
  }
  ...
  for (const [expression, tally] of [...byExpression.entries()].sort(
    ...
    console.log(
      `  ${mark} ${String(tally.allowed + tally.refused).padStart(4)}  ${expression}`,
    );
  }
