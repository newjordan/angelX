...
export function runRollbackVerification({
  previous,
  next,
  previousSha256,
  nextSha256,
  ...
  const receipt = {
    schema: 'angelX-prefix-rollback-verification/v1',
    timestamp_utc: new Date().toISOString(),
    host: { platform: platform(), arch: arch(), release: release() },
    status: 'fail',
    steps: [],
    coverage: {
      executable: 'bin/angel',
      operation: 'N -> N+1 -> N using retained N artifact',
      not_covered: [
        'bin/angelX launcher and aliases/shims',
        'angel-portal-renderer helper',
        'interactive terminal operation',
        'operator state migration',
        'package manager integration',
        'clean-host dependency provisioning',
        'other supported hosts',
      ],
      network_isolation:
        'build-info only with empty credential environment; no network namespace claim',
    },
  }
  let scratch
  ...
  try {
    if (platform() !== 'linux' || arch() !== 'x64')
      throw new Error('supported host is Linux x86_64')
    const candidates = [previous, next].map((path, index) => {
      const expected = [previousSha256, nextSha256][index]
      if (!/^[a-f0-9]{64}$/.test(expected || ''))
        throw new Error('explicit reviewed candidate sha256 required')
      ...
      return {
        ...
        metadata: {
          ...
          release: { reviewed_binary_sha256: expected },
          binary: {
    ...
    if (candidates[0].sha256 === candidates[1].sha256)
      throw new Error('N and N+1 must be distinct artifacts')
    ...
    function probe(binary) {
      ...
      if (result.error || result.status !== 0)
        throw new Error(`build-info probe failed: exit ${result.status}`)
      const info = JSON.parse(result.stdout)
      if (
        info?.schema !== 'angel-build-info/v1' ||
        !/^[a-f0-9]{64}$/.test(info.cockpit_source_sha256 || '')
      )
    ...
    if (canonicalJson(identities[0]) === canonicalJson(identities[1]))
      throw new Error('build identities must differ')
    for (const [index, operation] of [
      ...
      if (
        ...
        canonicalJson(result.build_info) !== canonicalJson(identities[index])
      )
    ...
    receipt.cleanup = {
      prefix_created: Boolean(scratch),
      removed: Boolean(scratch) && !existsSync(scratch),
    }
...
if (import.meta.url === (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href)) {
  ...
  const names = {
    '--previous': 'previous',
    '--next': 'next',
    '--previous-sha256': 'previousSha256',
    '--next-sha256': 'nextSha256',
    '--receipt': 'receiptPath',
  }
  try {
    for (let i = 0; i < args.length; i += 2) {
      if (!names[args[i]] || !args[i + 1])
