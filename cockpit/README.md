...
- `src/approval.rs` — human-approval gate (`ANGEL_MOA_APPROVE=1`): a non-local
  moa action (phone a SOTA / remote / peer) pauses for a TUI modal —
  **[y]** approve · **[a]** approve-all-this-kind-this-turn · **[n]** deny. With
  many agents the prompt is coalesced (one decision per kind per turn) and
  token-free (a UI round-trip, never a model call). Off by default.
- `src/deli.rs` — `/loop deli`: a single-session run of
  ...
  `_MIN_FINDINGS/_STATE_DIR`). The historical `ANGEL_DRIVER=deli` preference
  aliases the swarm route; use `/loop deli` for this deliberation wrapper.
