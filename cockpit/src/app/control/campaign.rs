...
pub(crate) enum CampaignPending {
    Inspect {
        ...
        rx: mpsc::Receiver<Result<crate::agent::harness::CampaignBase, String>>,
    },
    Prepare {
        engine: Arc<SwarmCompilerEngine>,
        cancelled: Arc<AtomicBool>,
...
impl App {
    pub(crate) fn campaign_command(&mut self, arg: Option<&str>) -> String {
        let command = match crate::drive::campaign::CampaignCommand::parse(arg) {
            Ok(command) => command,
            Err(error) => return format!("campaign error: {error}"),
        };
        if !matches!(
            &command,
            crate::drive::campaign::CampaignCommand::Advance
                | crate::drive::campaign::CampaignCommand::Review
        ) {
            if matches!(
                &command,
                crate::drive::campaign::CampaignCommand::Pause
                    | crate::drive::campaign::CampaignCommand::Abandon
