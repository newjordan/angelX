...
impl App {
    pub(crate) fn close_moa_deck(&mut self) {
        ...
        self.scryglass
            .controller
            .leave_route(crate::ui::scryglass::StageRoute::Formation);
    }
    ...
    pub(crate) fn open_moa_deck(&mut self, arg: Option<&str>) {
        match arg.map(str::trim).filter(|s| !s.is_empty()) {
            ...
            Some(a) if a.eq_ignore_ascii_case("ledger") || a.eq_ignore_ascii_case("report") => {
                self.system_msg(crate::agent::swarm::ledger::report_text(
                    None,
            ...
            Some(a) if formation_alias(a).is_some() => {
                ...
                self.system_msg(format!(
                    ...
                    crate::agent::formations::formation(id).name
                ));
            }
            _ => {
                self.agent_menu = None;
                self.scryglass
                    .navigate(crate::ui::scryglass::StageRoute::Formation);
                if self.moa_deck.is_none() {
                    let mut deck =
                        crate::agent::formations::MoaDeckState::new(self.bag.moa_model_choices());
    ...
    pub(crate) fn moa_deck_key(&mut self, code: KeyCode, modifiers: KeyModifiers) -> bool {
        ...
        if deck.focus() == crate::agent::formations::MoaDeckFocus::Models {
            return match code {
                KeyCode::Esc => {
        ...
        if deck.focus() == crate::agent::formations::MoaDeckFocus::Efforts {
            return match code {
                KeyCode::Esc => {
        ...
        match code {
            ...
            KeyCode::Up | KeyCode::Char('k') => {
                if deck.focus() == crate::agent::formations::MoaDeckFocus::Slots {
                    deck.move_slot(-1);
                } else {
            ...
            KeyCode::Enter => {
                if deck.focus() == crate::agent::formations::MoaDeckFocus::Formations {
    ...
    pub(crate) fn moa_arm_chip(&self) -> Option<String> {
        if let Some(engagement) = &self.moa_one_shot {
            let formation = crate::agent::formations::formation(engagement.formation);
            return Some(format!(
                " · {} {}/{} armed ",
                formation.name,
    ...
    pub(crate) fn moa_arm_status(&self) -> String {
        match (&self.moa_one_shot, &self.moa_session) {
            (Some(next), Some(session)) => format!(
                "{} next · {} session",
                crate::agent::formations::formation(next.formation).name,
                crate::agent::formations::formation(session.formation).name
            ),
            (Some(next), None) => {
                format!(
                    "{} · next",
    ...
    pub(crate) fn moa_formation_chip_label(&self) -> String {
        if let Some(engagement) = self.moa_one_shot.as_ref().or(self.moa_session.as_ref()) {
            let name = crate::agent::formations::formation(engagement.formation).name;
    ...
    pub(crate) fn select_moa_card(&mut self, id: crate::agent::formations::FormationId) {
    ...
    pub(crate) fn select_or_arm_moa_card(&mut self, id: crate::agent::formations::FormationId) {
    ...
    pub(crate) fn open_moa_effort_picker(&mut self) {
        ...
        if slot.role == crate::agent::formations::FormationRole::Scout {
            self.system_msg(format!(
            ...
            Err(e) => {
                if one_shot.is_some() {
                    if let Some(session) = self.moa_session.as_ref() {
                        crate::agent::formations::formation(session.formation).apply_sota_env();
                    } else {
                        crate::agent::formations::formation(
                            crate::agent::formations::FormationId::SoloStrike,
                        )
                        .apply_sota_env();
                    }
                } else {
    ...
    fn arm_moa_engagement(
        &mut self,
        engagement: crate::agent::formations::MoaEngagement,
        session: bool,
    ) {
        let formation = *crate::agent::formations::formation(engagement.formation);
        if formation.is_resting() {
        ...
        let staged = [
            (
                crate::agent::formations::FormationRole::Propose,
                &efforts.propose,
            ),
            (
                crate::agent::formations::FormationRole::Judge,
                &efforts.judge,
            ),
            (
                crate::agent::formations::FormationRole::Verify,
                &efforts.verify,
            ),
            (
                crate::agent::formations::FormationRole::Aggregate,
                &efforts.aggregate,
            ),
        ]
        .into_iter()
    ...
    fn moa_card_status_text(&self) -> String {
        let one = self
            .moa_one_shot
            .as_ref()
            .map(|engagement| crate::agent::formations::formation(engagement.formation).name)
            .unwrap_or("(none)");
        let session = self
            .moa_session
            .as_ref()
...
#[cfg(test)]
#[path = "../../../../tests/cockpit/app/app_control__moa__tests.rs"]
mod tests;
