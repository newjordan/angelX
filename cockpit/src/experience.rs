...
//!
//! Design mirrors [`crate::swarm::ledger`] (append-only JSONL, best-effort I/O,
...
pub(crate) fn ledger_path() -> PathBuf {
    match std::env::var("ANGEL_EXPERIENCE_LOG") {
        ...
        _ => crate::workspace_store::angel_subdir("experience").join("ledger.jsonl"),
    }
}
...
pub(crate) fn repo_value_for(folder: &Path) -> serde_json::Value {
    let id = crate::workspace_store::repo_identity(folder);
...
pub(crate) fn scrub_secrets(text: &str) -> String {
    ...
    crate::secrets::redact_str(&out.join(" ")).into_owned()
}
...
pub(crate) fn hiq_outcome_json(counters: &TurnCounters) -> serde_json::Value {
    ...
    let root = crate::harness::last_root_hiq();
    ...
    let peer_p1 = crate::harness::load_living_peer_p1_us();
    ...
    if let Some(snap) = crate::harness::load_forge_train_snap() {
        map.insert(
    ...
    let hold_n = crate::harness::load_living_peer_shape_holds(8).len();
    if hold_n > 0 {
    ...
    let open = crate::harness::load_living_peer_open_levers(4);
    if !open.is_empty() {
        map.insert(
...
pub(crate) fn record_backplane_outcome(outcome: &crate::backplane::TurnOutcome, workspace: &Path) {
    if cfg!(test) || !enabled() {
        return;
    }
    append_jsonl(
...
/// assertion site ([`crate::dossier::context_block`]), not inferred.
...
pub(crate) fn record_cmd_event(exp: &CmdExperience, folder: &Path) {
    ...
    if let Some(receipt) = crate::harness::exec::sandbox_receipt() {
...
fn maybe_rotate(path: &Path) {
    if let Ok((directory, name)) = crate::workspace_store::private_io::parent(path) {
...
fn acquire_jsonl_lock(path: &Path) -> std::io::Result<JsonlLock> {
    let (directory, _) = crate::workspace_store::private_io::parent(path)?;
    ...
    crate::workspace_store::lock_store(&file, "experience_lock")?;
...
fn ledger_cap(path: &Path) -> u64 {
    ...
    crate::store_caps::value(store, "max_bytes")
}
...
fn append_jsonl_bounded(path: &Path, value: &serde_json::Value, cap: u64) -> std::io::Result<()> {
    ...
    let mut line = crate::secrets::to_redacted_vec(value).map_err(std::io::Error::other)?;
    line.push(b'\n');
...
pub(crate) fn replace_jsonl(path: &Path, values: &[serde_json::Value]) -> std::io::Result<()> {
    ...
    for value in values {
        body.extend(crate::secrets::to_redacted_vec(value).map_err(std::io::Error::other)?);
        body.push(b'\n');
    }
...
mod tests {
    ...
    fn asserted_commands_are_project_scoped_and_deduped() {
        let base =
            std::env::temp_dir().join(format!("angel_asserted_scope_{}", std::process::id()));
