...
impl BarrelCfg {
    pub(crate) fn from_env() -> Self {
        ...
        let dir = std::env::var_os("ANGEL_BARREL_DIR")
            .filter(|s| !s.is_empty())
            .map(PathBuf::from)
            .unwrap_or_else(|| crate::platform::workspace_store::angel_subdir("barrel"));
...
pub(crate) fn capture_answer(club: &dyn Club, workspace: &Path, history: &[ChatMsg], answer: &str) {
    ...
    let job = Job {
        ts: now_secs(),
        repo: crate::knowledge::experience::repo_value_for(workspace),
...
pub(crate) fn redact_text(text: &str) -> (String, usize) {
    ...
    let redactor = crate::platform::secrets::Redactor::new();
    let out = text
        .lines()
        .map(|line| {
...
fn write_status(cfg: &BarrelCfg, state: &WriterState, ts: u64) {
    ...
    let _ = crate::platform::workspace_store::private_io::replace(
        ...
        "" | "status" => {
            ...
            status_text_in(
                ...
                &std::env::var_os("ANGEL_STILL_DIR").filter(|v| !v.is_empty())
                    .map(PathBuf::from).unwrap_or_else(|| crate::platform::workspace_store::angel_subdir("still")),
                now_secs(),
            )
        }
