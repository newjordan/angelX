...
use crate::agent::harness::{Tool, ToolRegistry};
use ratatui::Frame;
...
enum UiOperation {
    InspectImage {
        action: crate::ui::still_inspector::Action,
    },
    OpenControl {
        control: ControlKind,
    },
...
fn semantic_state(app: &App) -> Value {
    ...
    let catalog_semantics = app.scryglass.catalog_open().then(|| {
        ...
        json!({
            ...
            "count": crate::knowledge::library::catalog_shelves().len(),
            "shelf_id": shelf.id,
    ...
    let session_health = match app.session.save_status() {
        crate::knowledge::session::SessionSaveStatus::Healthy => json!({ "state": "healthy" }),
        crate::knowledge::session::SessionSaveStatus::Pending => json!({ "state": "pending" }),
    ...
    let operational_rows = app
        .messages
        .iter()
        .filter(|message| matches!(message.role, crate::ui::transcript::Role::Activity))
        .count();
    let tool_arg_repairs = crate::agent::club::tool_arg_repair_records();
    let mut semantics = json!({
        ...
        "world": {
            ...
            "selected_building": match stage_route { crate::ui::scryglass::StageRoute::Explore(building) => Some(format!("{building:?}")), _ => None },
    ...
    if app.atlas.enabled() {
        semantics["atlas"] = json!({
            ...
            "open": stage_route == crate::ui::scryglass::StageRoute::Vault,
    ...
    semantics["backplane"] = json!({
        "mode": format!("{:?}", crate::agent::backplane::mode()).to_ascii_lowercase(),
...
fn apply_operation(app: &mut App, operation: &UiOperation) -> Result<String, String> {
    ...
    if let UiOperation::OpenObservatory { campaign } = operation {
        ...
        app.submit();
        if app.scryglass.controller.route() != crate::ui::scryglass::StageRoute::Observatory {
    ...
    match operation {
        UiOperation::OpenControl { control } => {
            let expected = match control {
                ControlKind::Model => crate::ui::agent_panel::controls::AgentMenuKind::Model,
                ControlKind::Thinking => crate::ui::agent_panel::controls::AgentMenuKind::Thinking,
            };
