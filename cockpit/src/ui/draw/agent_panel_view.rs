...
pub(crate) fn portrait_state(app: &App) -> crate::ui::views::agent_view::PortraitState {
    crate::ui::views::agent_view::portrait_state_from(
...
fn bay_profile_lines<'a>(
    ...
    if portrait_states_enabled() {
        crate::ui::views::agent_view::compact_profile_lines_stateful(
            profile,
            label,
            portrait_state(app),
        )
    } else {
        crate::ui::views::agent_view::compact_profile_lines(profile, label, active)
    }
}
...
pub(crate) fn agent_host_metrics_line(
    ...
    Some(crate::ui::views::status_view::agent_metrics_line(
...
pub(crate) fn agent_token_lines(app: &App, width: u16, max_lines: usize) -> Vec<Line<'static>> {
    ...
    if active_agent_is_moa(app, idle_mode)
        && let Some(report) =
            crate::agent::swarm::ledger::token_report(app.tools.current_workspace())
    {
        let lines =
            crate::ui::views::status_view::moa_token_report_lines(&report, width, max_lines);
        if !lines.is_empty() {
            return lines;
        }
    }
    ...
    if usage_and_sota.1 {
        vec![Line::from(
            crate::ui::views::status_view::token_usage_meter(usage_and_sota.0),
        )]
    } else {
        Vec::new()
    }
    ...
    maybe_paint_side_column_kitty(|| {
        if portrait_states_enabled() {
            ...
            let path = crate::platform::runtime_paths::cockpit_dir()
                .join(crate::ui::helm::sheet(profile.key));
            ...
            let next = if state == crate::ui::views::agent_view::PortraitState::Idle
        ...
        {
            let opposite = crate::platform::runtime_paths::cockpit_dir().join(opposite);
...
pub(crate) fn render_agent_controls_in(
    ...
    let fallback_armed = crate::agent::club::fallback_armed();
