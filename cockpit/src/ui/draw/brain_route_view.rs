...
//!
//! Menu state and input behavior remain in `control::agent_menu`; this
...
pub(super) fn render_agent_control_menu(frame: &mut Frame, app: &mut App) {
    ...
    let operational_pick = crate::knowledge::route_intelligence::operational_pick_for_context(
        &app.route_evidence,
        &route_choices,
        context_used,
    );
    let quality_pick = crate::knowledge::route_intelligence::quality_pick_for_context(
        &app.route_evidence,
        &route_choices,
        context_used,
    );
    ...
    for (row, position) in (start..start + visible).enumerate() {
        ...
        let (mut text, enabled, action, secondary, pressure) = match menu.kind {
            AgentMenuKind::Model => {
                ...
                let connection = crate::ui::agent_panel::controls::connection_label(choice);
