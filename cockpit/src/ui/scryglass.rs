...
use crate::ui::media::MediaSource;
...
pub(crate) struct VideoIdentity {
    pub source: crate::ui::media::MediaSource,
    pub request_id: u64,
    pub generation: u64,
}
...
pub(crate) struct Scryglass {
    ...
    video: VideoSurface,
    lesson: Option<crate::ui::term::lookup::QuickLookup>,
    lesson_scroll: u16,
    catalog_scroll: u16,
    catalog_selection: usize,
    lesson_returns_to_catalog: bool,
    #[cfg(test)]
    queued_lesson_outcome: Option<crate::ui::term::lookup::TestLookupOutcome>,
...
impl Scryglass {
    ...
    fn begin_lesson_from(&mut self, term: String, return_to_catalog: bool) -> bool {
        ...
        if let Some(outcome) = self.queued_lesson_outcome.take() {
            self.lesson = Some(crate::ui::term::lookup::QuickLookup::queued(&term, outcome));
            return true;
        }
        self.lesson = Some(crate::ui::term::lookup::QuickLookup::spawn(term));
        true
    }
    ...
    pub(crate) fn selected_shelf(&self) -> &'static crate::knowledge::library::CurriculumShelf {
        &crate::knowledge::library::catalog_shelves()[self.catalog_selection()]
    }

    pub(crate) fn move_catalog_selection(&mut self, delta: isize) {
        let len = crate::knowledge::library::catalog_shelves().len();
        if len == 0 {
    ...
    pub(crate) fn select_catalog_last(&mut self) {
        self.catalog_selection = crate::knowledge::library::catalog_shelves()
            .len()
            .saturating_sub(1);
        self.catalog_scroll = self.catalog_selection.min(u16::MAX as usize) as u16;
    }
    ...
    pub(crate) fn begin_selected_lesson(&mut self) -> bool {
        ...
        let Some(lesson) = crate::ui::term::lookup::QuickLookup::for_shelf(shelf.id) else {
            return false;
        };
        if !self.controller.show_overlay(StageOverlay::Lesson) {
            return false;
    ...
    pub(crate) fn lesson(&self) -> Option<&crate::ui::term::lookup::QuickLookup> {
        self.lesson.as_ref()
    }
    ...
    pub(crate) fn lesson_loading(&self) -> bool {
        self.lesson
            .as_ref()
            .is_some_and(crate::ui::term::lookup::QuickLookup::is_loading)
    }

    pub(crate) fn lesson_roll_pending(&self) -> bool {
        self.lesson
            .as_ref()
            .is_some_and(crate::ui::term::lookup::QuickLookup::roll_pending)
    }
    ...
    pub(crate) fn set_ready_lesson(&mut self, term: &str, title: &str, summary: &str) -> bool {
        ...
        self.lesson_scroll = 0;
        self.lesson_returns_to_catalog = false;
        self.lesson = Some(crate::ui::term::lookup::QuickLookup::ready(
            term, title, summary,
        ));
        true
    }
    ...
    pub(crate) fn set_unrolled_lesson(&mut self, term: &str, title: &str, summary: &str) -> bool {
        ...
        self.lesson_scroll = 0;
        self.lesson = Some(crate::ui::term::lookup::QuickLookup::ready_unrolled(
            term, title, summary,
        ));
        true
    }
    ...
    #[cfg(test)]
    pub(crate) fn queue_lesson_outcome(
        &mut self,
        outcome: crate::ui::term::lookup::TestLookupOutcome,
    ) {
    ...
    pub(crate) fn paint_loading(frame: &mut Frame, area: Rect, tick: u64) {
        ...
        if let Some(cell) = frame.buffer_mut().cell_mut((x, y)) {
            cell.set_char(glyph);
            cell.set_fg(crate::ui::hud::HUD_PHOSPHOR);
            cell.set_style(
