use crate::agent::club::TokenUsage;
use crate::agent::swarm::ledger::MoaTokenReport;
#[cfg(test)]
use crate::platform::overwatch::OverwatchSnapshot;
use crate::ui::glyphs::Glyph;
use crate::ui::hud::{HUD_BLUE, HUD_DANGER, HUD_DIM, HUD_GOLD};
...
const HUD_TEXT_STYLE: Style = Style::new().fg(crate::ui::hud::HUD_TEXT);
...
fn moa_token_row_line(
    row: &crate::agent::swarm::ledger::MoaModelTokenRow,
    denom: u64,
    width: usize,
) -> Line<'static> {
...
pub fn image_viewer_title() -> &'static str {
    ...
    IMAGE_VIEWER_TITLE
        .get_or_init(|| {
            format!(
                " {} · /hide to return ",
                crate::ui::glyphs::media_prefix("image").trim()
            )
        })
        .as_str()
}
...
#[cfg(test)]
pub fn message_title_tabs(tabs: &[crate::agent::club::ClubTab]) -> Cow<'static, str> {
