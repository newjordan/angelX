...
const LOOKUP_TERM_MAX_CHARS: usize = crate::knowledge::library::LOCAL_TOPIC_MAX_CHARS;
...
pub(crate) struct QuickLookup {
    term: String,
    local: crate::knowledge::library::LocalLesson,
    text: String,
    shown: usize,
...
impl QuickLookup {
    ...
    pub(crate) fn for_shelf(shelf_id: &str) -> Option<Self> {
        crate::knowledge::library::local_lesson_for_shelf(shelf_id).map(Self::spawn_local)
    }

    fn spawn_local(local: crate::knowledge::library::LocalLesson) -> Self {
        if !crate::agent::harness::env_flag("ANGEL_QUICK_LOOKUP", true) {
    ...
    fn spawn_local_with_admission(
        ...
        let worker = std::thread::Builder::new()
            ...
            .spawn(move || {
                let result = match crate::ui::term::catch_background_unwind(|| {
    ...
    fn from_local(local: crate::knowledge::library::LocalLesson) -> Self {
        ...
        if let Some(outcome) = outcome {
            ...
            self.text = match outcome {
                Ok(Some(definition)) => {
                    ...
                    if let Some(local) =
                        crate::knowledge::library::local_lesson_with_context(&self.term, &context)
                    {
                        self.local = local;
                    }
    ...
    pub(crate) fn ready(term: &str, title: &str, summary: &str) -> Self {
        ...
        let local = crate::knowledge::library::local_lesson_with_context(term, &context)
...
fn lesson_for_lookup(term: &str) -> crate::knowledge::library::LocalLesson {
    crate::knowledge::library::local_lesson(term)
        .or_else(|| crate::knowledge::library::local_lesson("concept"))
...
pub(crate) fn selected_term(text: &str) -> Option<String> {
    ...
    crate::knowledge::library::normalize_topic(term)
}
...
fn http_agent() -> ureq::Agent {
    ...
    if !crate::platform::yolo::enabled() {
        ...
        builder = builder.resolver(move |netloc: &str| {
            crate::agent::tools::web::resolve_http_target(netloc, allow_private)
        });
    }
    builder.build()
}
