...
fn load_card(
    ...
    crate::ui::term::art::colored_image_braille(&formation.asset_path(), width, height)
}
...
fn solo_pulse(base: &[Cell], frame: usize) -> Vec<Cell> {
    ...
    base.iter()
        .map(|cell| Cell {
            glyph: cell.glyph,
            fg: if crate::ui::term::art::braille_bits(cell.glyph) != 0
...
fn recon_sweep(base: &[Cell], width: usize, height: usize, frame: usize) -> Vec<Cell> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let mut cell = source;
            if distance <= 1 && crate::ui::term::art::braille_bits(cell.glyph) != 0 {
...
fn duel_convergence(base: &[Cell], width: usize, height: usize, frame: usize) -> Vec<Cell> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if x.abs_diff(middle) <= 1 && crate::ui::term::art::braille_bits(cell.glyph) != 0 {
                cell.fg = GOLD;
            }
            merge_cell(&mut out[y * width + dest_x], cell);
        }
...
fn layered_convergence(
    ...
    for &(ox, oy, tint) in offsets {
        ...
        for y in 0..height {
            for x in 0..width {
                let source = base[y * width + x];
                if crate::ui::term::art::braille_bits(source.glyph) == 0 {
                    continue;
                }
...
fn merge_cell(dest: &mut Cell, source: Cell) {
    let bits = crate::ui::term::art::braille_bits(dest.glyph)
        | crate::ui::term::art::braille_bits(source.glyph);
    if bits != 0 {
        dest.glyph = crate::ui::term::art::braille_char(bits);
        dest.fg = source.fg;
    }
}
...
fn dissolve(
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let old_bits = crate::ui::term::art::braille_bits(old.glyph);
            let new_bits = crate::ui::term::art::braille_bits(new.glyph);
            let mut bits = 0u8;
            let mut new_votes = 0u8;
            let mut old_votes = 0u8;
            for local_y in 0..4 {
                for local_x in 0..2 {
                    let bit = crate::ui::term::art::braille_dot_bit(local_x, local_y);
            ...
            out[y * width + x] = Cell {
                glyph: crate::ui::term::art::braille_char(bits),
        ...
        for x in 0..width {
            if (x * 3 + y * 5 + seed).is_multiple_of(11) {
                cells[y * width + x] = Cell {
                    glyph: crate::ui::term::art::braille_char(0x42 | ((seed as u8) & 0x18)),
