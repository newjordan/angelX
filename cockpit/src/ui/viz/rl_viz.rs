...
use crate::drive::reinforce::telemetry::{self, RlPhase, RlSnapshot};
use crate::drive::rl_ctl::{CampaignOutcome, RlMode, RlState, RunProgress};
use crate::ui::hud;
use ratatui::{
...
fn attempt_lines(
    ...
    if chart_h >= 3 && progress.points.len() >= 2 {
        ...
        lines.extend(crate::ui::chart::line_chart(&rewards, width as u16, reward_h as u16).lines);
        if latency_h > 0 {
            ...
            lines.extend(
                crate::ui::chart::line_chart(&latency, width as u16, latency_h as u16).lines,
            );
        }
    }
...
fn attempt_row_line(
    point: crate::drive::rl_ctl::RunPoint,
    first_reward: f32,
    best_reward: f32,
    is_last: bool,
    running: bool,
