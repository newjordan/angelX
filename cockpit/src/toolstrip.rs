...
impl ToolStrip {
    ...
    pub(crate) fn result_event(
        ...
        e.result_digest = Some(crate::cut::sha256_hex(summary.as_bytes())[..16].to_string());
...
impl VerifierFailure {
    pub(crate) fn new(command: &str, summary: &str) -> Self {
        let command = crate::secrets::redact_str(command);
        let command: String = command.chars().take(200).collect();
        let summary = crate::secrets::redact_str(summary);
        ...
        let failure_digest =
            crate::cut::sha256_hex(format!("{command}\0{exit_code}\0{normalized}").as_bytes());
        Self {
            command,
            exit_code,
            tail,
...
pub(crate) fn child_quiet_secs(child: &crate::harness::ChildSnapshot) -> u64 {
    child
        .cpu_age_secs
        .into_iter()
...
pub(crate) fn worker_status_row(child: &crate::harness::ChildSnapshot, width: usize) -> StatusRow {
...
pub(crate) fn delegate_quiet_secs(delegate: &crate::harness::DelegateSnapshot) -> u64 {
    delegate.progress_age_secs.unwrap_or(delegate.elapsed_secs)
}

pub(crate) fn delegate_status_row(
    delegate: &crate::harness::DelegateSnapshot,
    width: usize,
) -> StatusRow {
    ...
    fn rolling_note_large_payload_measurement() {
        for repeat in 1..=5 {
            for bytes in [128, 4_096, 65_536, 1_048_576] {
                ...
                for frame in 0..256 {
                    let started = Instant::now();
                    let row = strip.rolling_note(
                        80,
                        now + Duration::from_millis(frame * 33),
                        true,
                        false,
                    );
...
"#;
        ...
        assert!(
            measured_submission_fingerprint("shell", "cat score.json && grep bench notes.md")
                .is_none()
        );
