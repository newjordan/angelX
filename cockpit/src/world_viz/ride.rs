...
fn composite_rider_overlay(frame: &mut image::RgbaImage, key: cinematics::RiderFrameKey) {
    crate::knight_cast::composite(frame, key);
}
...
mod tests {
    ...
    fn retired_launch_selectors_render_the_default_dotmax_frame() {
        ...
        for alias in [
            "3d", "mesh", "mesh3d", "dotmax", "top", "topdown", "top-down", "2d", "journey",
            "living", "raycast", "ray", "ambient", "art", "typo",
        ] {
    ...
    fn outdoor_rider_overlay_advances_at_clamped_cadence() {
        ...
        each_renderer(|renderer| {
            ...
            let a = world
                ...
                .expect("frame a");
            let ticks_to_next_frame = cinematics::RIDER_FRAME_HOLD_TICKS
                - world.tick % cinematics::RIDER_FRAME_HOLD_TICKS;
    ...
    fn world3d_settled_vistas_invert_values_hold_coverage_and_are_deterministic() {
        ...
        for building in EVERY_BUILDING {
            ...
            let (sky_luma, land_luma) = first.cells.iter().enumerate().fold(
                (0u64, u64::MAX),
                |(sky, land), (index, cell)| {
    ...
    fn every_region_stages_a_distinct_deterministic_frame_through_the_seam() {
        ...
        for region in EVERY_REGION {
            ...
            let (sky_luma, land_luma) = image.cells.iter().enumerate().fold(
                (0u64, u64::MAX),
                |(sky, land), (index, cell)| {
                    let row = index / cells.0;
                    let luma = u64::from(cell.fg[0]) * 3
    ...
    fn dump_region3d_stages_for_review() {
        let _pin = world3d::pin_world3d();
        let out = std::env::var("REALM_REGION3D_DUMP")
            .unwrap_or_else(|_| "../artifacts/z3-dump".to_string());
