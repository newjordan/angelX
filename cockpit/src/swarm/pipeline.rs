...
impl SwarmClub {
    ...
    pub(crate) fn maybe_advise(
        ...
        if !crate::advisor::final_enabled() || answer.trim().is_empty() {
            return answer;
        }
        if crate::advisor::already_annotated(&answer) {
            return answer;
        }
        let task = messages
            .iter()
            ...
            .unwrap_or_default();
        let prompt = crate::advisor::review_prompt(&task, &answer);
        let verdict = match self.call(crate::advisor::ADVISOR_SYS, &[ChatMsg::user(prompt)]) {
            Ok(reply) => crate::advisor::parse_verdict(&reply),
            Err(_) => return answer,
        };
        if verdict.is_clear() {
            return answer;
        }
        match crate::advisor::annotate(&verdict) {
            Some(annotation) => {
    ...
    fn research_block(&self, problem: &str, cancel: &AtomicBool) -> Option<String> {
        ...
        let (grok, searx) = std::thread::scope(|s| {
            let grok = want_grok.then(|| {
                s.spawn(|| {
                    let _budget_scope = crate::harness::formation_budget::enter(budget.clone());
    ...
    fn should_run_grok_research(&self, problem: &str) -> bool {
        self.clubs.research.is_some()
            && (self.k.research
                ...
                || crate::club::grok_research_always()
    ...
    fn propose_waves(
        ...
        let _seat_role = crate::harness::formation_budget::enter_role("propose");
        let ProposalRequest {
            base_sys,
            rest,
            problem,
    ...
    pub(crate) fn judge_select(
        ...
        let _seat_role = crate::harness::formation_budget::enter_role("judge");
        if drafts.len() <= 1 {
            return drafts;
        }
    ...
    fn synthesize(
        ...
        let _seat_role = crate::harness::formation_budget::enter_role("aggregate");
        let cap =
            per_draft_cap_for(&*self.clubs.aggregate, drafts.len()).unwrap_or_else(draft_min_chars);
    ...
    fn reduce_drafts(
        ...
        let _seat_role = crate::harness::formation_budget::enter_role("reduce");
    ...
    fn revise(
        ...
        let _seat_role = crate::harness::formation_budget::enter_role("revise");
        let task = format!(
    ...
    pub(crate) fn run_seeded(
        ...
        crate::experience::record_moa(&crate::experience::MoaExperience {
            driver: &self.name,
            route: trace.route,
            ok: result.is_ok(),
            ...
            judge: judge_on.then_some(crate::experience::JudgeFacts {
                panel: self.k.judge_panel.max(1),
                kept: trace.kept,
            }),
            verify: (verify_rounds > 0).then_some(crate::experience::VerifyFacts {
    ...
    fn run_inner(
        ...
        if self.k.delegate {
            ...
            if !requests.is_empty() {
                ...
                let denials: Vec<Option<crate::swarm_delegate::TestResult>> = requests
                    .iter()
                    .map(|r| {
                        if approve && r.placement.needs_approval() {
                            ...
                            if crate::approval::ask(r.placement.approval_scope(), &prompt)
                                == crate::approval::Decision::Deny
                            {
                                return Some(crate::swarm_delegate::TestResult {
                                    request: r.clone(),
                                    verdict: "denied",
                ...
                let results: Vec<_> = std::thread::scope(|s| {
                    ...
                    handles
                        ...
                        .map(|(h, r)| {
                            h.join()
                                .unwrap_or_else(|_| crate::swarm_delegate::TestResult {
                                    request: r.clone(),
                                    verdict: "error",
        ...
        for layer in 1..k.layers {
            let _seat_role = crate::harness::formation_budget::enter_role("refine");
