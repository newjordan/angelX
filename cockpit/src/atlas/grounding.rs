...
impl AtlasService {
    pub(super) fn validate_code_sources(&self, sources: &[AtlasSource]) -> Result<(), String> {
        ...
        for source in sources.iter().filter(|source| source.kind == CODE_SOURCE) {
            let (path, body) = crate::tools::nav::atlas_source(&self.project_root, &source.id)?;
            if path != source.id || crate::cut::sha256_hex(body.as_bytes()) != source.digest {
                return Err(format!(
    ...
    pub(crate) fn observe_code(
        ...
        let relative = crate::harness::workspace_relative(&self.workspace, Path::new(path))?;
        let workspace = self.workspace.canonicalize().map_err(|e| e.to_string())?;
        let absolute = workspace.join(relative);
        let relative = crate::harness::workspace_relative(&self.project_root, &absolute)?;
        let (path, body) = crate::tools::nav::atlas_source(
            &self.project_root,
            relative.to_str().ok_or("source path must be UTF-8")?,
        )?;
        let sha = crate::cut::sha256_hex(body.as_bytes());
        let mut hints = Vec::new();
        let mut total = 0;
        for (line, text) in body.lines().enumerate() {
            if crate::tools::nav::is_outline_decl(text)
    ...
    pub(super) fn reload_project(&self) -> Result<(), String> {
        self.ensure_enabled()?;
        if !crate::workspace_store::matches_project(
            &self.workspace,
            &self.project_root,
            &self.project_key,
        ) {
        ...
        while let Some(id) = queue.pop_front() {
                ...
                .filter(|source| source.kind == CODE_SOURCE)
            {
                if !digests.contains_key(&source.id) {
                    ...
                    let actual = crate::tools::nav::atlas_source(&self.project_root, &source.id)
                        .ok()
                        .map(|(_, body)| {
                            read_bytes += body.len();
