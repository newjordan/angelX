...
impl crate::App {
    ...
    fn loop_escalate_blocked_verifier(&mut self) -> bool {
        ...
        // Operator law 2026-09-11: nothing pauses unattended work by default. Three identical
        // blocked iterations still produce an escalation record and a blunt instruction, but the
        // loop keeps iterating; only an explicit `ANGEL_LOOP_BLOCKED_REPEAT_LIMIT` pauses it. (The
        // ripe loop on Apollo paused itself at 06:24Z after 3 identical ImportErrors and idled for
...
/// Default **off** — a deepseek/glm/kimi loop must not silently jump to openai
/// just because that label is first in the bag roster.
fn loop_sota_allow_openai() -> bool {
    crate::harness::env_flag("ANGEL_LOOP_SOTA_ALLOW_OPENAI", false)
}
...
/// Pick the loop's SOTA-tier club without defaulting to OpenAI.
///
/// Order:
/// 1. `ANGEL_LOOP_SOTA_CLUB` exact pin (when present and available)
/// 2. In-hand club when it is already SOTA-class and allowed
/// 3. Other available SOTA-class seats, **excluding** OpenAI/Codex unless
///    `ANGEL_LOOP_SOTA_ALLOW_OPENAI=1`
/// 4. OpenAI/Codex only when explicitly allowed (or pinned in step 1)
...
fn preferred_loop_sota_label(
    ...
    let mut ranked: Vec<(u8, String)> = labels
        ...
        .map(|l| {
            ...
            {
                0u8
            } else if is_openai_family_label(&l) {
                3
            } else {
                1
            };
...
mod tests {
    ...
    fn loop_sota_skips_openai_when_disallowed_even_if_first() {
        ...
        let only_oai =
            preferred_loop_sota_label("practice", &["openai".into()], &[true], false, None);
    ...
    fn execution_blocker_pauses_once_without_retry_and_preserves_progress() {
        ...
        let error = crate::harness::execution_blocker("shell", "tool error: shell command failed (exit 1)\nbwrap: setting up uid map: Permission denied").unwrap();
    ...
    fn podrace_submission_clock_steers_a_measured_but_unsubmitted_run() {
        ...
        let measured = |what: &str| ToolStripSnapshot {
            calls: 1,
            verified_outcome_actions: vec![format!(
                "measured:shell:./benchmark.sh {what}:result=ab"
            )],
            ..Default::default()
        };
        ...
        app.loop_harvest_with_tools(
            ...
            ToolStripSnapshot {
                ...
                verified_outcome_actions: vec![
                    "submitted:shell:hilbert submit cand:result=cd".into(),
                ],
                ..Default::default()
            },
        );
    ...
    fn loop_ctl_blocked_tail_redaction_and_stable_digest() {
        ...
        let empty = crate::toolstrip::VerifierFailure::new(
            "./benchmark.sh",
            "shell command failed (exit 1)",
        );
    ...
    fn loop_ctl_blocked_repeat_pause_ledger_and_gauge() {
        ...
        let rows: Vec<_> = app
            ...
            .iter()
            .filter(|m| {
                matches!(m.role, Role::Activity) && m.text.contains("verification blocked —")
            })
            .collect();
    ...
    fn podrace_first_candidate_clock_steers_unmeasured_runs() {
        ...
        let mut app = crate::seed_preview_app();
        app.loop_ctl = podrace();
        app.loop_ctl.verifier_blocked =
            Some("benchctl measure-job: missing required --golden".into());
    ...
    fn unchanged_red_acceptance_executes_once_and_survives_state_reload() {
        ...
        reloaded.last_failed_acceptance.as_mut().unwrap().attempts =
    ...
    fn adventure_quest_walks_the_regions_end_to_end() {
        ...
        app.loop_harvest_with_tools(
            ...
            ToolStripSnapshot {
                calls: 1,
                verified_outcome_actions: vec![
                    "submitted:shell:hilbert submit cand:result=2".into(),
                ],
                ..Default::default()
            },
        );
