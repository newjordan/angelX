    ...
    for action in actions {
        match action {
            StagedAction::Update {
                ...
                let new_tag =
                    crate::hashline::record_snapshot(&path, &String::from_utf8_lossy(&updated));
            ...
            StagedAction::Remove { path, original } => {
                ...
                crate::hashline::invalidate_snapshot(&path);
                undo.push(Undo::Restore {
                    path: path.clone(),
                    content: original,
                });
            ...
            StagedAction::Move {
                ...
                crate::hashline::relocate_snapshot(&from, &to);
                let new_tag = crate::hashline::record_snapshot(&to, &text);
                undo.push(Undo::Restore {
                    path: from.clone(),
                    content: original,
                });
