...
//!
//! Where the [swarm](crate::swarm) attacks a problem *in parallel* (a mixture of
...
//!   [`loop_ctl`](crate::loop_ctl), whose worker does have tools and whose
...
use crate::club::{ChatMsg, ChatRole, Club, ClubReply, StreamDelta, ToolDef};
use crate::iterate::{
...
// builder, finding parsing and dedup all live in [`crate::iterate`] now, shared
...
impl DeliClub {
    ...
    /// context (only [`curated_prompt`](crate::iterate::curated_prompt)), detects
    ...
    fn persist(&self, st: &DeliState) {
        ...
        let Some(workspace) = crate::experience::current_turn_workspace() else {
            return;
        };
...
impl Club for DeliClub {
    ...
    fn metadata(&self) -> Option<crate::club::Metadata> {
        self.inner.metadata()
    }
    fn metadata_cached(&self) -> Option<crate::club::Metadata> {
        self.inner.metadata_cached()
    }
    fn model_identity(&self) -> Option<String> {
        self.inner.model_identity()
    }
    fn route_identity(&self) -> crate::club::RouteIdentity {
...
fn score_run(
    ...
    for lead in &st.hypotheses {
        match crate::iterate::evidence_source(lead) {
            None => out.untagged += 1,
    ...
    fn deli_keeps_rl_tools_available_without_restarting_deliberation_after_a_tool_result() {
        ...
        impl Club for Actions {
            ...
            fn chat(&self, messages: &[ChatMsg], tools: &[ToolDef]) -> Result<ClubReply, String> {
                ...
                if hop == 0 {
                    assert!(
                        messages.iter().any(|m| m.role == ChatRole::Harness
                            && m.content.contains("Deli deliberation"))
                    );
