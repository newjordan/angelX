...
fn recap_preview(text: &str, width: usize) -> String {
    ...
    if single_line.is_empty() {
        ...
        crate::toolstrip::ellipsize(&single_line, width)
    }
}
...
fn mutation_paths(call: &crate::club::ToolCall) -> Vec<String> {
    crate::cut::mutation_targets(&call.name, &call.args)
}
...
impl App {
    ...
    pub(crate) fn change_workspace(&mut self, arg: Option<&str>) -> String {
        ...
        let (new_history, mut registry) = crate::bootstrap::build_history_and_registry(
            &self.bag,
            &new_session.id,
            target.clone(),
        );
        crate::ui_inspect::install(&mut registry, Arc::clone(&self.ui_broker));
        registry.enable_action_capsules();
        self.tools = Arc::new(registry);
        self.atlas = self.tools.atlas();
        self.atlas_view = crate::atlas::AtlasViewState::default();
        self.session = new_session;
        self.history = new_history;
        self.undone_exchange = None;
        self.goal = crate::goal::load_for(&target);
        self.memories = crate::memory::load_for(&target)
            .into_iter()
            .map(Arc::<str>::from)
            .collect();
        ...
        self.world = crate::world_viz::World::for_workspace(&target);
        self.scryglass = crate::scryglass::Scryglass::for_world(self.world.destination());
        self.world
    ...
    pub(crate) fn startup_resume(&mut self, id: Option<String>) {
        ...
        match crate::local_command::resume(id, &workspace) {
            crate::local_command::ResumeResult::Loaded {
                id,
                mut history,
                turns,
            } => {
                ...
                self.rebuild_display();
                self.session = crate::session::Session::with_id_for(&id, &workspace);
                self.system_msg(format!("resumed session {id} ({turns} turns) — continuing"));
            }
            crate::local_command::ResumeResult::NoSession => {
                self.system_msg("--resume: no saved session found — starting fresh".to_string());
            }
            crate::local_command::ResumeResult::Failed(e) => {
    ...
    pub(crate) fn status_text(&self) -> String {
        ...
        if let Some(mission) = crate::mission::latest_mission_line() {
            out.push_str(&format!("\n  mission  {mission}"));
        }
    ...
    pub(crate) fn toggle_approvals(&self, arg: Option<&str>) -> String {
        ...
        if let Some(rest) = raw.strip_prefix("actions") {
            ...
            let current = crate::harness::ActionCapsuleMode::parse(
                std::env::var("ANGEL_ACTION_CAPSULES").ok().as_deref(),
            );
            let next = match rest.trim().to_ascii_lowercase().as_str() {
                "" => match current {
                    crate::harness::ActionCapsuleMode::Off => {
                        crate::harness::ActionCapsuleMode::Approve
                    }
                    _ => crate::harness::ActionCapsuleMode::Off,
                },
                "observe" | "preview" => crate::harness::ActionCapsuleMode::Observe,
                "on" | "1" | "yes" | "approve" => crate::harness::ActionCapsuleMode::Approve,
                "off" | "0" | "no" | "disable" => crate::harness::ActionCapsuleMode::Off,
            ...
            match next {
                ...
                crate::harness::ActionCapsuleMode::Observe => {
                    // TODO: Audit that the environment access only happens in single-threaded code.
            ...
            return format!(
                ...
                match next {
                    crate::harness::ActionCapsuleMode::Off => "OFF",
                    crate::harness::ActionCapsuleMode::Observe => "OBSERVE",
                    crate::harness::ActionCapsuleMode::Approve => "APPROVE",
                },
                if matches!(next, crate::harness::ActionCapsuleMode::Approve) {
    ...
    pub(crate) fn toggle_yolo(&self, arg: Option<&str>) -> String {
        match arg
            ...
            .as_str()
        {
            "on" | "1" | "yes" | "enable" | "enabled" | "full" => crate::yolo::set(true),
            "smart" | "yolos" | "power" => crate::yolo::set_smart(true),
            "off" | "0" | "no" | "disable" | "disabled" | "guarded" => crate::yolo::clear_all(),
            "" | "status" => return crate::yolo::status_text(),
            _ => {
    ...
    pub(crate) fn toggle_yolos(&self, arg: Option<&str>) -> String {
            ...
            .as_str()
        {
            "on" | "1" | "yes" | "enable" | "enabled" | "smart" | "power" => {
                crate::yolo::set_smart(true)
            }
            "off" | "0" | "no" | "disable" | "disabled" | "guarded" => {
                if crate::yolo::smart_enabled() {
                    crate::yolo::set_smart(false);
            ...
            "" | "status" => return crate::yolo::smart_status_text(),
            _ => {
    ...
    pub(crate) fn toggle_comp(&mut self, arg: Option<&str>) -> String {
        ...
        if trimmed.is_empty() || trimmed.eq_ignore_ascii_case("status") {
            return crate::comp_mode::status_text();
        }

        let (subcmd, rest) = crate::loop_ctl::split_word(trimmed);
        ...
        match arg {
            ...
            Some(name) => {
                ...
                if *env == "ANGEL_FALLBACK" {
                    crate::club::set_fallback_armed(!was_on);
                }
                format!(
                    "{name} → {}{}",
    ...
    /// bounded [`crate::compaction`] core — filing them to long-term memory and replacing
    ...
    pub(crate) fn spawn_compact(&mut self) {
        const KEEP: usize = 8;
        let Some((sys_end, window_end)) = crate::compaction::select_window(&self.history, KEEP)
        else {
            self.system_msg(format!(
        ...
        let aux = crate::bootstrap::utility_roster(&self.bag);
        let workspace = self.tools.current_workspace().to_path_buf();
        let wing = crate::compaction::project_wing_for(&workspace);
        let source = crate::compaction::provenance(&self.session.id, "compact");
        let live = store.is_live();
        let use_model = crate::compaction::sync_compaction_uses_model();
        let chunk_threshold = crate::compaction::configured_compact_chunk_tokens();
        let window: Vec<ChatMsg> = self.history[sys_end..window_end].to_vec();
        let summary_window = crate::harness::compaction_summary_window(&window);
        ...
        std::thread::spawn(move || {
            ...
            let result = if use_model {
                ...
                let club = crate::harness::summarizer_for(club, &aux);
                crate::compaction::compact_window_with_state_budget(
                    club.as_ref(),
                    &summary_window,
                    &wing,
                    &source,
                ...
                crate::compaction::compact_window_fast(
                    &summary_window,
                    &wing,
                    &source,
                    chunk_threshold.saturating_mul(2),
            ...
            let outcome = match result {
                Some(result) => {
                    ...
                    note.recovery_context = crate::club::recovery_context_refs(&window);
                    BgOutcome::Compact {
    ...
    pub(crate) fn mention_file(
        &mut self,
        arg: Option<&str>,
    ) -> Option<crate::local_command::EvidenceTurn> {
        ...
        match crate::harness::confined_read_prefix(
            ...
            Ok(prefix) => {
                ...
                crate::harness::truncate_to_char_boundary(&mut content, MENTION_CAP_BYTES);
                ...
                Some(crate::local_command::EvidenceTurn::new(
    ...
    pub(crate) fn import_codex(&mut self, arg: Option<&str>) -> String {
        let sessions = crate::codex_import::list(10);
        ...
        match crate::codex_import::parse(&path) {
            Ok(mut history) if !history.is_empty() => {
                ...
                crate::bootstrap::refresh_history(
                    &self.bag,
                    self.tools.current_workspace(),
                    &mut history,
                );
    ...
    pub(crate) fn run_skill(
        ...
        if arg.is_some_and(|value| value.trim() == "check") {
            self.system_msg(crate::skills::check_for(self.tools.current_workspace()));
            return None;
        }
        ...
        if let Some(query) = skill_search {
            self.system_msg(crate::skills::search_for(
                self.tools.current_workspace(),
                query,
            ));
            return None;
        ...
        let Some(a) = arg else {
            let names = crate::skills::list_for(self.tools.current_workspace());
            if names.is_empty() {
                self.system_msg(format!(
                    "no skills — drop <name>.md files in {} (or set ANGEL_SKILLS_DIR)",
                    crate::skills::dir_display()
                ));
            } else {
                self.system_msg(format!(
        ...
        let catalog = crate::harness::load_skills_for(self.tools.current_workspace());
    ...
    fn commit_memories(&mut self, next: Vec<Arc<str>>, confirmation: String) -> String {
        if let Err(error) = crate::memory::save_for(&next, self.tools.current_workspace()) {
            return format!(
    ...
    pub(crate) fn run_memories(&mut self, arg: Option<&str>) -> String {
        ...
        match sub {
            "add" if !rest.is_empty() => {
                if let Err(error) = crate::memory::validate_add(&self.memories, rest) {
            ...
            _ => {
                if let Err(error) = crate::memory::validate_add(&self.memories, a) {
                    return format!("/memories: {error}");
                }
                let mut next = self.memories.clone();
    ...
    fn spawn_palace_search(&mut self, query: &str) -> String {
        ...
        let wing = crate::compaction::project_wing_for(self.tools.current_workspace());
...
fn select_chroniclers(
    roster: Vec<Arc<dyn crate::club::Club>>,
    in_hand: &str,
    max: usize,
) -> Vec<Arc<dyn crate::club::Club>> {
    let mut seen = std::collections::HashSet::new();
    let mut out: Vec<Arc<dyn crate::club::Club>> = Vec::new();
    for club in roster {
        let label = club.label().to_string();
        if label == "practice" || crate::club::is_sota_label(&label) || club.reports_to_palace() {
            continue;
        }
...
fn chronicle_one(
    ...
    let label = club
        .live_model_name()
        .map(|id| crate::club::short_model_label(&id))
    ...
    let outcome = club.chat_streaming(&msgs, &[], &cancel, &mut |d| match d {
        crate::club::StreamDelta::Reasoning(r) => reasoning.push_str(r),
        crate::club::StreamDelta::Content(c) => content.push_str(c),
...
fn gauntlet_note(charted: &[(String, String)], skipped: &[String]) -> String {
    ...
    let mut map = crate::questmap::render_party(charted, 72);
    if !skipped.is_empty() {
