...
impl App {
    ...
    pub(crate) fn open_module(&mut self, id: &str) -> String {
        let id = crate::runtime::ModuleId::new(id);
    ...
    pub(crate) fn toggle_shell(&mut self) {
        ...
        let shell = crate::runtime::ModuleId::new("shell");
    ...
    pub(crate) fn focus_module(&mut self, id: &str) {
        ...
        let module_id = crate::runtime::ModuleId::new(id);
    ...
    fn focused_scroll_pane(&self) -> crate::mouse::PaneId {
        match self.module_host.focused().map(|id| id.as_str()) {
            Some("agent") if self.module_host.is_running("agent") => crate::mouse::PaneId::AgentBay,
            Some("artifacts") if self.module_host.is_running("artifacts") => {
                crate::mouse::PaneId::Artifacts
            }
            _ => crate::mouse::PaneId::Transcript,
        }
    }
    ...
    pub(crate) fn scroll_focused_view_top(&mut self) {
        match self.focused_scroll_pane() {
            crate::mouse::PaneId::AgentBay => self.reasoning_scroll = usize::MAX,
            crate::mouse::PaneId::Artifacts => self.scryglass.scroll_teaching_top(),
    ...
    pub(crate) fn scroll_focused_view_bottom(&mut self) {
        match self.focused_scroll_pane() {
            crate::mouse::PaneId::AgentBay => self.reasoning_scroll = 0,
            crate::mouse::PaneId::Artifacts => self.scryglass.scroll_teaching_bottom(),
            _ => self.scroll = 0,
        }
    }
    ...
    pub(crate) fn interaction_pane_at(&self, x: u16, y: u16) -> Option<crate::mouse::PaneId> {
        ...
        if self
            .panel_frames
            .get(crate::panels::PanelKind::Artifacts)
            .is_some_and(|rect| crate::mouse::point_in(rect, x, y))
        {
            return Some(crate::mouse::PaneId::Artifacts);
        }
        self.panes.pane_at(x, y).map(|(pane, _)| pane).or_else(|| {
            self.panes.rect_of(crate::mouse::PaneId::AgentBay)?;
            let frame = self.panel_frames.get(crate::panels::PanelKind::AgentBay)?;
            crate::mouse::point_in(frame, x, y).then_some(crate::mouse::PaneId::AgentBay)
        })
    }
    ...
    pub(crate) fn scroll_pane_at(&mut self, x: u16, y: u16, rows: u16, up: bool) {
        ...
        let pane = hit.unwrap_or(crate::mouse::PaneId::Transcript);
        if up {
    ...
    pub(crate) fn scroll_pane_up(&mut self, pane: crate::mouse::PaneId, rows: u16) {
        match pane {
            crate::mouse::PaneId::AgentBay => {
                self.reasoning_scroll = self.reasoning_scroll.saturating_add(usize::from(rows))
            }
            crate::mouse::PaneId::Artifacts if self.scryglass.teaching_surface_open() => {
    ...
    pub(crate) fn scroll_pane_down(&mut self, pane: crate::mouse::PaneId, rows: u16) {
        match pane {
            crate::mouse::PaneId::AgentBay => {
    ...
    pub(crate) fn focus_pane_module(&mut self, pane: crate::mouse::PaneId) {
        match pane {
            crate::mouse::PaneId::Transcript => self.focus_module("core"),
            crate::mouse::PaneId::Input => {
                self.focus_module("core");
                self.tool_strip.pause_for_reading(false);
            }
            crate::mouse::PaneId::AgentBay => self.focus_module("agent"),
            crate::mouse::PaneId::Artifacts => self.focus_module("artifacts"),
            crate::mouse::PaneId::Shell => self.focus_module("shell"),
        }
    }
