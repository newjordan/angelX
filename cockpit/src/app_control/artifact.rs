...
struct BackgroundOutputChunk {
    stream: crate::harness::ProcessStream,
    start: u64,
    end: u64,
}
...
impl BackgroundLiveOutput {
    fn push(&mut self, stream: crate::harness::ProcessStream, bytes: &[u8]) {
        if bytes.is_empty() {
            return;
        }
        let tail = match stream {
            crate::harness::ProcessStream::Stdout => &mut self.stdout,
            crate::harness::ProcessStream::Stderr => &mut self.stderr,
        };
    ...
    fn snapshot(&mut self) -> Option<Arc<str>> {
        ...
        for (index, chunk) in self.chunks.iter().enumerate().rev() {
            let (retained_len, omitted) = match chunk.stream {
                crate::harness::ProcessStream::Stdout => (stdout.len() as u64, self.stdout.omitted),
                crate::harness::ProcessStream::Stderr => (stderr.len() as u64, self.stderr.omitted),
            };
        ...
        for chunk in self.chunks.iter().skip(render_start) {
            let (retained, omitted, omission_announced) = match chunk.stream {
                crate::harness::ProcessStream::Stdout => (
                    stdout.as_slice(),
                    self.stdout.omitted,
                    &mut stdout_omission_announced,
                ),
                crate::harness::ProcessStream::Stderr => (
                    stderr.as_slice(),
                    self.stderr.omitted,
            ...
            if rendered_stream != Some(chunk.stream) {
                if rendered_stream.is_some()
                    || chunk.stream == crate::harness::ProcessStream::Stderr
                {
                    raw.push(0x18);
                    let label = match (rendered_stream, chunk.stream) {
                        (None, crate::harness::ProcessStream::Stderr) => b"[stderr]\n".as_slice(),
                        (_, crate::harness::ProcessStream::Stdout) => b"\n[stdout]\n".as_slice(),
                        (_, crate::harness::ProcessStream::Stderr) => b"\n[stderr]\n".as_slice(),
                    };
            ...
            if !*omission_announced && omitted > 0 {
                ...
                raw.extend_from_slice(
                    format!(
                        ...
                        match chunk.stream {
                            crate::harness::ProcessStream::Stdout => "stdout",
                            crate::harness::ProcessStream::Stderr => "stderr",
                        }
                    )
                    .as_bytes(),
                );
    ...
    fn prune_chunks(&mut self) {
        ...
        self.chunks.retain(|chunk| {
            chunk.end
                > match chunk.stream {
                    crate::harness::ProcessStream::Stdout => stdout_start,
                    crate::harness::ProcessStream::Stderr => stderr_start,
                }
        });
    }
...
impl BackgroundReply {
    ...
    pub(crate) fn output_progress(&self) -> Arc<crate::harness::ToolOutputProgress> {
...
impl MemoryDepositBatch {
    ...
    pub(crate) fn spawn(self, notices: mpsc::Sender<BackgroundNotice>) {
        ...
        crate::harness::enqueue_memory_drawers_with_feedback(store, drawers, move |summary| {
