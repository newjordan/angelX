...
impl App {
    pub(crate) fn remember_brain_route(&self) {
        crate::route_preferences::remember(&self.bag);
    }
    ...
    fn route_choice_for_target(&self, target: (usize, usize)) -> Option<crate::club::RouteChoice> {
        self.bag
            .route_choices()
            .iter()
    ...
    pub(crate) fn open_agent_menu(&mut self, kind: AgentMenuKind) {
        ...
        {
            self.route_evidence = crate::route_intelligence::load_default();
        ...
        let selected = match menu.kind {
            AgentMenuKind::Model => {
                let context_used = self
                    ...
                    .used_tokens
                    .load(std::sync::atomic::Ordering::Relaxed);
                crate::route_intelligence::operational_pick_for_context(
                    &self.route_evidence,
                    &choices,
                    context_used,
                )
        ...
        let selected = match menu.kind {
            AgentMenuKind::Model => {
                let context_used = self
                    ...
                    .used_tokens
                    .load(std::sync::atomic::Ordering::Relaxed);
                crate::route_intelligence::quality_pick_for_context(
                    &self.route_evidence,
                    &choices,
                    context_used,
                )
