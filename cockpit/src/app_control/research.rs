...
use super::*;
use crate::research_workspace::{Action, Entry, Lens, Place, State, bounded};
...
impl App {
    ...
    pub(crate) fn open_research(&mut self, arg: Option<&str>) -> String {
        ...
        self.scryglass
            .navigate(crate::scryglass::StageRoute::Research);
    ...
    pub(crate) fn refresh_research(&mut self) -> String {
        ...
        if let Some(graph) = crate::harness::current_graph_snapshot() {
            let workspace_key = crate::cut::sha256_hex(
                crate::workspace_store::workspace_key(self.tools.current_workspace()).as_bytes(),
            );
            if graph.workspace_key_sha256 == workspace_key {
                use crate::harness::GraphNodePhase;
        ...
        if self.tools.rl().mode == crate::rl_ctl::RlMode::Campaign {
        ...
        if self.tools.rl().mode == crate::rl_ctl::RlMode::Campaign
            && let Some(snapshot) = crate::reinforce::telemetry::current()
        {
