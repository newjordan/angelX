...
pub(crate) fn sandbox_text(workspace: &Path) -> String {
    ...
    let profile = crate::authority_profile::active(false);
    format!(
...
pub(crate) fn clipboard_receipt(
    ...
    let transport = match transport {
        ...
        ClipboardTransport::PayloadTooLarge { encoded_bytes } => format!(
            ...
            crate::mouse::OSC52_MAX_PAYLOAD_BYTES
        ),
...
pub(crate) fn deliver_to_clipboard(text: &str, file_name: &str) -> String {
    ...
    let transport = if !crate::mouse::osc52_payload_fits(text) {
        ClipboardTransport::PayloadTooLarge {
            encoded_bytes: crate::mouse::base64_clipboard_len(text).unwrap_or(usize::MAX),
        }
    } else {
        let seq = crate::mouse::osc52_sequence(text);
