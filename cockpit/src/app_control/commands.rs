...
impl App {
    ...
    fn start_verifier(&mut self, arg: Option<&str>, spec: VerifierCommand) {
        ...
        std::thread::spawn(move || {
            ...
            let receipt = match tools.dispatch_with_cancel_and_progress(
                ...
                Err(error) => {
                    let message = crate::tools::runtime_missing::RuntimeMissing::decode(&error)
    ...
    fn start_approval_selftest(&mut self) {
        ...
        std::thread::spawn(move || {
            ...
            let decision = crate::approval::ask(
                crate::approval::ApprovalScope::ActionBatch(format!(
            ...
            let verdict = match decision {
                crate::approval::Decision::Approve | crate::approval::Decision::ApproveAll => {
                    "approved"
                }
                crate::approval::Decision::Deny => "denied",
            };
    ...
    pub(crate) fn submit(&mut self) {
        ...
        let raw: Arc<str> = if draft.is_empty() {
            ...
            Arc::from(crate::clipboard::IMAGE_QUESTION)
        } else {
            Arc::from(draft)
        };
        ...
        let user_msg = match parsed {
            ...
            ParsedInput::Show(path) => {
                ...
                let text = match crate::media::artifact_from_path_in(
                    &path,
            ...
            ParsedInput::Observatory(command) => {
                ...
                let text = match command {
                    crate::input::ObservatoryCommand::Browse => {
                    ...
                    crate::input::ObservatoryCommand::Campaign(id) => {
                    ...
                    crate::input::ObservatoryCommand::OpenReport(id) => {
            ...
            ParsedInput::Raytrace => {
                let opening =
                    self.scryglass.controller.route() != crate::scryglass::StageRoute::Raytrace;
                if opening {
                    self.observatory.clear_viewport();
                    self.scryglass
                ...
                let text = if opening {
                    let _ = self
                        .module_host
                        .activate(&crate::runtime::ModuleId::new("graph"));
            ...
            ParsedInput::Resume(id) => {
                ...
                match local_command::resume(id, &workspace) {
                    local_command::ResumeResult::Loaded {
                        ...
                        crate::bootstrap::refresh_history(&self.bag, &workspace, &mut history);
                ...
                if opened {
                    ...
                    let message = match self.scryglass.lesson() {
                        ...
                        None => format!(
                            ...
                            crate::library::TUTORS.len(),
                            crate::library::CURRICULUM.len()
                        ),
                    };
                    self.system_msg(message);
                } else {
            ...
            ParsedInput::ModelInfo(Some(arg)) => {
                if matches!(
                    ...
                    let text = match crate::route_preferences::clear() {
            ...
            ParsedInput::Retry => {
                ...
                self.system_msg(format!(
                    ...
                    crate::toolstrip::ellipsize(operator_text.lines().next().unwrap_or(""), 72)
                ));
            ...
            ParsedInput::NewChat => {
                ...
                self.history =
                    crate::bootstrap::build_history(&self.bag, self.tools.current_workspace());
                self.undone_exchange = None;
                self.parked_threads.clear();
                let _ = self.steer_queue.drain();
    ...
    pub(crate) fn launch_pending_turn(&mut self) {
        ...
        if crate::tools::vision::should_apply_vision_sidecar(self.bag.in_hand().as_ref(), &user_msg)
        {
            ...
            self.atlas.build_lens(
                ...
                self.memories.iter().map(|memory| memory.as_ref()).chain(
                    self.history
                        .iter()
                        .filter(|message| {
                            !crate::atlas::is_lens_message(&message.content)
        ...
        crate::campaign::replace_lens_message(&mut self.history, campaign_lens);
        if crate::backplane::mode() == crate::backplane::BackplaneMode::Active {
            ...
            crate::backplane::KnowledgeBroker::replace(&mut self.history, &selection);
            ...
            crate::atlas::replace_lens_message(&mut self.history, atlas_lens.clone());
        }
    ...
    fn turn_context_block(&self, raw: &str, skill_query: &str) -> Option<String> {
        ...
        if crate::backplane::mode() != crate::backplane::BackplaneMode::Active {
            body.push_str(&crate::memory::context_block(&self.memories));
        }
        ...
        if self.relentless_execution {
            controls.push(crate::harness::RELENTLESS_EXECUTION_DIRECTIVE.to_string());
        }
        if self.retard_mode {
            controls.push(crate::harness::RETARD_MODE_DIRECTIVE.to_string());
        }
        if self.solo_mode {
            controls.push(crate::tools::solo::SOLO_MODE_DIRECTIVE.to_string());
        }
        ...
        let magic = crate::magic_keywords::scan(raw);
    ...
    pub(crate) fn flush_queued_steers(&mut self) {
        ...
        if crate::backplane::mode() == crate::backplane::BackplaneMode::Active {
            ...
            let lens = self.atlas.build_lens(
                ...
                self.history
                    .iter()
                    .filter(|message| !crate::atlas::is_lens_message(&message.content))
                    .map(|message| message.content.as_ref()),
            );
            crate::atlas::replace_lens_message(&mut self.history, lens);
        }
    ...
    pub(crate) fn handoff_rl_command(&mut self, arg: Option<&str>) -> String {
        ...
        match verb.as_str() {
            ...
            "start" | "run" | "go" | "on" | "enable" | "every" => {
                ...
                let (iv, task) = crate::loop_ctl::split_interval(&rest);
            ...
            "iters" | "max" | "rolls" => match crate::loop_ctl::parse_usize_or_endless(&rest) {
                Some(n) => {
            ...
            "budget" | "tokens" => match crate::loop_ctl::parse_usize_or_endless(&rest) {
                Some(n) => {
            ...
            "deadline" | "time" | "duration" => {
                ...
                let secs = if crate::loop_ctl::parse_usize_or_endless(&lower) == Some(0)
                    && !lower.chars().all(|c| c.is_ascii_digit())
                {
                    // endless / forever / ∞ words → no deadline
                    Some(0)
                } else {
                    crate::loop_ctl::parse_interval_secs(&rest)
                };
                match secs {
                    Some(secs) => {
            ...
            "reset" | "clear" => {
                self.handoff_rl = crate::handoff_rl::HandoffRlState::default();
                "handoff RL reset to default state".to_string()
            }
            _ => {
                // Bare `/handoff-rl <task>` (optional leading interval) → workshop.
                let (iv, task) = crate::loop_ctl::split_interval(raw);
                self.handoff_rl_open_dialog(task, iv, false)
            }
        }
    }
    ...
    pub(crate) fn handoff_rl_open_dialog(
        ...
        self.loop_dialog = Some(if podrace {
            crate::loop_dialog::LoopLaunchDialog::handoff_rl_podrace(task, interval)
        } else {
            crate::loop_dialog::LoopLaunchDialog::handoff_rl(task, interval)
        });
    ...
    pub(crate) fn handoff_rl_start_with_limits(
        ...
        task: String,
        interval: u64,
        limits: Option<crate::loop_dialog::LoopLaunchSettings>,
    ) -> String {
    ...
    pub(crate) fn handoff_rl_start_immediate(
        ...
        let limits = if podrace {
            Some(
                crate::loop_dialog::LoopLaunchDialog::handoff_rl_podrace(task.clone(), interval)
                    .settings(),
            )
        } else {
            Some(
                crate::loop_dialog::LoopLaunchDialog::handoff_rl(task.clone(), interval).settings(),
            )
        };
        self.handoff_rl_arm_campaign(task, interval, limits)
    }
    ...
    fn handoff_rl_arm_campaign(
        ...
        let (task, max_iters_override) = crate::loop_ctl::parse_loop_start_options(&task);
    ...
    pub(crate) fn force_handoff_rl_restart(
        ...
        let mut history =
            crate::bootstrap::build_history(&self.bag, self.tools.current_workspace());
        history.push(crate::club::ChatMsg::user(note.clone()));
        self.history = history;
        ...
        self.messages.push(Message {
            ...
            text: format!(
                ...
                crate::handoff_rl::HANDOFF_RL_STARTER
            )
            .into(),
        });
        self.messages.push(Message {
    ...
    fn rl_command(&mut self, arg: Option<&str>) -> String {
        ...
        let (verb, rest) = crate::rl_ctl::split_command_verb(arg);
        ...
        match verb {
            "" => {
                if self.scryglass.controller.route() == crate::scryglass::StageRoute::Reinforce {
                    self.scryglass
                        .controller
                        .leave_route(crate::scryglass::StageRoute::Reinforce);
                ...
                if !rl.running() && rl.mode == crate::rl_ctl::RlMode::Idle {
                    let workspace = self.tools.current_workspace().to_path_buf();
                    if let Some((points, outcome)) =
                        crate::rl_ctl::hydrate_latest_campaign(&workspace)
                    {
                        ...
                        rl.mode = crate::rl_ctl::RlMode::Campaign;
                    }
                }
                format!(
                    "rl · Reinforce stage open\n  {}",
                    crate::rl_ctl::RlPlan::usage()
                )
            }
            "run" => {
                let workspace = self.tools.current_workspace().to_path_buf();
    ...
    fn graph_command(&mut self, arg: Option<&str>) -> String {
        ...
        let open_stage = |app: &mut Self| {
            app.focus_module("artifacts");
            app.scryglass
                .navigate(crate::scryglass::StageRoute::AgentGraph);
        };
        ...
        match verb {
            "" => {
                if self.scryglass.controller.route() == crate::scryglass::StageRoute::AgentGraph {
                    self.scryglass
                        .controller
                        .leave_route(crate::scryglass::StageRoute::AgentGraph);
            ...
            "list" => crate::graph_ctl::list_text(),
            "run" => {
                ...
                let roster = crate::bootstrap::delegation_roster(&self.bag);
                match self
                    .agent_graph
            ...
            "status" => crate::graph_ctl::status_text(&self.agent_graph),
    ...
    fn kg_command(&mut self, arg: Option<&str>) -> String {
        ...
        match arg {
            "" | "stats" => {
                let workspace = self.tools.current_workspace().to_path_buf();
                let engine = crate::harness::KnowledgeGraphEngine::new(workspace, None, Vec::new());
                match engine.stats() {
                    Ok(text) => text,
    ...
    fn rl_status_text(&self) -> String {
        ...
        match rl.mode {
            crate::rl_ctl::RlMode::Idle => lines.push(format!(
                "rl · idle — no campaign yet\n  {}",
                crate::rl_ctl::RlPlan::usage()
            )),
            crate::rl_ctl::RlMode::Campaign => {
    ...
    fn run_codex_cmd(&mut self, name: &str, arg: Option<&str>) -> Option<ChatMsg> {
        match name {
            ...
            "moa" => {
                let t = crate::swarm::ledger::report_text(arg, self.tools.current_workspace());
                self.system_msg(t);
            }
            "rollout" => {
            ...
            "dossier" => {
                let t = crate::dossier::status_text(self.tools.current_workspace());
                self.system_msg(t);
            }
            "ledger" | "turns" => {
                let t = crate::harness::ledger_status_text(arg.map(str::trim).unwrap_or(""));
                self.system_msg(t);
            }
            "cut" => {
                let t = crate::cut::status_text(arg, self.tools.current_workspace());
                self.system_msg(t);
            }
            "atlas" => {
            ...
            "vault" => {
                if self.atlas.enabled() {
                    self.atlas_view.set_lane(crate::atlas::AtlasLane::Artifacts);
                    self.focus_module("artifacts");
            ...
            "habits" => {
                let t = crate::habits::run(arg, self.tools.current_workspace());
                self.system_msg(t);
            }
            "refine" => {
                let t = crate::continual_harness::run(arg, self.tools.current_workspace());
                self.system_msg(t);
            }
            "conductor" => {
                let t = crate::conductor::run(arg, self.tools.current_workspace());
                self.system_msg(t);
            }
            "still" => {
                let t = crate::barrel::run(arg);
                self.system_msg(t);
            }
            "repos" => {
                ...
                match query {
                    None => self.system_msg(crate::repos::run(None)), // usage — offline
                    ...
                    Some(q) => {
                        ...
                        std::thread::spawn(move || {
                            let _ = tx.send(BgOutcome::Note(crate::repos::run(Some(&q))));
                        });
                        self.bg_job = Some(job);
                    }
                }
            ...
            "science" => {
                ...
                let query = arg.map(str::trim).filter(|q| !q.is_empty());
                match query {
                    None => self.system_msg(crate::science::run(None)), // usage — offline
                    Some(_) if self.thinking.is_some() || self.bg_job.is_some() => {
                        self.system_msg(
                    ...
                    Some(q) => {
                        ...
                        std::thread::spawn(move || {
                            let _ = tx.send(BgOutcome::Note(crate::science::run(Some(&q))));
                        });
                        self.bg_job = Some(job);
                    }
                }
            ...
            "world" => match arg {
                Some(a) if a.eq_ignore_ascii_case("zoom") => {
                    ...
                    self.scryglass.navigate(crate::scryglass::StageRoute::Realm);
                ...
                Some(a) if a.eq_ignore_ascii_case("3d") => {
                    crate::world_viz::world3d::toggle();
                        ...
                        .is_some_and(|word| word.eq_ignore_ascii_case("view")) =>
                {
                    ...
                    let view = match wanted {
                        Some(word) => match crate::world_viz::world3d::WorldView::parse(word) {
                            Some(view) => Some(view),
                            None => {
                                self.system_msg(format!(
                        ...
                        None => Some(crate::world_viz::world3d::cycle()),
                    };
                    if let Some(view) = view {
                        crate::world_viz::world3d::set(view);
                        self.system_msg(crate::world_viz::world3d::status_line());
                    }
                }
            ...
            "quest" => {
                ...
                if let Some(question) = sub(arg, "gauntlet") {
                    ...
                    if let Some((label, trace)) = source {
                        ...
                        self.scryglass.navigate(crate::scryglass::StageRoute::Quest);
                        self.focus_module("artifacts");
    ...
    fn atlas_command(&mut self, arg: Option<&str>) -> String {
        ...
        let result = match action {
            ...
            "add" => {
                let kind = crate::atlas::AtlasKind::parse(second)
            ...
            "link" => {
                ...
                let kind = match relation.next() {
                    Some("related") => Some(crate::atlas::AtlasLinkKind::Related),
                    Some("derived_from") => Some(crate::atlas::AtlasLinkKind::DerivedFrom),
                    _ => None,
                };
            ...
            "review" => {
                self.atlas_view.set_lane(crate::atlas::AtlasLane::Review);
                self.atlas_view.query.clear();
                Ok("opened Atlas Review lane".to_string())
            }
            "shared" => {
                self.atlas_view.set_lane(crate::atlas::AtlasLane::Shared);
                self.atlas_view.query.clear();
            ...
            query => {
                self.atlas_view.set_lane(crate::atlas::AtlasLane::Project);
    ...
    /// `/retard`: plain-language communication mode until the operator turns it off.
    fn run_retard(&mut self, arg: Option<&str>) -> String {
        match arg
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .map(|s| s.to_ascii_lowercase())
            .as_deref()
        {
            None | Some("on") | Some("start") | Some("arm") | Some("go") => {
                self.retard_mode = true;
                "retard mode ON — literal retard contract active until /retard off".to_string()
            }
            Some("off") | Some("stop") | Some("clear") => {
                self.retard_mode = false;
                "retard mode OFF".to_string()
            }
            Some("status") => format!(
                "retard mode {}",
                if self.retard_mode {
                    "ON — user is literally retarded; extra-simple + bug-free/easy-use checks"
                } else {
                    "OFF"
                }
            ),
            Some(_) => "usage: /retard [on|off|status]".to_string(),
        }
    }
    ...
    fn run_solo(&mut self, arg: Option<&str>) -> String {
            ...
            .as_deref()
        {
            None | Some("on") | Some("start") | Some("arm") | Some("go") => {
                self.solo_mode = true;
                crate::tools::solo::set_solo_mode(true);
            ...
            Some("status") => format!(
                "solo mode {}",
                if self.solo_mode || crate::tools::solo::solo_mode_active() {
    ...
    fn usage_text(&self) -> String {
        ...
        for row in crate::turn::cache_ledger_lines() {
            text.push_str("\n    ");
            text.push_str(&row);
        }
        text
    ...
    fn context_text(&self, arg: Option<&str>) -> String {
        ...
        let tools = self.tools.defs_for_turn(
            window,
            max_hops.is_some(),
            crate::harness::competition_mode_active(&self.history),
        );
        let budget = crate::harness::compaction_budget(
            &*club,
            crate::harness::env_usize("ANGEL_CONTEXT_BUDGET_TOKENS", 0),
        );
    ...
    for message in history {
        let heading = match message.role {
            ...
            ChatRole::Assistant
                if !message.content.trim().is_empty()
                    && !crate::compaction::is_plan_snapshot(&message.content) =>
            {
                "Angel"
            }
            _ => continue,
        ...
        if message.role == ChatRole::User && !message.attachments.is_empty() {
            let images = message
                .attachments
                .iter()
                .filter(|media| matches!(media, crate::club::Media::Image { .. }))
                .count();
        ...
        Some(m) if m.role == ChatRole::System => std::iter::once(m.clone())
            .chain(
                history
                    .iter()
                    .skip(1)
                    .take_while(|message| crate::bootstrap::is_workspace_context(message))
                    .cloned(),
            )
            .collect(),
        _ => Vec::new(),
...
pub(crate) fn context_report(
    ...
    for m in history {
        ...
        match m.role {
            ChatRole::System | ChatRole::Harness if crate::compaction::is_compaction_note(m) => {
                compaction.0 += chars;
                compaction.1 += 1;
            }
            ChatRole::System | ChatRole::Harness
                if m.content
                    .starts_with(crate::harness::AUTO_RECALL_NOTE_PREFIX) =>
            {
                recall.0 += chars;
                recall.1 += 1;
            }
    ...
    let schema_tokens = crate::harness::estimate_tool_tokens(tools);
    out.push_str(&format!(
...
fn append_tool_schema_costs(mut report: String, tools: &[crate::club::ToolDef]) -> String {
    let mut costs = tools
        .iter()
        .map(|tool| {
            let bytes =
...
fn build_broker_selection(
...
) -> crate::backplane::BrokerSelection {
    use crate::backplane::{
    ...
    let dossier = crate::dossier::broker_context_block(workspace);
    if !dossier.trim().is_empty() {
        candidates.push(KnowledgeCandidate::new(
            format!(
                "dossier:{}",
                &crate::cut::sha256_hex(dossier.as_bytes())[..16]
            ),
            project_key,
            "verified-dossier",
    ...
    if let Some(lens) = atlas_lens {
        candidates.extend(crate::backplane::atlas_lens_candidates(lens, project_key));
    }
    ...
    let used = crate::harness::estimate_tokens(history);
    let remaining = context_window
        .map(|window| window.saturating_sub(used))
        .unwrap_or(crate::backplane::RECALL_TOKEN_CEILING + 256);
...
mod tests {
    ...
    fn launch_pending_turn_does_not_block_on_vision_sidecar() {
        ...
        assert!(
            !launch.contains("save_async(&self.history)")
                && !launch.contains("self.history.clone()"),
    ...
    fn detailed_context_orders_active_tool_schema_costs_largest_first() {
        let tools = vec![
            ...
            crate::club::ToolDef {
                ...
                description: "a much longer description that consumes more schema bytes"
                    .to_string(),
