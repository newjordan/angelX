...
mod plan_tests {
    ...
    fn plan_rejects_unknown_flags_and_unnamed_inputs() {
        ...
        let half =
            RlPlan::parse(&workspace, &["--task".to_string(), "tidy".to_string()]).unwrap_err();
        ...
        let zero =
            RlPlan::parse(&workspace, &["--rounds".to_string(), "0".to_string()]).unwrap_err();
    ...
    fn command_line_splitting_is_lossless_and_reports_bad_quotes() {
        assert_eq!(
            split_command_args("--task \"multi word task\" --verify \"python -m unittest\"")
                .unwrap(),
            vec![
                "--task",
                "multi word task",
        ...
        assert_eq!(
            split_command_args(
                r#"--task "keep \"nested\" quotes, $HOME, ; | > # and backslashes""#
            )
            .unwrap(),
            vec![
                "--task",
        ...
        assert_eq!(
            split_command_args("--case audit :: sh tests/audit.sh :: /tmp/dir with spaces")
                .unwrap(),
            vec![
                "--case",
                "audit",
