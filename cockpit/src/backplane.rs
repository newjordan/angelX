...
use crate::club::{Bag, ChatMsg, RouteChoice, RouteIdentity};
...
fn stable_id(namespace: &str, parts: &[&str]) -> String {
    let canonical = parts
        ...
        .collect::<Vec<_>>()
        .join("\u{1f}");
    let digest = crate::cut::sha256_hex(format!("{namespace}\u{1e}{canonical}").as_bytes());
        ...
        for choice in choices.iter() {
            ...
            next.surfaces
                ...
                .or_insert_with(|| SurfaceDescriptor {
                    ...
                    cost_class: if crate::club::is_sota_label(&choice.driver) {
                        "metered".to_string()
                    } else {
                        "local".to_string()
                    },
...
impl KnowledgeCandidate {
    pub(crate) fn new(
        ...
        Self {
            ...
            freshness: 1.0,
            independent: true,
            digest: crate::cut::sha256_hex(normalize_content(&content).as_bytes()),
...
impl KnowledgeBroker {
    pub(crate) fn select(
        ...
        for (_, candidate) in ranked {
            let content = crate::evidence::fence(
    ...
    pub(crate) fn replace(history: &mut Vec<ChatMsg>, selection: &BrokerSelection) {
        history.retain(|message| {
            ...
            message.role != crate::club::ChatRole::Harness || !broker_managed
        });
    ...
    pub(crate) fn append(history: &mut Vec<ChatMsg>, selection: &BrokerSelection) {
        ...
        let previous = history.iter().rev().find(|message| {
            message.role == crate::club::ChatRole::Harness && is_broker_message(&message.content)
        });
    ...
    pub(crate) fn existing_candidates(
        ...
        history
            ...
            .rev()
            .find(|message| {
                message.role == crate::club::ChatRole::Harness
...
pub(crate) fn selected_context(history: &[ChatMsg]) -> (Vec<String>, Vec<String>) {
    ...
    if ids.is_empty() {
        ...
        for (index, message) in history.iter().enumerate() {
            ...
            if message
                .content
                .starts_with(crate::dossier::DOSSIER_BLOCK_HEADER)
            {
                push_context_receipt(
                    &mut ids,
                    &mut digests,
...
fn push_context_receipt(
    ...
    ids.push(id);
    digests.push(crate::cut::sha256_hex(content.as_bytes()));
}
...
fn parse_broker_candidates(block: &str, project_key: &str) -> Vec<KnowledgeCandidate> {
    ...
    while let Some(line) = lines.next() {
        ...
        let mut candidate = KnowledgeCandidate::new(
            ...
            kind,
            authority,
            crate::evidence::unfence(&content),
        );
        if !digest.is_empty() {
...
fn safe_receipt(receipt: String) -> String {
    ...
    .any(|marker| lowercase.contains(marker))
    {
        return format!(
            "redacted:sha256:{}",
            crate::cut::sha256_hex(compact.as_bytes())
        );
    }
...
mod tests {
    ...
    fn broker_provenance_generated_harness_retains_exact_attribution() {
        ...
        let persisted =
            serde_json::to_value(ChatMsg::harness(selection.block.as_ref().unwrap().as_str()))
                .unwrap();
        let restored: ChatMsg = serde_json::from_value(persisted).unwrap();
        let history = vec![
        ...
        assert!(
            !outcome.influence.atlas && !outcome.influence.dossier && !outcome.influence.episodic
        );
        assert!(
