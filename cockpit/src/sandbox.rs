...
fn configured_enabled() -> bool {
    crate::harness::env_flag("ANGEL_SANDBOX", true)
}
...
/// confinement live.
pub fn enabled() -> bool {
    configured_enabled() && !crate::yolo::enabled()
}
...
pub fn prepare(policy: &SandboxPolicy) -> Result<PreparedSandbox, String> {
    ...
    if (crate::yolo::enabled() && !policy.mandatory) || !policy.enforce {
...
fn exec_helper_inner(
    ...
    let confined = policy.enforce && (!crate::yolo::enabled() || policy.mandatory);
    ...
    if !policy.sealed_reads.is_empty() && crate::yolo::enabled() {
...
mod seatbelt_profile_tests {
    ...
    fn seatbelt_profile_mirrors_the_landlock_posture() {
        ...
        assert!(
            profile.contains(
                r#"(allow file-write* (subpath "/angel-nonexistent-root/work \"space\""))"#
            ),
            "{profile}"
        );
