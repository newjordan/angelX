...
//! reports, native to the cockpit and modelled on [`crate::science`].
//!
...
//! the academic fan-out in [`crate::science`] is the wrong index and a raw
...
impl Repo {
    ...
    pub(crate) fn line(&self) -> String {
        let name = crate::science::sanitize_metadata(&self.full_name, MAX_NAME_CHARS);
        ...
        let lang = self
            .language
            .as_deref()
            .map(|l| crate::science::sanitize_metadata(l, 40))
            .filter(|l| !l.is_empty())
        ...
        let desc = crate::science::sanitize_metadata(&self.description, MAX_DESC_CHARS);
...
impl RepoSet {
    ...
    pub(crate) fn brief(&self, mode: Mode, limit: usize) -> String {
        let mut out = format!(
            ...
            crate::science::sanitize_metadata(&self.query, 200),
            mode.label(),
            self.repos.len(),
        ...
        if !self.notes.is_empty() {
            let notes = self
                .notes
                .iter()
                .map(|n| crate::science::sanitize_metadata(n, 240))
                .collect::<Vec<_>>();
...
fn status_reason(resp: ureq::Response) -> String {
    ...
    let message = serde_json::from_str::<serde_json::Value>(&raw)
        ...
        .unwrap_or(raw);
    crate::science::sanitize_metadata(&message, 200)
}
...
fn parse_github(v: &serde_json::Value) -> Vec<Repo> {
    ...
    for w in items {
        let full_name = crate::science::sanitize_metadata(
        ...
        let description = crate::science::sanitize_metadata(
        ...
        let topics = w
            ...
            .map(|a| {
                a.iter()
                    ...
                    .map(|t| crate::science::sanitize_metadata(t, MAX_TOPIC_CHARS))
                    .filter(|t| !t.is_empty())
                    .take(MAX_TOPICS)
                    .collect()
            })
...
/// briefing. No query → usage. Mirrors [`crate::science::run`].
pub(crate) fn run(arg: Option<&str>) -> String {
    ...
    if set.repos.is_empty() {
        ...
        return format!(
            "repos \"{}\": {why}",
            crate::science::sanitize_metadata(query, MAX_QUERY_CHARS)
        );
    }
    set.brief(mode, 10)
}
