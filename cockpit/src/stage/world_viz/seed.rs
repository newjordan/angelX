...
pub(crate) fn town_name(seed: u64) -> String {
    ...
    let h = crate::stage::identity::TOWN_HEAD[(seed % 12) as usize];
    let m = crate::stage::identity::TOWN_MID[((seed / 12) % 6) as usize];
    let t = crate::stage::identity::TOWN_TAIL[((seed / 72) % 8) as usize];
    format!("{h}{m}{t}")
}
