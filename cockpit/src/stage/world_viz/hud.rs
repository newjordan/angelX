...
use crate::ui::hud::{HUD_AMBER, HUD_DIM, HUD_GOLD, HUD_TEXT};
...
impl World {
    ...
    pub(crate) fn quest_hud(&self, width: usize) -> Option<Line<'static>> {
        ...
        // one source of truth.
        if !crate::drive::comp_mode::ambient_stage_sim_allowed() || width == 0 {
            return None;
        }
    ...
    pub(crate) fn status_line(&self, width: usize) -> Line<'static> {
        ...
        let memory_status = (self.target == Building::Chapel).then_some(match self.memory_health {
            crate::knowledge::memory::store::MemoryHealth::Disabled => {
                (" · memory disabled", crate::ui::hud::HUD_DIM)
            }
            crate::knowledge::memory::store::MemoryHealth::Healthy => {
                (" · memory healthy", crate::ui::hud::HUD_VERIFIED)
            }
            crate::knowledge::memory::store::MemoryHealth::Degraded => {
                (" · memory degraded", crate::ui::hud::HUD_DANGER)
            }
        });
        ...
        let atlas_span = (self.target == Building::Chapel
            ...
            .then(|| {
                let color = match self.atlas_health {
                    crate::knowledge::atlas::AtlasHealth::Healthy => crate::ui::hud::HUD_VERIFIED,
                    crate::knowledge::atlas::AtlasHealth::Disabled => crate::ui::hud::HUD_DIM,
                    crate::knowledge::atlas::AtlasHealth::Degraded
                    | crate::knowledge::atlas::AtlasHealth::Inert => crate::ui::hud::HUD_DANGER,
                };
                Span::styled(
                    format!(
