...
#[cfg(test)]
pub(crate) use terrain::on_road_band;
...
const DISTRICT_FRACTIONS: [(f64, f64); 8] = crate::stage::identity::district_fractions();
...
impl Building {
    pub(crate) fn label(self) -> &'static str {
        crate::stage::identity::landmark(self).display_name
    }
...
pub(crate) struct World {
    ...
    memory_health: crate::knowledge::memory::store::MemoryHealth,
    atlas_health: crate::knowledge::atlas::AtlasHealth,
    atlas_review_count: usize,
    clerk_health: crate::knowledge::atlas_clerk::ClerkHealth,
    ...
    hearth: crate::stage::hearth::HearthState,
    ...
    village: Option<crate::stage::village::Village>,
...
impl World {
    ...
    pub(crate) fn new(seed: u64) -> Self {
        let mut world = World {
            ...
            event_seq: 0,
            event_diagnostics: 0,
            memory_health: crate::knowledge::memory::store::MemoryHealth::Disabled,
            atlas_health: crate::knowledge::atlas::AtlasHealth::Disabled,
            atlas_review_count: 0,
            clerk_health: crate::knowledge::atlas_clerk::ClerkHealth::Idle,
            clerk_queue_depth: 0,
    ...
    pub(crate) fn note_memory_health(
        &mut self,
        health: crate::knowledge::memory::store::MemoryHealth,
    ) {
    ...
    pub(crate) fn note_atlas_status(
        &mut self,
        health: crate::knowledge::atlas::AtlasHealth,
        review_count: usize,
    ) {
    ...
    pub(crate) fn memory_health(&self) -> crate::knowledge::memory::store::MemoryHealth {
        self.memory_health
    }
    ...
    fn load_rewards(path: &std::path::Path) -> LoadedRewards {
        ...
        let hearth =
            serde_json::from_value::<crate::stage::hearth::HearthState>(v["hearth"].clone())
                .unwrap_or_default();
    ...
    pub(crate) fn enable_village(
        &mut self,
        state: crate::stage::village::VillageState,
        ...
        self.village = Some(crate::stage::village::Village {
    ...
    pub(crate) fn note_village(&mut self, pulse: crate::stage::village::VillagePulse) {
        ...
        match pulse.forge {
            ...
            Some(f) => {
                    ...
                    && v.state.last_adapter.as_deref() != Some(adapter.as_str())
                {
                    ...
                    let new = crate::stage::village::adapter_level(&adapter);
        ...
        if dirty && let Some(p) = &v.path {
            crate::stage::village::save_state(p, &v.state);
        }
    ...
    pub(crate) fn village_report(&self) -> String {
        ...
        let mut out = format!(
            "{}{}{}",
            crate::stage::identity::VILLAGE_HEADING_PREFIX,
            self.town_name,
            crate::stage::identity::VILLAGE_HEADING_SUFFIX
        );
        ...
        out.push_str(&format!(
            ...
            v.new_samples,
            f64::from(crate::stage::village::granary_fill(v.state.dataset_total)) * 100.0,
            v.ollama_models,
        ));
    ...
    fn tick_hearth(&mut self) {
        if !self
            .tick
            .is_multiple_of(crate::stage::hearth::HEARTH_BEAT_TICKS)
        {
            return;
        }
        ...
        let (ollama, heads_lit, forge_hot, grain) = match &self.village {
            Some(v) => {
                ...
                let grain =
                    (crate::stage::village::granary_fill(v.state.dataset_total) * 100.0) as u32;
    ...
    fn tick_muster(&mut self) {
        ...
        match crate::ui::viz::agentviz::current() {
            None => {
                self.muster.clear();
                self.muster_seq = 0;
