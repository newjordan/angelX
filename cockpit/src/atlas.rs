...
impl AtlasService {
        ...
        let root = std::env::var_os("ANGEL_ATLAS_DIR")
            ...
            .unwrap_or_else(|| {
                if cfg!(test) {
                    ...
                    crate::workspace_store::angel_subdir("atlas")
                }
            });
    ...
    pub(crate) fn build_lens<'a>(
        ...
        for ranked in selected {
            ...
            let line = format!(
                ...
                crate::evidence::neutralize_sentinels(&content).replace("\n- [", "\n- (·"),
                why
            );
            if LENS_HEADER.len()
                + LENS_SENTINEL.len()
                + 2
                + crate::evidence::fence("atlas", &provenance, &format!("{block}{line}")).len()
                > MAX_LENS_BYTES
            {
                break;
            }
        ...
        let block = format!(
            "{LENS_HEADER}{}\n{LENS_SENTINEL}",
            crate::evidence::fence("atlas", &provenance, &block)
        );
    ...
    fn write_lesson(&self, action: &str, proposal_id: &str, final_item: Option<&AtlasItem>) {
        ...
        crate::experience::append_jsonl(&self.lesson_path, &row);
    }
}
...
fn write_snapshot(
    ...
    let bytes = crate::secrets::to_redacted_vec(snapshot)
        .map_err(|error| format!("serialize Atlas snapshot: {error}"))?;
    if bytes.len() as u64 > crate::store_caps::value("atlas_snapshot", "max_bytes") {
...
impl FileLock {
    fn acquire(path: &Path) -> Result<Self, String> {
        ...
        #[cfg(unix)]
        crate::workspace_store::lock_store(&file, "atlas_snapshot_lock")
...
fn sanitize_text(value: &str, root: &Path, max_bytes: usize) -> Result<String, String> {
    ...
    Ok(bounded(
        crate::secrets::redact_str(value.trim()).as_ref(),
        max_bytes,
    ))
}
