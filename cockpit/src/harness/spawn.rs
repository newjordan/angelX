...
fn bounded_moa_synthesis_with_deadline(
    ...
    if let Some(deadline) = deadline {
        ...
        return match rx.recv_timeout(remaining) {
            Ok(Ok(crate::club::ClubReply::Text(answer))) if !answer.trim().is_empty() => {
                Some(answer)
            }
    ...
    match rx.recv() {
        Ok(Ok(crate::club::ClubReply::Text(answer))) if !answer.trim().is_empty() => Some(answer),
        Ok(_) | Err(_) => None,
    }
}
...
pub(crate) fn load_personas() -> Vec<Skill> {
    let embedded = [
        ...
        ("skeptic", include_str!("../../personas/skeptic/PERSONA.md")),
        (
            "speed-freak",
...
impl SpawnTool {
    ...
    fn seat_clubs(&self, spec: &str, n: usize) -> Result<Vec<Arc<dyn Club>>, String> {
        ...
        match self.clubs.get(&spec) {
            Some(c)
                if crate::tools::consult::is_optional_local_label(c.label())
                    && !c.is_available() =>
            {
                let live: Vec<&Arc<dyn Club>> = self
                    ...
                    .values()
                    .filter(|peer| {
                        crate::tools::consult::is_optional_local_label(peer.label())
                            && peer.is_available()
                    })
                    .collect();
                if !live.is_empty() {
            ...
            None if crate::club::is_sota_label(&spec)
