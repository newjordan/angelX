...
pub(crate) fn is_permanent_provider_error(error: &str) -> bool {
    if crate::club::error_indicates_auth_failed(error)
        || crate::club::error_indicates_quota_exhausted(error)
    {
        return true;
    }
...
pub(crate) fn is_recoverable_stream_error(error: &str) -> bool {
    error.starts_with(crate::club::INCOMPLETE_STREAM_ERR) || error.contains("stream stalled")
}
...
pub(crate) fn mutation_call_paths(call: &ToolCall) -> Vec<String> {
    ...
    match call.name.as_str() {
        ...
        "apply_patch" => {
            ...
            for p in crate::cut::mutation_targets(&call.name, &call.args) {
