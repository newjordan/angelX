...
fn configured_unproductive_policy(metered_sota: bool, competition: bool) -> UnproductivePolicy {
    ...
    let escalate = env_usize(
        ...
        if metered_interactive { 8 } else { 0 },
    );
    if competition {
    ...
    if metered_sota {
        UnproductivePolicy {
            escalate,
            // Observation stays useful, but termination is an operator choice.
            // Long implementation and verification runs are not failures just
            // because the local progress classifier cannot credit their work.
            stop: env_usize("ANGEL_UNPRODUCTIVE_STREAK_STOP", 0),
        }
    } else {
        ...
        UnproductivePolicy {
            escalate,
            stop: if task_active {
                env_usize("ANGEL_UNPRODUCTIVE_STREAK_STOP", 0)
            } else {
                0
            },
        }
...
macro_rules! turn_counters {
    (
        ...
        crate::experience::TurnCounters {
            ...
            tool_errors_by_class: crate::experience::ToolErrorClasses::default(),
    ...
    pub(crate) fn finish(&self, turn_elapsed_ms: u128) -> TaskTimingTelemetry {
        let mut result = TaskTimingTelemetry {
            ...
            calls: serde_json::json!({"model_calls":self.model_samples,
                ...
                "provider_calls":crate::harness::trajectory::provider_call_samples()}),
            other_ms: turn_elapsed_ms
                .saturating_sub(self.model_ms)
                .saturating_sub(self.tool_ms),
        };
        result.refresh_task_lifecycle();
        crate::harness::trajectory::retain_task_timing(&result);
        result
    }
}
...
impl TaskTimingTelemetry {
    pub(crate) fn refresh_task_lifecycle(&mut self) {
        if let Some((wall, startup, shutdown, phases)) =
            crate::harness::trajectory::task_lifecycle_snapshot()
        {
            self.wall_ms = wall;
...
fn task_acceptance_snapshot(
    ...
    Some(TaskAcceptanceTelemetry {
        ...
        command_sha256: command
            .map(|command| crate::cut::sha256_hex(command.as_bytes()))
            .unwrap_or_default(),
        armed,
...
fn inject_task_proc_completions(
    ...
    let notices =
        crate::tools::proc::take_completions(&registry.workspace_boundary().canonical_root, 8);
...
/// turn being interrupted (see [`crate::steer`]). `None` — sub-agents, bounded
...
pub fn run_turn_steered(
    ...
    max_hops: Option<usize>,
    events: &mpsc::Sender<TurnEvent>,
    steers: Option<&crate::steer::SteerQueue>,
) -> Result<String, String> {
    run_turn_steered_observed(
...
fn run_turn_steered_observed(
    ...
    let idle = crate::turn::configured_turn_idle_timeout_secs().map(Duration::from_secs);
    ...
    std::thread::scope(|scope| {
        ...
        let watcher = scope.spawn(move || {
            ...
            while !done.load(Ordering::Acquire) {
                ...
                if effective_cancel.load(Ordering::Acquire) {
                    crate::tools::proc::stop_owned_for(
                        owner,
        ...
        impl Drop for Finish<'_> {
            fn drop(&mut self) {
                ...
                if std::thread::panicking() {
                    crate::tools::proc::stop_owned(self.1);
                }
            }
        }
        ...
        if let Some(reason) = reason {
            crate::tools::proc::stop_owned_for(owner, reason);
        }
        drop(finish);
        drop(relay_tx);
        let _ = watcher.join();
        let kills = crate::tools::proc::take_owned_kills(owner);
        crate::harness::trajectory::note_proc_kills(&kills);
        ...
        if let Ok(outcome) = &mut result {
            outcome.tools = crate::harness::trajectory::tool_ledger_snapshot();
        }
        if timed_out.load(Ordering::Acquire) {
            return match result {
                Ok(mut outcome) => {
...
fn run_turn_tiered(
    ...
    crate::experience::note_turn_workspace(registry.current_workspace());
    ...
    let project_doc_bytes = history
        .iter()
        .take_while(|message| crate::bootstrap::is_pinned_preamble(message))
        .find_map(crate::bootstrap::project_doc_bytes)
        .unwrap_or(0);
    ...
    crate::harness::trajectory::note_turn_session(&registry.session_id);
    ...
    let error_stop = env_usize("ANGEL_ERROR_LIMIT", 0);
    ...
    let tool_errors_by_class = std::cell::Cell::new(crate::experience::ToolErrorClasses::default());
    ...
    let hop_cache_ledger = crate::turn::cache_hop_ledger_enabled();
    ...
    let mut prev_defs_fingerprint = crate::turn::defs_fingerprint(&defs);
    ...
    let write_exp =
        |stop: &str, ok: bool, hop: usize, mut counters: crate::experience::TurnCounters| {
            ...
            crate::experience::record_turn(
                &crate::experience::TurnExperience {
                    ...
                    counters,
                    tokens,
                    failovers: crate::experience::drain_failovers(),
                },
                registry.current_workspace(),
            );
        };
    ...
    // (`crate::cut::turn_reward` documents the semantics). A turn that wrote no
    // source earns no verdict and stays unlabeled, exactly as before.
    let mut verdicts = crate::cut::TurnVerdicts::default();
    let mut post_write_verification = crate::cut::PostWriteVerification::default();
    ...
    let task_accept_requested = std::env::var("ANGEL_TASK_ACCEPT_CMD")
        ...
        .filter(|command| {
            ...
            !registry.external_evaluator_only
                && crate::harness::lane::subcall_depth() == 0
    ...
    macro_rules! observed_outcome {
        ($outcome:expr_2021, $source:expr_2021) => {{
            ...
            crate::harness::trajectory::note_task_answer();
            outcome.acceptance = task_acceptance_snapshot(
                ...
                || {
                    crate::harness::workspace_state::workspace_evidence_sha256(
                        registry.current_workspace(),
                    )
                },
            );
    ...
    macro_rules! handle_unproductive_streak {
        () => {{
            ...
            let (notice, diagnosis) = crate::harness::trajectory::unproductive_escalation(
                hop,
                streak_escalate,
                streak_stop,
            );
            ...
            if let Some(note) = diagnosis {
                ...
                crate::harness::trajectory::note_timing(
                    &timing.finish_with_history(turn_start.elapsed().as_millis(), history),
                );
                crate::harness::trajectory::note_stop_reason("escalated_unproductive");
    ...
    'turn: loop {
        ...
        if cancel.load(Ordering::Relaxed) {
            ...
            crate::harness::trajectory::note_stop_reason(TurnStopReason::Interrupt.as_str());
            ...
            || ((research_compose || research_compose_sent) && !compose_fits)
        {
            ...
            crate::harness::trajectory::note_stop_reason(TurnStopReason::Deadline.as_str());
            log_trajectory(club, history, &note, hop, true, verdicts.reward());
            write_exp(
                "deadline",
                false,
            ...
            && hop >= max
        {
            ...
            crate::harness::trajectory::note_stop_reason(TurnStopReason::MaxHops.as_str());
        ...
        if let Some(q) = steers {
            let queued = q.drain();
            if !queued.is_empty() {
                history.push(crate::steer::context_message());
                hop_breakers.push("steer injection");
            }
            for msg in queued {
                let _ = events.send(TurnEvent::Notice(format!(
                    "steer delivered → {}",
                    crate::steer::snippet(&msg.content)
                )));
                history.push(msg);
            }
        }
        ...
        if activation_gen != prev_activation_gen {
            ...
            let next_fp = crate::turn::defs_fingerprint(&defs);
        ...
        if final_mile_answer_only && !final_mile_answer_notice_sent {
            ...
            crate::harness::trajectory::note_escalation(hop, "final_mile_answer_advisory");
        ...
        let bg_defer = crate::compaction::sync_compaction_uses_model()
            && bg_owner
        ...
        if final_mile_answer_only && !crate::club::final_response_requested(history) {
        ...
        if research_compose {
            ...
            crate::harness::trajectory::note_escalation(hop, "research_compose");
            let _ = events.send(TurnEvent::Notice(
        ...
        let reply = loop {
            ...
            crate::harness::trajectory::begin_model_request();
                ...
                match effective_effort.as_deref() {
                    Some(effort) => club.chat_streaming_with_effort(
                        ...
                        &mut |delta| match delta {
                            crate::club::StreamDelta::Content(t) => {
                                if !t.is_empty() {
                            ...
                            crate::club::StreamDelta::Reasoning(r) => {
                                if !r.is_empty() {
                            ...
                            crate::club::StreamDelta::Heartbeat => {
            ...
            let result = result.and_then(|reply| match reply {
                crate::club::ClubReply::Text(ref text)
            ...
            match result {
                Ok(reply) => {
                    ...
                    {
                        crate::harness::trajectory::note_route_switch(
                            hop,
                            &last_answer_route,
                            &resolved,
                        );
                    ...
                    glm_last_hop_answered.set(matches!(reply, crate::club::ClubReply::Text(_)));
                    ...
                    if let Some((cache_snap, input_snap)) = hop_cache_before {
                        ...
                        let sample = crate::turn::HopCacheSample {
                            hop,
                        ...
                        if let Some(notice) = crate::turn::cache_ledger_fold(club.label(), sample) {
                    ...
                    if turn_expired(turn_start.elapsed().as_secs(), turn_budget) {
                        ...
                        if research_turn
                            && let crate::club::ClubReply::Text(answer) = &reply
                            && !answer.trim().is_empty()
                            && !crate::club::contains_raw_tool_markup(answer)
                        {
                ...
                Err(err) => {
                    ...
                    crate::harness::trajectory::note_escalation(hop, "provider_unavailable");
                    crate::harness::trajectory::note_stop_reason(
        ...
        let reply = match reply {
            crate::club::ClubReply::Calls(calls) if final_mile_answer_only => {
                ...
                let _ = crate::club::take_pending_tool_reasoning();
                let names = calls
                    .iter()
                ...
                crate::club::ClubReply::Text(format!(
            ...
            crate::club::ClubReply::Text(answer)
                if final_mile_answer_only && crate::club::contains_raw_tool_markup(&answer) =>
            {
            ...
            crate::club::ClubReply::Text(answer) if toolcall_scavenge && !defs.is_empty() => {
                let recovered = crate::club::scavenge_stranded_tool_calls(&answer, &defs);
                if recovered.is_empty() {
                    ...
                    crate::club::ClubReply::Calls(recovered)
                }
            }
            reply => reply,
        };
        match reply {
            crate::club::ClubReply::Text(answer) => {
                ...
                if task_capture.is_some()
                    && crate::tools::proc::owned_work_pending(proc_owner, proc_workspace)
                {
                    ...
                    if final_mile_answer_only {
                        ...
                        crate::harness::trajectory::note_stop_reason(source);
                ...
                let raw_markup = !defs.is_empty() && crate::club::contains_raw_tool_markup(&answer);
                if raw_markup {
                ...
                if let Some(command) = task_accept_cmd.as_deref() {
                    ...
                    if passed {
                        crate::harness::trajectory::note_verified(
                    ...
                    )
                {
                        ...
                        && operator_releases_unverified_completion(registry.current_workspace())
                    {
                        ...
                        let _ = events.send(TurnEvent::SuppressPartial);
                        history.push(ChatMsg::assistant(answer));
                        crate::harness::trajectory::note_escalation(hop, "final_verify_advisory");
                ...
                {
                    crate::harness::trajectory::note_escalation(hop, "no_edit_advisory");
                ...
                crate::harness::trajectory::note_research_answer(&answer);
                crate::harness::trajectory::note_task_answer();
                crate::barrel::capture_answer(club, registry.current_workspace(), history, &answer);
                history.push(ChatMsg::assistant(answer.clone()));
                crate::harness::trajectory::note_timing(
                    &timing.finish_with_history(turn_start.elapsed().as_millis(), history),
                );
                ...
                crate::harness::trajectory::note_stop_reason(TurnStopReason::Answer.as_str());
            ...
            crate::club::ClubReply::Calls(mut calls) => {
                ...
                let private_reasoning = crate::club::take_pending_tool_reasoning();
                    ...
                    && first_write_rejections >= first_write_rejection_limit
                {
                    ...
                    crate::harness::trajectory::note_stop_reason(TurnStopReason::Spin.as_str());
                    log_trajectory(club, history, &note, hop, true, verdicts.reward());
                    write_exp(
                        "first_write_stop",
                        false,
                ...
                if poll_guard_suppressing {
                    ...
                    if !passive_poll_nudge_sent {
                        crate::harness::trajectory::note_escalation(hop, "passive_poll_advisory");
                ...
                if final_mile_rejects_inspection(final_mile_active, verification_needed, &calls) {
                    crate::harness::trajectory::note_escalation(hop, "final_mile_advisory");
                ...
                }
                {
                    if spin_stop > 0 && count_spin && spin >= spin_stop {
                        crate::harness::trajectory::note_escalation(hop, "spin_stop");
                ...
                let deny_actions = if let Some(batch) = &action_batch {
                    ...
                    if action_capsule_mode.needs_approval() {
                        ...
                        let denied = matches!(decision, crate::approval::Decision::Deny);
                        if denied {
                ...
                let mut dispatch_one = |index: usize, call: &ToolCall, gate: &AtomicBool| {
                    ...
                    let result = if first_write_denied_this {
                        ...
                        if last_storm_notice.as_deref() != Some(call.name.as_str()) {
                            crate::harness::trajectory::note_escalation(
                                hop,
                    ...
                    if outcome.execution == ExecutionOutcome::Succeeded
                        ...
                        && let Some((kind, label, url)) =
                            crate::media::presentation_from_result(&result)
                    {
                ...
                for (result, _, outcome) in results.iter() {
                    ...
                    if is_error_result(result) {
                        ...
                        classes.record(crate::experience::classify_tool_error(result));
                    ...
                    results.into_iter().enumerate()
                {
...

                    {
                        ...
                        let class = errored
                            .then(|| crate::experience::classify_tool_error(&result).label());
                        ...
                        crate::harness::trajectory::note_last_tool_attribution(
                            tool_shares[call_index],
                        );
                        if let Some(entry) = registry.routed_execution(call, &result) {
                            crate::harness::trajectory::note_tool_routing(&entry.receipt);
                        }
                    }
                    if started && is_mutation_call(call) {
                        ...
                        for path in crate::cut::mutation_targets(&call.name, &call.args) {
                    ...
                    if succeeded && is_mutation_call(call) {
                        ...
                        if is_first_write_progress_call(call) {
                            ...
                            crate::harness::trajectory::note_first_action(
                                hop,
                                turn_start.elapsed().as_millis() as u64,
                            );
                ...
                crate::harness::trajectory::note_progress_hop(progress_mutated);
                if repeated_poll_guard.observe(repeated_poll_fingerprint, progress_mutated) {
                    ...
                    crate::harness::trajectory::note_escalation(hop, "repeated_poll_stop");
                    crate::harness::trajectory::note_stop_reason(TurnStopReason::Spin.as_str());
                    log_trajectory(club, history, &note, hop, true, verdicts.reward());
                    write_exp(
                        "spin",
                ...
                if let Some(detector) = tool_cycle.as_mut() {
                    ...
                    {
                        crate::harness::trajectory::note_escalation(hop, "spin_cycle");
                ...
                if consecutive_verification_failures >= 3 && !verification_recovery_emitted {
                    ...
                    crate::harness::trajectory::note_escalation(hop, "verification_recovery");
                    history.push(ChatMsg::harness(
                        "[harness-telemetry] VERIFICATION RECOVERY: 3 consecutive verification failures detected. Pause speculative edits and inspect the first failing diagnostic. Preserve the current diff and all pre-existing work. Repair the smallest supported cause, or selectively undo only your own known-bad change after saving it. Report infrastructure failures honestly; never discard unrelated changes or assume a clean baseline exists."
                            .to_string(),
                    ));
                    let _ = events.send(TurnEvent::Notice(
                ...
                if green_verify_achieved && !green_verify_nudge_emitted {
                    ...
                    crate::harness::trajectory::note_escalation(hop, "green_verify_advisory");
                    ...
                    && !post_edit_logic_reviewed
                {
                    ...
                    crate::harness::trajectory::note_escalation(hop, "post_edit_logic_advisory");
                ...
                if spin > 0 && spin.is_multiple_of(spin_nudge) {
                    crate::harness::trajectory::note_escalation(hop, "spin_advisory");
                ...
                if all_errored {
                    if err_streak > 0 && err_streak.is_multiple_of(error_nudge) {
                        crate::harness::trajectory::note_escalation(hop, "error_advisory");
                        history.push(ChatMsg::harness(ERROR_NUDGE.to_string()));
                    }
                    if error_stop > 0 && err_streak >= error_stop {
                        crate::harness::trajectory::note_escalation(hop, "error_stop");
                        let note = format!(
...
///    ([`crate::cut::PostWriteVerification`]). On by default; `ANGEL_CUT_VERIFY=0` opts out.
///
...
fn apply_final_advisor(
    ...
    if !crate::advisor::final_enabled()
        || answer.trim().is_empty()
        || crate::advisor::already_annotated(&answer)
    {
        return answer;
    }
    ...
    let prompt = format!(
        "{}\n\n{}",
        crate::advisor::ADVISOR_SYS,
        crate::advisor::review_prompt(&task, &answer)
    );
    registry.auxiliary.utility_entered("advisor");
    match club.respond(&prompt) {
        Ok(reply) => match crate::advisor::annotate(&crate::advisor::parse_verdict(&reply)) {
...
fn run_hop_advisor(
    ...
    let task = last_user_task(history);
    let summary = crate::advisor::bound_hop_summary(&last_hop_summary(history), 4000);
    let prompt = format!(
        "{}\n\n{}",
        crate::advisor::ADVISOR_HOP_SYS,
        crate::advisor::hop_review_prompt(&task, &summary)
    );
    registry.auxiliary.utility_entered("advisor");
    let reply = club.respond(&prompt).ok()?;
    crate::advisor::annotate_hop(&crate::advisor::parse_verdict(&reply))
}
...
/// trajectory row is finally *rewarded* on ([`crate::cut::TurnVerdicts`]). One
...
struct PostWriteObservers<'a> {
    ...
    verdicts: &'a mut crate::cut::TurnVerdicts,
    rollout_recorder: &'a mut RolloutRecorder,
    verification: &'a mut crate::cut::PostWriteVerification,
}
...
fn post_write_verdict(
    ...
    let targets = crate::cut::mutation_targets(&call.name, &call.args);
    if targets.is_empty() {
...
fn finish_post_write_verification(
    ...
    club: &dyn Club,
    hop: usize,
    verification: &mut crate::cut::PostWriteVerification,
    verdicts: &mut crate::cut::TurnVerdicts,
    ...
    verification
        ...
        .filter_map(|receipt| {
            ...
            crate::cut::record_final_verification(
                registry.current_workspace(),
                hop,
                &club.resolved_route_identity(),
                &receipt,
...
mod tests {
    ...
    fn revision_hasher_stays_idle_when_rendered_requirement_is_off() {
        ...
        let off =
            task_acceptance_snapshot(None, false, None, 0, 0, 0, None, "answer", None, || {
                off_hits += 1;
