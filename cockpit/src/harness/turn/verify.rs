...
pub(crate) fn is_verification_call(call: &ToolCall) -> bool {
    match call.name.as_str() {
        ...
        "shell" => {
            let command = crate::tools::shell::shell_command_arg(&call.args).unwrap_or("");
...
pub(crate) fn is_progress_verifier_call(name: &str, args: &Value) -> bool {
    ...
    let command = crate::tools::shell::shell_command_arg(args).unwrap_or("");
...
pub(crate) fn verification_identity(call: &ToolCall) -> Option<String> {
    ...
    match call.name.as_str() {
        ...
        "shell" => {
            let command = crate::tools::shell::shell_command_arg(&call.args)?;
...
pub(crate) fn with_turn_deadline_cancel<R>(
    ...
    crate::tools::http_transport::with_deadline(deadline, || {
        ...
        impl Drop for ProcessOwnerTransfer {
            fn drop(&mut self) {
                crate::tools::proc::transfer_owned(self.0, self.1);
            }
        }
...
pub(crate) fn verification_outcome(call: &ToolCall, result: &str) -> Option<VerificationOutcome> {
    ...
    let outcome = match call.name.as_str() {
        ...
        "cargo" => {
            ...
            match first {
                Some("test") => {
                    let parsed = crate::harness::parse_test_result(result);
...
pub(crate) fn turn_event_outcome(call: &ToolCall, result: &str, denied: bool) -> ToolOutcome {
        ...
        || (is_error_result(result) && starts(&format!("{} panicked:", call.name)))
    {
        ...
        && crate::sandbox::process_owner::KillReceipt::from_error(result)
...
pub(crate) fn verification_gate_prompts() -> bool {
    matches!(crate::yolo::profile(), crate::yolo::Profile::Guarded)
}
...
pub(super) fn operator_releases_unverified_completion(workspace: &Path) -> bool {
    ...
    let decision = crate::approval::ask(
        crate::approval::ApprovalScope::TurnGate("unverified completion".to_string()),
        &format!(
    ...
    matches!(decision, crate::approval::Decision::Approve)
}
