...
impl DelegateTool {
    ...
    fn run_from_with_cancel(
        ...
        if crate::tools::solo::solo_mode_active() && crate::club::is_sota_label(club.label()) {
            return Err(format!(
        ...
        let mut session = crate::session::Session::at(session_dir, run_id.clone());
        session.bind(&repo);
        let outcome = (|| {
            ...
            let binding = super::rollout::TaskRolloutBindingV1::new(
                ...
                crate::harness::run_identity::source_sha256().into(),
            );
...
pub(crate) fn delegate_session_dir() -> PathBuf {
    if cfg!(test) {
        ...
        crate::session::sessions_dir().join("delegates")
    }
}
...
impl Tool for DelegateTool {
    ...
    fn call_with_native_context(
        ...
        if let Some(target) = self.clubs.get(&club_key)
            && crate::tools::consult::is_optional_local_label(target.label())
            && !target.is_available()
        {
            let live: Vec<String> = self
                ...
                .values()
                .filter(|c| {
                    crate::tools::consult::is_optional_local_label(c.label()) && c.is_available()
                })
                .map(|c| c.label().to_string())
                .collect();
            return Ok(super::delegated_lineage::NativeToolResult::plain(
                crate::tools::consult::skip_down_local_message("delegate", target.label(), &live),
            ));
        }
...
fn delegate_diff_digest(
    ...
    Ok(crate::cut::sha256_hex(&captured.output.stdout))
}
...
fn delegate_token_delta(before: crate::club::TokenUsage, after: crate::club::TokenUsage) -> u64 {
    after
        .total_input
