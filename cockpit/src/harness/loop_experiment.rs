...
pub(crate) struct LoopExperimentResult {
    ...
    pub requested_route: crate::club::RouteIdentity,
    pub resolved_route: crate::club::RouteIdentity,
    ...
    #[serde(skip)]
    pub evidence: Option<crate::reinforce::EvaluatorEvidence>,
...
fn git_output_with_limit(
    ...
    let mut forwarding = if confine_forwarder {
        ...
        crate::sandbox::command("/bin/sh", std::iter::empty::<&str>(), &policy)
            .map_err(|e| e.to_string())?
    } else {
        Command::new("/bin/sh")
    };
    ...
    if let Some(policy) = forwarding_policy {
        crate::sandbox::set_helper_policy(&mut forwarding, &policy).map_err(|e| e.to_string())?;
    }
    let capture =
...
fn snapshot_live(
    ...
    for path in active_files(root, cancel)? {
        ...
        manifest.insert(
            path,
            SnapshotFile {
                sha256: crate::cut::sha256_hex(&bytes),
                executable,
            },
        );
    }
...
pub(crate) fn run_loop_experiment(
    ...
    let mut result = LoopExperimentResult {
        answer: String::new(),
        task_sha256: crate::cut::sha256_hex(request.task.as_bytes()),
    ...
    let execution = std::thread::scope(|scope| {
        ...
        let execution = (|| -> Result<(), String> {
            ...
            if !cancel.load(Ordering::Acquire) {
                ...
                result.patch_sha256 = Some(crate::cut::sha256_hex(&patch));
                    ...
                    .filter(|command| !command.trim().is_empty())
                {
                    ...
                    let execution = crate::reinforce::recovery_eval::run(
                        crate::reinforce::recovery_eval::RecoveryEvalRequest {
                            command,
                            workspace: &working,
                    ...
                    match execution {
                        Ok(execution) => {
                            ...
                            if capture_enabled {
                                let typed =
                                    crate::reinforce::recovery_training_supported(&evidence);
                                result.training_capture = Some(
                                        ...
                                        || result.error.is_some()
                                    {
                                        ...
                                        crate::experience::note_turn_workspace(&working);
                                        match crate::reinforce::finish_coding_eval(
                                            club.label(),
                                            &history,
                                            &request.task,
                                            &result.answer,
                                ...
                                if let Some(status) = result.training_capture.as_mut() {
                                    ...
                                    if !row_path.exists() {
                                        ...
                                        row["repo"] = crate::experience::repo_value_for(&working);
                                        if let Ok(path) = &evidence_path
                                            && let Ok(bytes) = std::fs::read(path)
                                        {
                                            row["evaluator_artifact_sha256"] =
                                                serde_json::json!(crate::cut::sha256_hex(&bytes));
                                        }
    ...
    result.result_sha256 = Some(crate::cut::sha256_hex(&result_bytes));
    Ok(result)
}
...
mod tests {
    ...
    fn git_capture_preserves_large_output_and_refuses_output_over_bound() {
        fixture(|_root, source| {
            ...
            assert!(
                git_output_with_limit(source, &["show", "HEAD:large.txt"], &cancel, 1024).is_err()
            );
        });
    }
