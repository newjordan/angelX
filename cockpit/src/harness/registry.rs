...
/// Operator-armed communication mode (`/retard`). Standing steer until explicitly
/// turned off. Literal operator contract for science / UX ablation — do not soften.
pub const RETARD_MODE_DIRECTIVE: &str = "[retard mode active] The user is literally retarded \
and needs extra help ensuring code is bug free and easy to use. Explain things in a way a \
retard can understand: short words, tiny steps, zero unexplained jargon, concrete examples, \
...
pub trait Tool: Send + Sync {
    ...
    fn call_with_cancel(
        ...
        crate::tools::http_transport::with_cancel(cancel, || self.call(args))
    }
...
pub struct ToolRegistry {
    ...
    pub(crate) mutation_targets: Arc<crate::tools::build::MutationTargets>,
    ...
    clerk: Arc<crate::atlas_clerk::AtlasClerkWorker>,
    ...
    backplane: Arc<crate::backplane::BackplaneRegistry>,
    /// Shared internal service behind the model-facing `swarm_compile` adapter.
    swarm_compiler: Option<Arc<crate::harness::swarm_compile::SwarmCompilerEngine>>,
    /// One controller shared by `/rl`, the stage, and loop-native tools.
    rl: Arc<std::sync::Mutex<crate::rl_ctl::RlState>>,
}
...
pub(crate) fn use_essential_schemas(bounded_task: bool, competition: bool) -> bool {
    if competition || crate::comp_mode::enabled() {
        return true;
    }
...
impl ToolRegistry {
    pub fn new() -> Self {
        ...
        let atlas = crate::atlas::AtlasService::open(&workspace);
        let clerk = crate::atlas_clerk::AtlasClerkWorker::shared(Arc::clone(&atlas));
        Self {
            ...
            clerk,
            workspace,
            mutation_targets: Arc::new(crate::tools::build::MutationTargets::default()),
            ...
            backplane: Arc::new(crate::backplane::BackplaneRegistry::default()),
            swarm_compiler: None,
            rl: Arc::new(std::sync::Mutex::new(crate::rl_ctl::RlState::default())),
        }
    }
    ...
    pub(crate) fn set_backplane(&mut self, backplane: crate::backplane::BackplaneRegistry) {
        self.backplane = Arc::new(backplane);
    }

    pub(crate) fn backplane(&self) -> Arc<crate::backplane::BackplaneRegistry> {
        Arc::clone(&self.backplane)
    }

    pub(crate) fn swarm_compiler(
        &self,
    ) -> Option<Arc<crate::harness::swarm_compile::SwarmCompilerEngine>> {
        self.swarm_compiler.as_ref().map(Arc::clone)
    }

    pub(crate) fn refresh_backplane(&self, bag: &crate::club::Bag) {
    ...
    pub(crate) fn atlas(&self) -> Arc<crate::atlas::AtlasService> {
        Arc::clone(&self.atlas)
    }

    pub(crate) fn clerk(&self) -> Arc<crate::atlas_clerk::AtlasClerkWorker> {
    ...
    pub(crate) fn set_workspace(&mut self, workspace: PathBuf) {
        ...
        self.atlas = crate::atlas::AtlasService::open(&workspace);
        self.clerk = crate::atlas_clerk::AtlasClerkWorker::shared(Arc::clone(&self.atlas));
    ...
    pub(crate) fn rl(&self) -> std::sync::MutexGuard<'_, crate::rl_ctl::RlState> {
        self.rl.lock().unwrap_or_else(|error| error.into_inner())
    }

    fn register_rl_campaign(&mut self) {
        self.register_deferred(Box::new(
            crate::tools::loop_research::LoopResearchTool::new(
                self.workspace.clone(),
                Arc::clone(&self.rl),
            ),
        ));
        self.register_deferred(Box::new(crate::tools::rl_campaign::RlCampaignTool::new(
            self.workspace.clone(),
            Arc::clone(&self.rl),
        )));
    }
    ...
    pub fn with_defaults() -> Self {
        ...
        let (cargo, native) = crate::tools::build::capture_verifier_runtimes(&workspace);
        let mut r = Self::new();
        ...
        r.register(Box::new(crate::tools::goal::GoalTool::new(&workspace)));
        r.register(Box::new(
            crate::continual_harness::ContinualHarnessTool::new(&workspace),
        ));
    ...
    pub fn with_team_self(
        ...
        let swarm_compiler = Arc::new(
            crate::harness::swarm_compile::SwarmCompilerEngine::new_with_cargo(
                workspace.clone(),
                roster.clone(),
                self_club.clone(),
                cargo.clone(),
        ...
        r.register(Box::new(crate::tools::consult::ConsultModelTool::new(
            roster.clone(),
            self_club.clone(),
        )));
        r.register(Box::new(crate::tools::consult::CodeReviewTool::new(
            roster.clone(),
            self_club.clone(),
        )));
        crate::tools::consult::maybe_register_leanstral(&mut r, &roster);
        r.register(Box::new(
    ...
    /// [`crate::harness::registration::ProviderScope`]. Retracting any one tool
...
/// capture ([`crate::cut::capture_writes`]): this is the single write path in
