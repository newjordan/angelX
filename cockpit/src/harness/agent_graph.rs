...
use super::*;
use crate::club::{OutputBudgetPolicy, OutputBudgetSource, RouteIdentity, RouteMetadata};
...
impl GraphTextArtifactV1 {
    fn capture(text: &str) -> Self {
        ...
        Self {
            ...
            sha256: crate::cut::sha256_hex(text.as_bytes()),
...
impl GraphTraceTerminationV1 {
    ...
    fn failure(kind: GraphTraceTerminationKind, reason: &str) -> Self {
        Self {
            ...
            detail_sha256: Some(crate::cut::sha256_hex(reason.as_bytes())),
        }
    }
    ...
    fn from_turn(
        ...
        Self {
            ...
            detail_sha256: detail.map(|value| crate::cut::sha256_hex(value.as_bytes())),
        }
    }
}
...
impl GraphRuntimeConfigV1 {
    ...
    fn new(
        ...
        let cockpit_source_sha256 = crate::harness::run_identity::source_sha256().to_string();
        ...
        .to_string();
        let harness_treatment_sha256 = crate::cut::sha256_hex(
        ...
        let mut config = Self {
            ...
            unrestricted: crate::yolo::enabled(),
...
fn graphs_bundled_dir() -> PathBuf {
    std::env::var_os("ANGEL_BUNDLED_GRAPHS_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("graphs"))
}
...
impl AgentGraphEngine {
    ...
    pub(crate) fn run(
        ...
        let _signal_guard = crate::sandbox::process_owner::defer_exit_for_graph();
        ...
        let graph_spec_sha256 = crate::cut::sha256_hex(
            &serde_json::to_vec(spec)
                .map_err(|error| format!("encode graph spec receipt: {error}"))?,
        );
        let task_sha256 = crate::cut::sha256_hex(task.as_bytes());
        let workspace_key_sha256 = crate::cut::sha256_hex(
            crate::workspace_store::workspace_key(&self.workspace).as_bytes(),
        );
        ...
        for node in &spec.nodes {
            ...
            if let Some(effort) = effort.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
                ...
                let (scoped, applied) = crate::club::scoped_reasoning_effort(club, effort);
                club = scoped;
                match applied {
        ...
        let episode_id = crate::cut::sha256_hex(episode_nonce.as_bytes());
        ...
        loop {
            if winding.is_none()
                && (cancel.load(Ordering::Relaxed)
                    || crate::sandbox::process_owner::exit_requested())
            {
                    ...
                    match reserve_spawn_seats(1, inflight_limit) {
                        Ok(mut permits) => {
                            ...
                            let feedback_sha256 = feedback
                                .get(&node.id)
                                .map(|body| crate::cut::sha256_hex(body.as_bytes()));
                            ...
                            let spawn_result = std::thread::Builder::new()
                                ...
                                .spawn(move || {
                                    ...
                                    *thread_timing.lock().unwrap_or_else(|e| e.into_inner()) =
                                    ...
                                    let stall_window = crate::club::identity_stream_stall_secs();
                                        ...
                                        if grant == Grant::None {
                                            ...
                                            loop {
                                                match club.chat_streaming(
                                                    ...
                                                    Ok(crate::club::ClubReply::Text(text))
                                                    ...
                                                    Ok(crate::club::ClubReply::Text(_)) => {
                                                    ...
                                                    Ok(crate::club::ClubReply::Calls(_)) => {
        ...
        let episode = GraphEpisodeV1 {
            ...
            termination: GraphEpisodeTerminationV1 {
                ...
                detail_sha256: winding
                    .as_deref()
                    .map(|reason| crate::cut::sha256_hex(reason.as_bytes())),
                final_output_sha256: (!answer.is_empty())
                    .then(|| crate::cut::sha256_hex(answer.as_bytes())),
            },
...
fn graph_result_envelope(source: &str, output: Option<&str>, body_cap: usize) -> String {
    ...
    let mut receipt = format!(
        ...
        crate::cut::sha256_hex(body.as_bytes())
    );
    if excerpt_chars > 0 {
        receipt.push('\n');
...
fn graph_trace_id(episode_id: &str, role: &str, attempt_index: u32) -> String {
    crate::cut::sha256_hex(format!("{episode_id}:{role}:{attempt_index}").as_bytes())
}
...
fn episode_store_dirs(workspace: &Path) -> Result<(PathBuf, PathBuf), String> {
    let dir = std::env::var_os("ANGEL_AGENT_GRAPH_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| crate::workspace_store::angel_subdir("agent-graphs"))
...
fn persist_episode_receipt(workspace: &Path, episode: &GraphEpisodeV1) -> Result<(), String> {
    ...
    let body = crate::secrets::to_redacted_vec_pretty(episode)
...
pub(crate) struct AgentGraphTool {
    ...
    catalog_line: String,
    cargo: PinnedCargo,
    mutation_targets: Arc<crate::tools::build::MutationTargets>,
}

impl AgentGraphTool {
    pub(crate) fn with_mutation_targets(
        mut self,
        targets: Arc<crate::tools::build::MutationTargets>,
    ) -> Self {
...
mod tests {
    ...
    fn checked_in_graph_persona_resolves_and_is_injected_into_real_node_prompt() {
        ...
        let _user =
            crate::tests::TestEnvGuard::set("ANGEL_PERSONAS_DIR", empty_user.to_str().unwrap());
    ...
    fn validation_allows_transitively_ordered_code_and_parallel_read_only_nodes() {
        ...
        validate_graph_spec(&graph).expect(
            "transitive depends_on orders code nodes while read-only branches remain parallel",
        );
    }
...
"#;
    ...
    fn completed_run_persists_auditable_episode_sidecar() {
        ...
        let cancel = AtomicBool::new(false);
        let outcome =
            AgentGraphEngine::new(workspace.clone(), Some(club as Arc<dyn Club>), Vec::new())
                .run(&graph, "task", &cancel)
    ...
    fn fan_in_joins_parallel_branches() {
        ...
        for trace in &outcome.episode.traces {
            assert!(
                trace.finished_at.unwrap().monotonic_ns >= trace.started_at.unwrap().monotonic_ns
            );
...
"#,
        ...
        let persisted: GraphEpisodeV1 = serde_json::from_str(
            &std::fs::read_to_string(&path).expect("failed episode persisted"),
        )
    ...
    fn operator_cancel_is_attributed_in_the_sealed_node_receipt() {
        ...
        assert!(
            trace.cancel_requested_at.unwrap().monotonic_ns
                >= trace.started_at.unwrap().monotonic_ns
        );
        assert!(
            trace.stopped_at.unwrap().monotonic_ns
                >= trace.cancel_requested_at.unwrap().monotonic_ns
        );
