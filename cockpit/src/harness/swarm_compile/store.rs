...
impl RunStore {
    pub(super) fn for_workspace(workspace: &Path) -> Self {
        let root = std::env::var_os("ANGEL_SWARM_RUNS_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
                crate::workspace_store::angel_subdir("swarm-runs")
    ...
    pub(super) fn save(&self, run: &SwarmRun) -> Result<PathBuf, String> {
        ...
        let body = crate::secrets::to_redacted_vec_pretty(run)
    ...
    pub(super) fn event(
        ...
        let mut line =
            crate::secrets::to_redacted_vec(&record).map_err(|e| format!("encode event: {e}"))?;
        line.push(b'\n');
