...
fn auto_compact_chunk_threshold(budget: usize) -> usize {
    crate::compaction::configured_compact_chunk_tokens().min((budget / 2).max(1_024))
}
...
///
/// Routing goes through the [`crate::librarian::Librarian`] — the gate that will
...
pub(crate) fn file_report(
    ...
    let report = crate::librarian::Report {
        wing: crate::compaction::project_wing_for(workspace),
        topic: report_topic(history),
        body: body.to_string(),
        source: crate::compaction::provenance(session_id, "swarm"),
    };
...
impl MemoryWriteJob {
    fn run(self) {
        match self {
            Self::Report { store, report } => {
                let _ = crate::librarian::Librarian::new(store).file(&report);
            }
            Self::Drawers {
                store,
                drawers,
                ...
                for drawer in &drawers {
                    match crate::term::catch_background_unwind(|| store.deposit(drawer)) {
    ...
    fn reject(self, error: &'static str) {
        ...
        {
            let _ = crate::term::catch_background_unwind(|| {
                completion(MemoryWriteSummary {
    ...
    let _ = std::thread::Builder::new()
        ...
        .spawn(move || {
            while let Ok(job) = rx.recv() {
                ...
                let _ = crate::term::catch_background_unwind(|| job.run());
            }
        });
    tx
}
...
pub(crate) fn compaction_budget(club: &dyn Club, env_budget: usize) -> usize {
    ...
    let is_sota = crate::club::is_sota_label(club.label());
...
pub(crate) fn wants_cheap_summarizer(club: &dyn Club) -> bool {
    env_flag("ANGEL_COMPACT_LOCAL", true)
        && (crate::club::is_sota_label(club.label()) || club.reports_to_palace())
}
...
pub(crate) fn compaction_summarizer() -> Option<crate::club::HttpClub> {
    ...
    let key = std::env::var("ANGEL_COMPACT_KEY")
        .ok()
        .filter(|s| !s.trim().is_empty());
    Some(crate::club::HttpClub::new("compact", url, model, key))
}
...
pub(crate) fn refresh_knowledge_broker_with_prefix(
    ...
    if mode != crate::backplane::BackplaneMode::Active {
        ...
        crate::atlas::replace_lens_message(history, lens);
        if mode == crate::backplane::BackplaneMode::Legacy {
            return;
        }
    }
    let project_key = registry.atlas().project_key().to_string();
    let mut candidates =
        crate::backplane::KnowledgeBroker::existing_candidates(history, &project_key);
    ...
    let previous_atlas = history
        ...
        .flat_map(|message| {
            if crate::atlas::is_lens_message(&message.content) {
                crate::backplane::atlas_lens_candidates(&message.content, &project_key)
            } else {
                crate::backplane::KnowledgeBroker::existing_candidates(
    ...
    candidates.extend(
        crate::memory::load_for(registry.current_workspace())
            .into_iter()
            .enumerate()
            .map(|(index, memory)| {
                crate::backplane::KnowledgeCandidate::new(
                    format!("memory:{index}"),
                    &project_key,
                    "operator-memory",
    ...
    let dossier = crate::dossier::broker_context_block(registry.current_workspace());
    if !dossier.trim().is_empty() {
        candidates.push(crate::backplane::KnowledgeCandidate::new(
            format!(
                "dossier:{}",
            ...
            &project_key,
            "verified-dossier",
            crate::backplane::KnowledgeAuthority::VerifiedDossier,
            dossier,
        ));
    }
    let caddy =
        crate::caddy::render_card_for_task(registry.current_workspace(), crate::caddy::card_cap());
    if !caddy.trim().is_empty() {
        candidates.push(crate::backplane::KnowledgeCandidate::new(
            format!("caddy:{}", &crate::cut::sha256_hex(caddy.as_bytes())[..16]),
            &project_key,
    ...
    let selection =
        crate::backplane::KnowledgeBroker::select(&project_key, &query, candidates, remaining);
    if mode == crate::backplane::BackplaneMode::Shadow {
        eprintln!(
...
fn apply_broker_selection(
    history: &mut Vec<ChatMsg>,
    selection: &crate::backplane::BrokerSelection,
    preserve_prefix: bool,
) {
    if preserve_prefix {
        crate::backplane::KnowledgeBroker::append(history, selection);
    } else {
        crate::backplane::KnowledgeBroker::replace(history, selection);
    }
}
...
pub(crate) fn maybe_auto_recall(
    ...
    let wing = crate::compaction::project_wing_for(registry.current_workspace());
    ...
    if crate::backplane::active() {
        ...
        candidates.extend(blocks.iter().enumerate().map(|(index, block)| {
            crate::backplane::KnowledgeCandidate::new(
                format!(
                    "palace:{}:{index}",
                    &crate::cut::sha256_hex(block.as_bytes())[..12]
                ),
                &project_key,
                "episodic-recall",
                crate::backplane::KnowledgeAuthority::Episodic,
...
fn knowledge_broker_route_budget(history: &[ChatMsg], budget: usize, tools: &[ToolDef]) -> usize {
    if budget == 0 {
        return crate::backplane::RECALL_TOKEN_CEILING + AUTO_RECALL_SAFETY_TOKENS;
    }
...
pub(crate) fn compaction_turn_context_anchor(
    ...
    if history[window_end..]
        .iter()
        .any(crate::app_control::is_turn_context_message)
    {
        return None;
    }
    history[window_start..window_end]
        .iter()
        .rev()
        .find(|message| crate::app_control::is_turn_context_message(message))
        .cloned()
}
...
pub(crate) fn bound_task_anchor(task: &str, max_bytes: usize) -> String {
    ...
    let omission = format!(
        ...
        crate::cut::sha256_hex(task.as_bytes())
    );
        ...
        .map(|message| {
            if message.role == ChatRole::User {
                ChatMsg::harness(format!(
                    ...
                    crate::cut::sha256_hex(message.content.as_bytes())
                ))
            } else {
                message.clone()
            }
...
pub(crate) fn constraint_retention_note(
    ...
    let boundary = window
        .iter()
        .filter(|m| crate::compaction::is_compaction_note(m))
        ...
        .filter(|m| m.role == ChatRole::User)
    {
        ...
        let digest = crate::cut::sha256_hex(message.content.as_bytes());
...
fn compaction_replacement(
    ...
    recovery_context: Vec<crate::club::RecoveryContextRef>,
) -> Vec<ChatMsg> {
...
pub(crate) fn maybe_compact(
    ...
    let Some((sys_end, window_end)) =
        crate::compaction::select_window_with_token_tail(history, keep_recent, keep_recent_tokens)
    else {
    ...
    let wing = crate::compaction::project_wing_for(registry.current_workspace());
    let source = crate::compaction::provenance(session_id, "auto-compact");
    ...
    let Some(result) = crate::compaction::compact_window_with_state_budget(
        summarizer,
    ...
    let recovery_context = crate::club::recovery_context_refs(&history[sys_end..window_end]);
...
fn maybe_compact_for_turn_measured(
    ...
    if crate::compaction::sync_compaction_uses_model() {
    ...
    let turn_context_anchor = compaction_turn_context_anchor(history, sys_end, window_end);
    let wing = crate::compaction::project_wing_for(registry.current_workspace());
    let source = crate::compaction::provenance(&registry.session_id, "auto-compact");
    let live = registry.store.is_live();
    let Some(result) = crate::compaction::compact_window_fast(
        &compaction_summary_window(&history[sys_end..window_end]),
        &wing,
        &source,
        budget,
...
pub(crate) struct BgCompact {
    ...
    result: Arc<std::sync::Mutex<Option<Option<crate::compaction::CompactionResult>>>>,
}
...
pub(crate) fn maybe_start_bg_compact(
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let out = crate::term::catch_background_unwind(|| {
...
fn try_splice_bg_compact_measured(
    ...
    registry.auxiliary.utility_entered("background_compaction");
    let recovery_context = crate::club::recovery_context_refs(&bg.window);
