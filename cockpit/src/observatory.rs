...
fn default_manifest_path() -> PathBuf {
    ...
    // profile. On a host where the binary was built, `CARGO_MANIFEST_DIR`
    // resolves to this source tree, so an unconditional fallback would silently
    // present the bundled test fixture as the operator's real catalog. Absence
    ...
    if std::env::var_os("ANGEL_OBSERVATORY_DEV_FIXTURE").is_some() {
        let repository_fixture =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/observatory/manifest-v2.json");
...
mod tests {
    ...
    const VERSION_TWO: &str = include_str!("../fixtures/observatory/manifest-v2.json");
    ...
    fn unsupported_versions_and_duplicate_ids_are_rejected() {
        let error = Catalog::from_str(Path::new("manifest.json"), r#"{"version":3,"reports":[]}"#)
            .unwrap_err();
