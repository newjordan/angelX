...
use crate::tools::build::{parse_direct_argv, path_within};
...
pub(super) fn select(kind: Kind, args: &Value, root: &Path) -> Result<Selection, String> {
    ...
    if requested == "go" {
        ...
        let extra = crate::tools::build::parse_direct_argv(args["args"].as_str().unwrap_or(""))?;
    ...
    if argv_bytes > crate::tools::build::MAX_DIRECT_ARG_BYTES {
        return Err(
...
fn python_pytest(root: &Path, extra: Vec<String>) -> Result<Vec<String>, String> {
    ...
    let installed = candidates.into_iter().find(|path| {
        ...
        path.join("pytest/__init__.py").is_file()
            && !crate::tools::build::task_writable_roots(root).iter().any(|r| path_within(&path, r))
