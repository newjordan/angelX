...
use crate::tools::build::{CapturedCommand, TestOutcome, tail, timeout_detail};
use std::path::Path;
use std::sync::Arc;
...
pub(super) fn run(
    ...
    let deadline = crate::harness::tool_timeout().map(|duration| started + duration);
    ...
    for argv in &actual_argvs {
        ...
        let mut command = crate::sandbox::command(&executable.path, argv, policy)?;
        crate::tools::build::apply_verifier_env(&mut command, args)?;
        ...
        let captured = crate::harness::output_timed_extensible_cancellable_with_progress(
            command,
            remaining,
            cancel,
            progress.clone(),
