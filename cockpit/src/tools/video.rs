...
use crate::club::{ChatMsg, Club, ClubReply, Media, ToolDef};
use crate::harness::{Tool, run_sandboxed};
...
impl VideoBeatsTool {
    ...
    fn worker_path() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/tools/video_beats_worker.py")
    }
}
...
pub(crate) fn maybe_register_video_tools(r: &mut crate::harness::ToolRegistry, workspace: PathBuf) {
    if crate::harness::env_flag("ANGEL_VIDEO_TOOLS", true) {
...
impl VideoLookTool {
    ...
    fn vision_club() -> Result<std::sync::Arc<dyn Club>, String> {
        crate::tools::vision::resolve_vision_club().map_err(|e| format!("video_look: {e}"))
    }
}
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        if is_image {
            ...
            let times: Vec<f64> = if let Some(arr) = args["timestamps"].as_array() {
                ...
                let dur = crate::tools::vision::probe_duration(&path).unwrap_or(0.0);
