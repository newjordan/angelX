...
//!
//! Policy: mid-turn outsourcing to paid SOTA (openai/codex/…) is **opt-in**
//! (`ANGEL_ALLOW_SOTA_CONSULT=1`) and **forbidden in solo mode**. Unknown club
//! names fail closed — never fall through to "first roster entry" (that silently
//! landed DeepSeek on Codex during competition runs).

use crate::club::{ChatMsg, Club, ClubReply, ToolDef};
use crate::harness::{HandleKind, Tool, maybe_offload_root_body};
use crate::tools::solo::{deny_sota_if_blocked_except, solo_mode_active};
use serde_json::Value;
use std::sync::Arc;

#[cfg(test)]
#[path = "consult/deli_tests.rs"]
mod deli_tests;
    ...
    fn resolve_club(&self, name: &str) -> Result<(Arc<dyn Club>, Option<String>), String> {
        ...
        if trimmed.eq_ignore_ascii_case("smart") || trimmed.eq_ignore_ascii_case("sota") {
            ...
            let (seat, club) = match self.find_in_roster(&seat.club) {
                Some(club) => (seat, club),
                None => {
                    let alt = crate::club::smart_seat_alt();
...
impl Tool for ConsultModelTool {
    ...
    fn def(&self) -> ToolDef {
        ...
        let policy = if solo_mode_active() {
            ...
            "Paid SOTA/codex consults require ANGEL_ALLOW_SOTA_CONSULT=1. Prefer self + local tools."
        };
        ToolDef {
    ...
    fn call_with_cancel(
        ...
        let club: Arc<dyn Club> = if method == "deli" {
            Arc::new(crate::deli::DeliClub::for_consult(
                club,
                rounds,
                pinned_effort.clone(),
            ))
...
impl Tool for CodeReviewTool {
    ...
    fn def(&self) -> ToolDef {
        let policy = if solo_mode_active() {
            ...
            "Default self (in-hand). Paid SOTA requires ANGEL_ALLOW_SOTA_CONSULT=1."
        };
        ToolDef {
...
mod tests {
    ...
    fn consult_model_calls_resolved_club() {
        let mock_luna: Arc<dyn Club> =
            Arc::new(MockClub::new("luna", "Luna response for code help"));
    ...
    #[test]
    fn consult_openai_blocked_without_allow() {
