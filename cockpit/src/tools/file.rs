...
mod replace_tests {
    ...
    fn edit_recovery_preserves_ambiguity_and_stale_guards() {
        ...
        for original in [
            ...
            let error = tool.call(&serde_json::json!({"path":"source.rs","old":"        before();","new":"after();"})).unwrap_err();
...
mod interruption_tests {
    ...
    fn patch_exit_drain_sigint_between_real_patch_writes() {
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "tools::file::interruption_tests::patch_exit_drain_sigint_between_real_patch_writes", "--nocapture"])
            .env(CHILD, root.path())
            .spawn().unwrap();
