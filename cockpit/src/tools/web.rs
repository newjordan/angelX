    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        let resp = match req.call() {
            ...
            Err(http_transport::Error::Status(code, resp)) => {
                let snippet = crate::science::sanitize_metadata(
...
pub(crate) struct GrokResearchTool {
    club: std::sync::Arc<crate::club::GrokResearchClub>,
}

impl GrokResearchTool {
    pub(crate) fn from_env() -> Option<Self> {
        crate::club::GrokResearchClub::from_env().map(|club| Self {
...
impl Tool for GrokResearchTool {
    ...
    fn call_with_cancel(
        ...
        crate::club::Club::respond_cancellable(
            &*self.club,
...
impl Tool for WebFetchTool {
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        if out.len() > max {
            crate::harness::truncate_to_char_boundary(&mut out, max);
            if max >= 200_000 {
                out.push_str(
...
impl Tool for HttpRequestTool {
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        if !http_method_allowed(
            &method,
            crate::yolo::enabled() || env_flag(HTTP_MUTATIONS_ENV, false),
        ) {
            return Err(format!(
...
mod decode_tests {
    ...
    fn http_error_bodies_yield_to_deadline_for_all_web_tools() {
        ...
        for tool in tools {
            ...
            let error =
                http_transport::with_deadline(Some(start + Duration::from_millis(400)), || {
