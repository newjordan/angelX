...
fn revalidate_executable(executable: &PinnedExecutable, label: &str) -> Result<(), String> {
    ...
    let observed = crate::harness::sha256_reader_hex(&mut file).map_err(|error| {
        format!(
...
pub(crate) fn bounded_rustup_cargo_path(command: Command, timeout: Duration) -> Option<PathBuf> {
    let out = crate::workspace_store::capture_bounded_command(command, timeout)?;
...
fn capture_pinned_cargo(workspace: &Path) -> Result<PinnedCargoExecutable, String> {
    ...
    if let Some(raw) = std::env::var_os("ANGEL_CARGO_BIN") {
        ...
        if !path.is_file() {
            return Err(crate::tools::runtime_missing::RuntimeMissing::new(
                "cargo",
    ...
    if controls.has_toolchain() {
        return Err(format!(
            ...
            {
                crate::tools::runtime_missing::RuntimeMissing::new(
...
fn initial_executable_digest(path: &Path, label: &str) -> Result<OnceLock<String>, String> {
    ...
    let hash = crate::harness::sha256_reader_hex(&mut file)
...
struct CapturedCommand {
    ...
    timeout_diag: Option<crate::harness::TimeoutDiagnostics>,
}
...
pub(crate) fn nested_crate_dirs(workspace: &Path) -> Vec<PathBuf> {
    let mut dirs: Vec<PathBuf> = crate::workspace_lang::detect(workspace)
        .into_iter()
        .filter(|h| {
            h.lang == crate::workspace_lang::Lang::Rust
                && h.manifest
                && h.dir != Path::new(".")
...
fn sandboxed_cargo_output_with_env(
    ...
    let mut trusted_args = trusted_verifier
        ...
        .transpose()?;
    if trusted_verifier && crate::yolo::enabled() {
    ...
    if policy.mandatory || crate::sandbox::sealed::identity().is_some() {
    ...
    let progress = if collect_sources {
        progress.map(|sink| {
            Arc::new(move |stream, bytes: &[u8]| {
                if stream == crate::harness::ProcessStream::Stderr {
                    sink(stream, bytes);
                }
...
impl Tool for CargoTool {
    ...
    fn call_with_cancel_and_progress(
        ...
        let output = crate::harness::cap_text_owned(combined, 4000, 0)
            .trim()
            .to_string();
        ...
        let experience = crate::experience::CmdExperience {
            ...
            shell: crate::experience::CmdShell::Direct,
        };
        crate::experience::record_cmd_event(&experience, &self.workspace);
        if out.cancelled {
...
impl Tool for RunTestsTool {
    ...
    fn call_with_cancel_and_progress(
        ...
        use crate::workspace_lang::{self, Lang};
...
impl RunTestsTool {
    ...
    fn run_cargo(
        ...
        let yolo_pin = crate::yolo::enabled()
            .then(|| self.cargo.pin_note())
            .flatten();
        let yolo = yolo_pin.is_none() && crate::yolo::enabled();
        ...
        crate::experience::record_cmd_event(
            &crate::experience::CmdExperience {
                tool: "run_tests",
    ...
    fn run_plan(
        &self,
        scan_root: &Path,
        plan: crate::workspace_lang::TestPlan,
        args: &Value,
        ...
        let counts =
            crate::workspace_lang::parse_runner_output(plan.lang, &out.stdout, &out.stderr);
        ...
        let reward = if crate::yolo::enabled() {
...
fn sandboxed_program_output(
    ...
    let node = (program == "npm")
        .then(|| crate::tools::runtime_missing::resolve("node"))
        .transpose()?;
    let resolved = crate::tools::runtime_missing::resolve(program)?;
        ...
        output_timed_extensible_cancellable_with_progress(cmd, tool_timeout(), cancel, progress)
            .map_err(|error| {
                if error.contains("No such file or directory")
                    && dir.is_dir()
                    && crate::tools::runtime_missing::pin(program).is_some()
                {
                    crate::tools::runtime_missing::RuntimeMissing::new(
                        program,
    ...
    if out.status.code() == Some(127)
        && let Some(error) = crate::tools::runtime_missing::from_command_not_found(
...
#[cfg(test)]
use crate::workspace_lang::resolve_on_path;
...
fn apply_verifier_env(command: &mut std::process::Command, args: &Value) -> Result<(), String> {
    ...
    for (key, value) in env {
        if !crate::harness::shell_verifier::valid_env_key(key)
            || crate::harness::shell_verifier::protected_env_key(key)
        {
