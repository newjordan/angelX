...
use super::*;
use crate::harness::{LoopExperimentRequest, LoopExperimentResult};
...
fn scope_root(
    ...
    runs_root(workspace)
        .join("learning")
        .join(crate::cut::sha256_hex(scope.to_string().as_bytes()))
}
...
fn run(
    ...
    let source_sha = crate::harness::freeze_active_source(workspace, &source, cancel)?;
