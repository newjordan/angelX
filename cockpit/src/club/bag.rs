...
impl Bag {
    ...
    pub fn standard() -> Self {
        ...
        let openai_oauth_available = crate::openai_codex::ChatGptAuth::load().is_some()
            && crate::openai_codex::CodexClub::default_model().is_some();
        ...
        if let (Some(auth), Some(_model)) = (
            crate::openai_codex::ChatGptAuth::load(),
            crate::openai_codex::CodexClub::default_model(),
        ) {
            let shared = crate::openai_codex::CodexClub::shared_state(auth);
            let available = Arc::new(AtomicBool::new(true));
            let selection = crate::openai_codex::CodexClub::resolve_selection();
            let model = selection.model.clone();
            let configured_effort = Some(selection.effort.clone());
            // Drop private-test Codex Spark surfaces so ←/→ cannot select them.
            let mut catalog: Vec<_> = crate::openai_codex::CodexClub::model_catalog()
                .into_iter()
                .filter(|c| !crate::openai_codex::is_private_test_codex_model(&c.slug))
                .collect();
            if !catalog.iter().any(|candidate| candidate.slug == model) {
                catalog.insert(
                    ...
                    crate::openai_codex::CodexModelInfo {
                        ...
                        default_reasoning_level: configured_effort
                            .clone()
                            .unwrap_or_else(|| crate::openai_codex::OPENAI_LUNA_EFFORT.to_string()),
                        supported_reasoning_levels: Vec::new(),
                        visibility: String::new(),
                        priority: 0,
                        ..crate::openai_codex::CodexModelInfo::default()
                    },
                );
            }
            ...
            for candidate in catalog {
                if crate::openai_codex::is_private_test_codex_model(&candidate.slug)
        ...
        for (ai, si, label, inner) in &swarm_slots {
            ...
            agents[*ai].slots[*si].club = Arc::new(crate::swarm::SwarmClub::from_env_with(
                label,
                Arc::clone(inner),
                resolve,
            ));
    ...
    pub(crate) fn moa_model_choices(&self) -> Vec<crate::formations::MoaModelChoice> {
        self.agents
            .iter()
            .enumerate()
            .filter(|(_, agent)| {
                !crate::club::is_logical_wrapper_label(&agent.name) && agent.name != "practice"
            })
            .flat_map(|(agent_index, agent)| {
                agent
                    ...
                    .map(move |(slot_index, slot)| {
                        ...
                        crate::formations::MoaModelChoice {
                            route: crate::formations::MoaModelRef {
                                agent_index,
                                slot_index,
                                agent: agent.name.clone(),
                                driver,
                                route_id: crate::backplane::RouteId::chat(
                                    &agent.name,
    ...
    pub(crate) fn atlas_teacher_route(&self) -> Option<crate::atlas_clerk::ClerkRoute> {
        let mut best: Option<(u32, crate::atlas_clerk::ClerkRoute)> = None;
        for agent in &self.agents {
            ...
            for slot in &agent.slots {
                ...
                let parameters = crate::backplane::parameter_billions(&model).unwrap_or(0);
                if parameters < 30 {
                    continue;
                }
                let candidate = crate::atlas_clerk::ClerkRoute {
                    route_id: crate::backplane::RouteId::chat(
                        &agent.name,
                        &route.driver,
                        route.reasoning_effort.as_deref(),
                    ),
                    model_revision: crate::backplane::ModelRevision::chat(&model),
    ...
    pub(crate) fn activate_sota_moa_with_roster(
        ...
        let slots = crate::formations::formation(roster.formation()).slots();
        ...
        for (formation_slot, selected) in slots.iter().zip(roster.assignments()) {
            ...
            let live_route = crate::backplane::RouteId::chat(
                &agent.name,
                &slot.club.route_identity().driver,
                slot.club.reasoning_effort().as_deref(),
            );
            let live_revision = crate::backplane::ModelRevision::chat(&live);
        ...
        let moa: Arc<dyn Club> = Arc::new(
            crate::swarm::SwarmClub::from_sota_env_roles(
            ...
            .with_role_effort_overrides(crate::swarm::SeatEfforts {
                propose: roster
    ...
    pub fn tabs(&self) -> std::sync::Arc<Vec<ClubTab>> {
        ...
        let tabs: Vec<ClubTab> = self
            ...
            .filter(|(i, a)| {
                a.name != "practice"
                    && (*i == self.in_hand || !crate::club::is_mathgod_label(&a.name))
            })
            .filter_map(|(i, a)| {
    ...
    pub fn cycle(&mut self) {
        ...
        for step in 1..=n {
            ...
            if self.agents[idx].name != "practice"
                && !crate::club::is_mathgod_label(&self.agents[idx].name)
    ...
    pub(crate) fn route_choices(&self) -> std::sync::Arc<Vec<RouteChoice>> {
            ...
            && *generation == self.generation
        {
            ...
            let mut idx = 0;
            for agent in &self.agents {
                if agent.name == "practice" || crate::club::is_mathgod_label(&agent.name) {
                    continue;
                }
                for slot in &agent.slots {
                    if slot.label == "sota-moa" || crate::club::is_mathgod_label(&slot.label) {
                        continue;
                    }
