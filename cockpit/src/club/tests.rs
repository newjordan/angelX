...
#[test]
fn saved_api_credentials_do_not_enable_clubs_or_failover_routes() {
    let _guard = env_lock();
    let _disabled = ScopedEnv::unset("ANGEL_API_CLUBS");
    ...
    for alias in [
        ...
        assert!(build(alias).is_none(), "saved key must not enable {alias}");
    }
    let _enabled = ScopedEnv::set("ANGEL_API_CLUBS", " glm , KIMI ");
    assert!(build("glm").is_some());
    assert!(build("glm-5.3-flash").is_some());
    assert!(build("kimi").is_some());
    assert!(build("meta").is_none());
    assert!(build("openrouter").is_none());
    let _wildcard = ScopedEnv::set("ANGEL_API_CLUBS", "*");
    assert!(build("meta").is_none());
    assert!(build("glm").is_none());
}
...
fn gpu_comp_coordinator_delegates_questions_with_the_mission_contract() {
    ...
    assert!(text.contains("Turbo x12"), "{text}");
    assert!(text.contains("Leanstral"), "{text}");
    assert!(text.contains("DICE"), "{text}");
    assert!(
        text.contains("Do not repeat setup instructions"),
        "coordinator prompt should prevent the static-card loop: {text}"
    );
    assert!(
        text.contains("one Turbo coordinator conversation")
            && text.contains("runner artifact exists"),
        "the logical club must not impersonate a completed 12-lane dispatch: {text}"
    );
}
...
#[test]
fn atlas_model_serving_is_opt_in_policy() {
    let _guard = env_lock();
    // TODO: Audit that the environment access only happens in single-threaded code.
    unsafe { std::env::remove_var("ANGEL_ATLAS_MODEL_SERVING") };
    assert!(model_serving_target_allowed("turbo", "100.64.0.4"));
    assert!(!model_serving_target_allowed("atlas", "100.64.0.2"));
    assert!(!model_serving_target_allowed("atlas-1", "100.64.0.2"));
    assert!(model_serving_target_allowed("custom", "100.64.0.2"));

    // TODO: Audit that the environment access only happens in single-threaded code.
    unsafe { std::env::set_var("ANGEL_ATLAS_MODEL_SERVING", "1") };
    assert!(model_serving_target_allowed("atlas-1", "100.64.0.2"));
    // TODO: Audit that the environment access only happens in single-threaded code.
    unsafe { std::env::remove_var("ANGEL_ATLAS_MODEL_SERVING") };
}
