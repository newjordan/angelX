...
//! CLI-only toy. **Never** uses `XAI_API_KEY` / paid API-key env vars.
//!
...
//! every scout call. The ACP child also scrubs every paid API-key env var.
//!
...
fn build_grok_acp_command(key: &GrokAcpKey) -> Command {
    ...
    scrub_grok_paid_api_env(&mut cmd);
...
fn scrub_grok_paid_api_env(cmd: &mut Command) {
    for key in [
        "XAI_API_KEY",
        "GROK_API_KEY",
        "ANGEL_GROK_KEY",
...
/// Build every selectable first-class Grok OAuth HTTP model when `auth.json`
/// is present. The configured model stays under the stable `grok` alias; Grok
/// 4.6 and every other supported model in the local CLI catalog get concrete
/// sibling rows. All rows share one refreshable OAuth authority and never read
/// `XAI_API_KEY` / paid API-key env vars.
///
...
pub(crate) fn grok_oauth_http_clubs() -> Vec<(String, Arc<dyn Club>, Arc<AtomicBool>)> {
    let Some(auth) = GrokOauthAuth::load() else {
        return Vec::new();
    };
    // Must be able to produce a token (or refresh) before we advertise the seat.
    ...
    for candidate in std::iter::once(GROK_DEFAULT_MODEL.to_string()).chain(grok_cli_model_ids()) {
        if GROK_SOTA_MODEL_OPTIONS
            .iter()
    ...
    models
        ...
        .map(|(index, model)| {
            ...
            let club: Arc<dyn Club> = Arc::new(
                crate::club::HttpClub::new(label, GROK_DEFAULT_API_BASE, model, None)
...
mod tests {
    ...
    #[test]
    fn grok_api_key_mode_is_disabled_even_when_forced() {
        let _lock = env_lock();
    ...
    #[test]
    fn grok_api_key_env_does_not_configure_without_oauth() {
        let _lock = env_lock();
        let _file = EnvGuard::set("ANGEL_GROK_OAUTH_FILE", "/definitely/missing/auth.json");
        let _key = EnvGuard::set("ANGEL_GROK_KEY", "api-token");
        let _api = EnvGuard::unset("ANGEL_GROK_USE_API");
    ...
    fn grok_harness_surface_denies_nested_agent_tools() {
        ...
        assert!(
            args.windows(2).any(|w| {
                w[0] == "--system-prompt-override" && w[1].contains("pure completion")
            }),
    ...
    fn grok_startup_file_read_rejects_fifo_without_waiting_for_writer() {
        let _lock = env_lock();
        let directory =
            std::env::temp_dir().join(format!("grok-startup-fifo-{}", std::process::id()));
    ...
    fn grok_oauth_http_catalog_lists_46_and_live_supported_models() {
        let _lock = env_lock();
        let dir =
            std::env::temp_dir().join(format!("angel0-grok-oauth-club-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let auth_path = dir.join("auth.json");
        ...
        let links = grok_oauth_http_clubs();
        let routes = links
            .iter()
        ...
        let current = grok_oauth_http_clubs();
    ...
    fn fake_grok_acp_fixture(tag: &str) -> (PathBuf, PathBuf, PathBuf) {
        ...
        let dir =
            std::env::temp_dir().join(format!("angel0-grok-acp-{}-{tag}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let auth = dir.join("auth.json");
...
"#,
    ...
    fn live_grok_oauth_http_calls_an_offered_host_tool() {
        ...
        let _effort = EnvGuard::set("ANGEL_GROK_REASONING_EFFORT", "low");
        let clubs = grok_oauth_http_clubs();
        let (_, club, _) = clubs
            .into_iter()
    ...
    fn grok_research_tool_advertises_and_guards_query() {
        ...
        let _lock = env_lock();
        let dir =
            std::env::temp_dir().join(format!("angel0-grok-tool-auth-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let auth = dir.join("auth.json");
        std::fs::write(&auth, r#"{"entry":{"key":"oauth-token"}}"#).unwrap();
        let _file = EnvGuard::set("ANGEL_GROK_OAUTH_FILE", auth.to_str().unwrap());
        ...
        let tool =
            crate::tools::web::GrokResearchTool::from_env().expect("grok tool builds from env");
    ...
    mod non_http_usage_contract_fixture_tests {
        include!("grok/non_http_usage_contract_fixture_tests.rs");
    }
}
