...
impl FallbackClub {
    ...
    fn note_transition(chain: &[&Arc<dyn Club>], index: usize, reason: &str) {
        if let Some(next) = chain.get(index + 1) {
            crate::experience::note_failover(chain[index].label(), next.label(), reason);
        }
    }
...
pub(crate) fn mathgod_club(
    ...
    Some((
        Arc::new(crate::swarm::SwarmClub::mathgod(sol, grok, extra)),
        sol_avail,
    ))
}
...
pub(crate) fn sota_moa_club_with_breadth(
    ...
    Some(Arc::new(
        crate::swarm::SwarmClub::from_sota_env_roles("sota-moa", propose, judge, verify, aggregate)
