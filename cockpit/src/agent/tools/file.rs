...
use crate::agent::tools::nav::suggest_workspace_paths;
...
fn try_read_virtual(root: &Path, path: &str) -> Result<Option<String>, String> {
    ...
    if crate::agent::git::hub_url::is_github_uri(path) {
        return Ok(Some(crate::agent::git::hub_url::resolve_github_uri(
            root, path,
        )?));
    }
...
fn read_agent_uri(rest: &str) -> Result<String, String> {
    use crate::agent::harness::{HandleStoreLimits, session_disclose, session_list_receipts};
...
fn read_skill_uri(root: &Path, rest: &str) -> Result<String, String> {
    ...
    if name.is_empty() {
        ...
        let skills = crate::agent::harness::load_skills_for(root);
        if skills.is_empty() {
    ...
    let skill = skills
        ...
        .ok_or_else(|| {
            let available: Vec<_> = crate::agent::harness::load_skills_for(root)
                .into_iter()
                .map(|s| s.name)
                .collect();
            format!(
...
fn scan_and_register_conflicts(path: &str, text: &str, first_line: usize) -> Option<String> {
    ...
    let blocks = crate::agent::conflict::scan_conflict_lines(&lines, first_line);
    if blocks.is_empty() {
        return None;
    }
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        if hashline_anchors_enabled() {
                ...
                confined_read_limited(&self.root, Path::new(path), HASHLINE_ANNOTATE_MAX_BYTES)
            {
                ...
                let tag = crate::agent::hashline::record_snapshot(path, &whole_str);
...
impl Tool for WriteFileTool {
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        crate::agent::harness::hardlink_result(|| {
            let path = args["path"].as_str().ok_or("missing 'path'")?;
...
fn replacement_recovery(
    ...
    let mut recovery = serde_json::json!({
        ...
        "expect_tag": atomic_retry.map(|(tag, _)| tag.to_owned())
            .unwrap_or_else(|| crate::agent::hashline::content_tag(content)),
    });
...
/// Supports REM (delete), MV (rename), and block ops via [`crate::agent::hashline`].
fn apply_hashline_patch(root: &Path, text: &str, stage: bool) -> Result<String, String> {
    use crate::agent::hashline::SectionPlan;

    let sections = crate::agent::hashline::parse(text)?;
    ...
    for section in &sections {
        ...
        let outcome = crate::agent::hashline::plan_section_detailed(&original_str, section)?;
        match &outcome.plan {
            SectionPlan::Move { to, .. } => {
                safe_path(root, to)?;
    ...
    if stage {
        let (actions, notes) = crate::agent::staged_edit::actions_from_hashline_plans(plans);
        return crate::agent::staged_edit::stage_batch(root, actions, notes);
    }
    ...
    #[cfg(target_os = "linux")]
    let _exit_guard = exit_safe_patch::Guard::enter()?;
    let receipts = crate::agent::staged_edit::commit_actions(root, actions)?;
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        crate::agent::harness::hardlink_result(|| {
            ...
            if diff.trim_start().starts_with("*** Begin Patch") {
                if crate::agent::hashline::looks_like_hashline(diff) {
...
fn apply_with(
    ...
    let mut cmd = {
        ...
        let command_args = pre_args
            ...
            .collect::<Vec<_>>();
        crate::agent::sandbox::command(program, &command_args, &policy)?
    };
    #[cfg(not(target_os = "linux"))]
    let mut cmd = {
...
fn check_and_apply_with(
    ...
    for target in extract_diff_targets(&diff) {
        crate::agent::harness::confined_break_hardlink(root, Path::new(&target))?;
    }
