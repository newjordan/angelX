//! The `repo_search` agent tool — the repo-surfacing bench (`crate::knowledge::repos`),
...
use crate::knowledge::repos::Mode;
...
impl Tool for RepoSearchTool {
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        ...
        if query.chars().count() > crate::knowledge::repos::MAX_QUERY_CHARS {
            return Err(format!(
                "'query' must be at most {} characters",
                crate::knowledge::repos::MAX_QUERY_CHARS
            ));
        }
        let mode = args["mode"]
            .as_str()
        ...
        let set = crate::knowledge::repos::surface_cached(query.trim(), mode, limit, 1800);
