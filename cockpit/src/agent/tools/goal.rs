...
//! ([`crate::drive::goal`]). A goal the agent sets is re-injected into subsequent
...
impl Tool for GoalTool {
    ...
    fn call(&self, args: &Value) -> Result<String, String> {
        match args["action"].as_str().unwrap_or("show") {
            "show" => match crate::drive::goal::load_for(&self.workspace) {
            ...
            "set" => {
                ...
                let mut goal = crate::drive::goal::Goal::new(text);
                ...
                crate::drive::goal::save_for(&mut goal, &self.workspace)?;
                Ok(format!("goal set → {text}"))
            }
            "clear" => {
                crate::drive::goal::clear_for(&self.workspace)?;
