...
impl MutationTargets {
    pub(crate) fn record(&self, tool: &str, args: &Value) {
        if let Ok(mut state) = self.0.lock() {
            ...
            crate::knowledge::cut::for_each_mutation_target_path(tool, args, |path| {
...
pub(super) fn required(workspace: &Path, direct: &MutationTargets) -> Required {
    ...
    let git = crate::agent::harness::changed_rust_paths_for_verification(workspace);
...
pub(super) fn annotate(
    ...
    let missing = required
        ...
        .take(8)
        .map(|p| crate::agent::harness::cap_text_owned(p.display().to_string(), 240, 0))
        .collect::<Vec<_>>();
