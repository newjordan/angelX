...
pub(crate) fn helper_main() -> io::Result<()> {
    ...
    if wire.guarded && !crate::platform::yolo::enabled() {
        let private = crate::agent::harness::env_flag("ANGEL_HTTP_ALLOW_PRIVATE_NETWORK", false);
        builder = builder.resolver(move |netloc: &str| {
            crate::agent::tools::web::resolve_http_target(netloc, private)
        });
    }
