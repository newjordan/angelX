...
//! [`crate::knowledge::experience::cmd_verdict`], which is the sole authority on what a
...
use crate::knowledge::experience::CmdShell;
...
fn bounded_scratch_path(path: &Path) -> bool {
    ...
    roots
        .iter()
        .any(|root| crate::agent::harness::workspace_relative(root, path).is_ok())
}
...
fn task_shell_editable_redirect(command: &str, cwd: Option<&Path>) -> Option<String> {
    ...
    for target in targets {
        ...
        let relative = match crate::agent::harness::workspace_relative(root, supplied) {
...
fn shell_invocation() -> &'static ShellInvocation {
    ...
    SHELL.get_or_init(|| {
        ...
        if crate::knowledge::experience::env_flag("ANGEL_SHELL_PIPEFAIL", true) {
...
impl ShellTool {
    pub(crate) fn with_mutation_targets(
        mut self,
        targets: std::sync::Arc<crate::agent::tools::build::MutationTargets>,
    ) -> Self {
    ...
    fn observe(
        ...
        (
            crate::agent::harness::ExecObservation,
            &'static ShellInvocation,
        ),
        String,
    > {
...
impl Tool for ShellTool {
    ...
    fn call_with_cancel(
        ...
        crate::knowledge::experience::record_cmd_event(&experience, &folder);
        if obs.cancelled {
        ...
        match verdict {
            crate::knowledge::experience::VERDICT_FAIL => {
                ...
                if obs.exit == Some(127)
                    && let Some(hint) =
                        crate::platform::workspace_lang::missing_program_hint(command)
                {
                    message.push('\n');
