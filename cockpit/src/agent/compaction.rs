...
//!      memory palace ([`crate::knowledge::memory::store`]), so nothing is lost across
...
//! it mirrors [`crate::agent::swarm::SwarmClub`]'s scoped-thread fan-out over a `Sync`
//! club.

use crate::agent::club::{ChatMsg, ChatRole, Club};
use crate::agent::harness::{
    estimate_tokens, is_error_result, is_mutation_call, render_transcript, verification_outcome,
};
use crate::knowledge::memory::store::Drawer;
...
pub fn select_window_with_token_tail(
    ...
    let sys_end = history
        .iter()
        .position(|message| !crate::app::bootstrap::is_pinned_preamble(message))
...
fn collect_handoff_note(window: &[ChatMsg]) -> Option<String> {
    ...
    for message in window {
        ...
        if let Some(body) = message
            .content
            .strip_prefix(crate::agent::tools::plan::HANDOFF_STATE_PREFIX)
        {
            let body = body.trim();
            if !body.is_empty() {
                current = Some(body.to_string());
...
fn finish_compaction(
    ...
    let handoff_snapshot = state.handoff.as_ref().map(|note| {
        let max_chars = crate::agent::tools::plan::HANDOFF_NOTE_MAX_CHARS
...
fn collect_verification_ledger(window: &[ChatMsg]) -> VerificationLedger {
    ...
    for message in window {
        ...
        for call in message.tool_calls.iter() {
            ...
            let verification = crate::agent::harness::is_verification_call(call);
        ...
        if call.verification && !denied {
            let outcome = if is_error_result(&message.content) {
                Some(crate::agent::harness::VerificationOutcome::Failed)
            } else {
                verification_outcome(&call.call, &message.content)
            }
            .unwrap_or(crate::agent::harness::VerificationOutcome::Inconclusive);
...
fn parse_todo_state_result(result: &str) -> Option<PlanLedger> {
    let raw = result
        ...
        .last()?
        .trim()
        .strip_prefix(crate::agent::tools::plan::TODO_STATE_PREFIX)?;
    parse_plan_json(raw)
}
...
fn plan_proof(snapshot: &str) -> String {
    let proof = serde_json::json!({
        "sha256": crate::knowledge::cut::sha256_hex(snapshot.as_bytes()),
    });
...
fn prune_old_tool_outputs(window: &[ChatMsg]) -> Vec<ChatMsg> {
    ...
    let protected = crate::agent::harness::protected_tool_result_indices(window);
