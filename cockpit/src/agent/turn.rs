use crate::agent::club::{CacheUsage, ChatMsg, Club, RouteIdentity, TokenUsage, ToolDef};
use crate::agent::harness::{
    ToolRegistry, TurnEvent, TurnStopReason, run_turn_steered_checkpointed_observed,
};
use crate::agent::steer::SteerQueue;
use crate::knowledge::session::Session;
...
fn run_worker_guarded<T>(work: impl FnOnce() -> Result<T, String>) -> Result<T, String> {
    match crate::ui::term::catch_background_unwind(work) {
        Ok(result) => result,
        Err(_) => Err(
...
struct LoopWorkerScopeGuard<'a>(&'a crate::agent::harness::ToolRegistry);
...
impl Thinking {
    ...
    pub(crate) fn spawn(
        ...
        let idle_timeout_secs = crate::agent::club::model_defaults::budgets(
            ...
            .spawn(move || {
                ...
                let is_loop_worker = loop_owner.is_some() && {
                    let convo_blob = convo
                        .iter()
                        .rev()
                        .find(|m| m.role == crate::agent::harness::ChatRole::User)
                        .map(|m| m.content.as_ref())
                        .unwrap_or_default();
                    crate::agent::harness::ascii_contains_ignore_case(convo_blob, "competition")
                };
...
pub(crate) fn cache_hop_ledger_enabled() -> bool {
    crate::agent::harness::env_flag("ANGEL_CACHE_HOP_LEDGER", true)
}
...
fn cache_ledger_drop_threshold_pct() -> u64 {
    crate::agent::harness::env_usize("ANGEL_CACHE_LEDGER_DROP_PCT", 20) as u64
}
