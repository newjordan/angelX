...
fn submission_words(command: &str) -> Result<Vec<Vec<Word>>, String> {
    ...
    while let Some(&(start, ch)) = chars.peek() {
        ...
        if !dynamic {
            let decoded = crate::agent::tools::build::parse_direct_argv(&command[start..end])?;
            if decoded.len() != 1 {
    ...
    for mut words in commands {
        ...
        while at < words.len()
            && (words[at]
                .value
                .split_once('=')
                .is_some_and(|(key, _)| crate::agent::harness::shell_verifier::valid_env_key(key))
        ...
        if !matches!(name, "yukon" | "hilbert") {
            ...
            if words.iter().any(|w| w.dynamic && w.value.contains("<<")) {
                ...
                let delimiter = crate::agent::tools::build::parse_direct_argv(
...
pub(crate) fn model_label() -> Result<String, String> {
    crate::agent::harness::run_identity::live_model()
...
fn note_file_path(command: &str, workspace: Option<&Path>) -> Option<PathBuf> {
    let words = crate::agent::tools::build::parse_direct_argv(command).ok()?;
    let mut raw = None;
    let mut at = 2;
    while at < words.len() {
        if words[at] == "--" {
