...
impl RolloutStore {
    ...
    pub(crate) fn for_workspace(workspace: &Path) -> Self {
        ...
        let base = std::env::var_os("ANGEL_HARNESS_ROLLOUT_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| crate::platform::workspace_store::angel_subdir("harness-rollouts"));
        Self {
    ...
    fn prepare_directory(&self, leaf: &Path) -> Result<(), String> {
        use crate::platform::workspace_store::private_io::PrivateDirectory;
        for path in self
            .configured_base
            .iter()
            .map(PathBuf::as_path)
    ...
    pub(crate) fn project_identity(workspace: &Path) -> ProjectIdentity {
        let repo = crate::platform::workspace_store::repo_identity(workspace);
        ProjectIdentity {
            workspace_key: crate::platform::workspace_store::workspace_key(workspace),
            repo_key: repo.key,
            canonical_root_sha256: crate::knowledge::cut::sha256_hex(
                repo.root.to_string_lossy().as_bytes(),
            ),
        }
    }
    ...
    pub(crate) fn read_blob(&self, reference: &BlobRef) -> Result<Vec<u8>, String> {
        ...
        if bytes.len() as u64 != reference.bytes
            || crate::knowledge::cut::sha256_hex(&bytes) != reference.sha256
        {
    ...
    pub(crate) fn read_blob_by_digest(&self, sha256: &str) -> Result<Vec<u8>, String> {
        ...
        check_private_file(&path)?;
        let bytes = read_bounded(&path, MAX_BLOB_BYTES as u64)?;
        if crate::knowledge::cut::sha256_hex(&bytes) != sha256 {
    ...
    pub(crate) fn save_manifest(&self, manifest: &mut HarnessRolloutV1) -> Result<PathBuf, String> {
        ...
        manifest.manifest_sha256 = Some(crate::knowledge::cut::sha256_hex(&digest_body));
    ...
    pub(crate) fn load_manifest(
        ...
        let digest_body =
            serde_json::to_vec(&manifest).map_err(|error| format!("encode manifest: {error}"))?;
        if crate::knowledge::cut::sha256_hex(&digest_body) != claimed {
...
fn event_digest(
    ...
    Ok(crate::knowledge::cut::sha256_hex(&body))
}
...
fn secure_dir(path: &Path) -> Result<(), String> {
    crate::platform::workspace_store::private_io::PrivateDirectory::open_existing(path)
        .and_then(|directory| directory.secure_owner_only())
...
fn reject_sensitive_bytes(bytes: &[u8]) -> Result<(), String> {
    if std::str::from_utf8(bytes)
        .ok()
        .is_some_and(crate::platform::secrets::contains_secret)
    {
