...
pub(crate) fn run_code_mode_tool(
    ...
    let max_script_bytes = if crate::platform::yolo::enabled() && recipe != "repo_recon" {
        usize::MAX
    } else {
    ...
    let allow_effects = if recipe == "repo_recon" {
        ...
        args.get("allow_effects")
            .and_then(Value::as_bool)
            .unwrap_or_else(crate::platform::yolo::code_effects_allowed)
    };
        ...
        if let (Some((_, event_tx)), Some(child_id)) = (event_context, child_id.as_ref()) {
            ...
            let _ = event_tx.send(TurnEvent::ToolResult {
                ...
                summary: if event_result.starts_with("tool error:") {
                    crate::platform::secrets::redact_str(&event_result).into_owned()
                } else {
...
#[cfg(test)]
#[path = "../../../../tests/cockpit/harness/code_mode__recipe_tests.rs"]
mod recipe_tests;
