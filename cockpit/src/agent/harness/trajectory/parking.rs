...
fn resolve(root: &Path, digest: &str) -> io::Result<Vec<u8>> {
    ...
    if crate::knowledge::cut::sha256_hex(&body) != digest {
        return Err(io::Error::new(
...
fn original_body(root: &Path, content: &str) -> io::Result<(String, Vec<u8>)> {
    let stored = crate::platform::secrets::redact_bytes(content.as_bytes());
    let hash = crate::knowledge::cut::sha256_hex(&stored);
...
pub(crate) fn park_tool_result(
    ...
    if !replacement.is_empty() {
        let name = format!(
            "receipt-{}",
            crate::knowledge::cut::sha256_hex(replacement.as_bytes())
        );
