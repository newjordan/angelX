...
pub(crate) fn smart_auto_approves(scope: &crate::agent::approval::ApprovalScope) -> bool {
    if !smart_enabled() {
        return false;
    }
    matches!(
        scope,
        crate::agent::approval::ApprovalScope::ActionBatch(_)
...
#[cfg(test)]
#[path = "../../../tests/cockpit/app/yolo__tests.rs"]
mod tests;
