...
pub(crate) fn render(
    ...
    if let Some(art) =
        crate::knight_journey::try_art(kind, label, elapsed_secs, cell_width, art_height, motion)
    {
...
pub(crate) fn warm_assets() {
    crate::knight_journey::warm();
    let root = default_asset_root();
    crate::knight_cast::warm();
...
pub(crate) fn journey_status_rows(
    ...
    crate::knight_journey::scene_for(kind, label)?;
    Some(finish_text(
        Vec::new(),
        kind,
        label,
...
fn finish_text(
    ...
    let journey_status = crate::knight_journey::scene_for(kind, label).map(|_| {
...
fn default_asset_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("assets/tourney")
}
...
fn draw_rider(
    ...
    use crate::knight_cast::{Pose as CastPose, Side};
    let side = if mirror {
        Side::RedLeft
    } else {
        Side::BlueRight
...
mod tests {
    ...
    fn missing_and_corrupt_assets_use_visible_fallbacks() {
        let root =
            std::env::temp_dir().join(format!("angel-tourney-assets-{}", std::process::id()));
    ...
    fn library_sequences_wire_into_mapped_ceremonies() {
        ...
        let scratch =
            std::env::temp_dir().join(format!("angel-tourney-twins-{}", std::process::id()));
        ...
        for twin in [&no_library, &plates_only] {
            ...
            let _ = std::os::unix::fs::symlink(
                root.join("arena-plate.png"),
                twin.join("arena-plate.png"),
            );
        }
        ...
        for (kind, elapsed, twin) in [
            ...
            let wired =
                render_with_asset_root(kind, "seq", elapsed, 72, 16, MotionMode::Full, &root);
            let plain =
                render_with_asset_root(kind, "seq", elapsed, 72, 16, MotionMode::Full, twin);
            assert_ne!(
                signature(&wired),
                signature(&plain),
