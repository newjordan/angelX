...
use crate::agent::tools::self_model;
use crate::drive::loop_ctl::{self, LoopPending, LoopState, LoopStatus, count_passed_in};
...
impl crate::App {
    ...
    fn self_start(&mut self, goal: String) -> String {
        ...
        let mut registry =
            crate::app::bootstrap::build_registry(&self.bag, &self.session.id, wt_crate.clone());
        crate::ui::ui_inspect::install(&mut registry, std::sync::Arc::clone(&self.ui_broker));
    ...
    pub(crate) fn self_request_integrate(&mut self, gate_summary: &str) {
        ...
        if crate::platform::yolo::auto_integrate_self_edits() {
    ...
    pub(crate) fn self_restore_workspace(&mut self) {
        ...
        let mut registry =
            crate::app::bootstrap::build_registry(&self.bag, &self.session.id, prev.clone());
        crate::ui::ui_inspect::install(&mut registry, std::sync::Arc::clone(&self.ui_broker));
        self.tools = Arc::new(registry);
...
fn worktree_base() -> PathBuf {
    ...
    crate::platform::workspace_store::angel_subdir("self-worktrees")
}
...
fn same_canonical_project(left: &Path, right: &Path) -> bool {
    let left = crate::platform::workspace_store::repo_identity(left);
    let right = crate::platform::workspace_store::repo_identity(right);
