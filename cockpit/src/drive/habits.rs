...
pub(crate) fn proposed_dir() -> PathBuf {
    match std::env::var("ANGEL_HABIT_PROPOSED_DIR") {
        ...
        _ => crate::platform::workspace_store::angel_subdir("skills-proposed"),
    }
}
...
fn live_dir() -> PathBuf {
    match std::env::var("ANGEL_SKILLS_DIR") {
        Ok(p) if !p.trim().is_empty() => PathBuf::from(p),
        _ => crate::platform::workspace_store::angel_subdir("skills"),
    }
}
...
fn state_dir() -> PathBuf {
    std::env::var_os("ANGEL_HABITS_DIR")
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| crate::platform::workspace_store::angel_subdir("habitsmith"))
}
...
fn parse_proposal(dir: &Path, text: &str, fallback: &str) -> Proposal {
    let sk = crate::agent::harness::parse_skill(text, fallback);
    Proposal {
        name: sk.name,
...
pub(crate) fn status_text_in(dir: &Path, status_path: &Path, workspace: &Path) -> String {
    ...
    let worker = crate::platform::runtime_paths::script("runtime/habitsmith-tick.mjs");
