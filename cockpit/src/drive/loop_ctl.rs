...
//!
//! Where [`deli`](crate::drive::deli) runs a bounded iteration loop *inside one turn*,
...
//! [`run_turn`](crate::agent::harness::run_turn), so it inherits all the per-turn
...
//!   ([`crate::drive::iterate`]) while keeping the selected model, effort, and tier.
...
//!   ([`crate::drive::goal::Goal::accept_cmd`]) that command is run off-thread and scored
//!   through the [`reinforce`](crate::drive::reinforce) verifiable reward
...
//!   (mirroring [`session`](crate::knowledge::session)); resuming that same cockpit session
...
use crate::ui::toolstrip::ToolStripSnapshot;
...
pub struct LoopState {
    ...
    /// The task to pursue; empty => drive the active [`Goal`](crate::drive::goal::Goal).
    pub task: String,
    ...
    #[serde(default)]
    pub verifier_failure: Option<crate::ui::toolstrip::VerifierFailure>,
    #[serde(default)]
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub(crate) pending_recovery_contexts: Vec<crate::agent::club::RecoveryContextRef>,
...
fn acceptance_config_identity(baseline_passed: Option<usize>) -> String {
    format!(
        ...
        crate::agent::harness::run_identity::source_sha256(),
        crate::drive::reinforce::TEST_VERIFIER_CONTRACT,
...
fn acceptance_workspace_fingerprint(workspace: &Path) -> Option<String> {
    crate::agent::harness::workspace_evidence_sha256(workspace)
...
impl crate::App {
    ...
    fn loop_restart_dialog(&mut self, rest: String, deli: bool) -> String {
        ...
        .map(|state| crate::drive::loop_dialog::LoopLaunchSettings {
    ...
    fn loop_dialog_start(&mut self) -> String {
        ...
        self.scryglass
            .controller
            .leave_route(crate::ui::scryglass::StageRoute::Workshop);
    ...
    fn loop_start_with_limits(
        ...
        st.last_workspace_fingerprint = st
            .workspace
            .as_deref()
            .and_then(crate::agent::harness::workspace_fingerprint);
        st.task = task.clone();
    ...
    pub(crate) fn loop_iteration_convo(&self) -> Vec<ChatMsg> {
        ...
        let learned =
            crate::drive::continual_harness::context_block(self.tools.current_workspace());
        if !learned.is_empty() {
    ...
    pub(crate) fn loop_harvest_stopped(
        ...
        reason: crate::agent::harness::TurnStopReason,
    ) {
        self.loop_account_rl();
        self.clear_partial();
    ...
    pub(crate) fn loop_harvest_error_with_tools(&mut self, err: String, tools: ToolStripSnapshot) {
        ...
        self.clear_partial();
        if crate::agent::harness::is_execution_blocker(&err) {
        ...
        let session_fault = crate::agent::harness::classify_session_fault(&err);
    ...
    fn loop_workspace_diff(&self) -> Option<String> {
        ...
        let stat = crate::platform::workspace_store::capture_repo_probe(
            "git",
            &["diff", "--stat", rev],
            &ws,
            5,
    ...
    pub(crate) fn loop_finish(&mut self, status: LoopStatus, reason: &str) {
        ...
        let kind = match status {
            LoopStatus::Done => crate::ui::viz::lifecycle_viz::CeremonyKind::LoopDone,
            LoopStatus::Paused => crate::ui::viz::lifecycle_viz::CeremonyKind::LoopPaused,
            LoopStatus::Stopped => crate::ui::viz::lifecycle_viz::CeremonyKind::LoopStopped,
            LoopStatus::Failed => crate::ui::viz::lifecycle_viz::CeremonyKind::LoopFailed,
    ...
    fn loop_escalate_blocked_verifier(&mut self) -> bool {
        ...
        let reason = format!(
            ...
            failure.diagnostic(),
            failure.command,
            crate::platform::secrets::redact_str(&workspace.display().to_string())
        );
        ...
        crate::agent::harness::note_escalation(self.loop_ctl.iteration, "verifier_blocked_repeat");
        if limit_explicit {
    ...
    pub(crate) fn loop_spawn_self_gate(&mut self) {
        ...
        std::thread::spawn(move || {
            let (v, detail) =
                crate::agent::tools::self_model::run_self_gate_with_baseline(&workspace, baseline);
    ...
    fn loop_request_sota(&mut self) {
        ...
        save(&self.loop_ctl);
        self.start_lifecycle_ceremony(
            crate::ui::viz::lifecycle_viz::CeremonyKind::LoopEscalate,
            self.loop_task_text(),
        );
    }
...
/// [`has_evidence_tag`](crate::drive::iterate::has_evidence_tag): on top of requiring a
...
fn loop_sota_enabled() -> bool {
    crate::agent::harness::env_flag("ANGEL_LOOP_SOTA", true)
}
...
fn loop_sota_allow_openai() -> bool {
    crate::agent::harness::env_flag("ANGEL_LOOP_SOTA_ALLOW_OPENAI", true)
}
...
fn is_loop_local_label(label: &str) -> bool {
    ...
    if low.is_empty() || crate::agent::club::is_logical_wrapper_label(&low) {
        return false;
    }
    !is_sota_label(label)
}
...
fn select_loop_local_club(bag: &crate::agent::club::Bag) -> Option<Arc<dyn Club>> {
...
fn preferred_loop_local_label(
    ...
    if !in_hand.trim().is_empty()
        && !crate::agent::club::is_logical_wrapper_label(in_hand)
        && labels
            .iter()
            .zip(available.iter())
            .any(|(l, a)| l == in_hand && *a)
...
fn select_loop_sota_club(bag: &crate::agent::club::Bag) -> Option<Arc<dyn Club>> {
...
fn preferred_loop_sota_label(
    ...
    let mut ranked: Vec<(u8, String)> = labels
        ...
        .filter(|(_, a)| **a)
        .map(|(l, _)| l.clone())
        .filter(|l| is_sota_label(l) && !crate::agent::club::is_logical_wrapper_label(l))
...
pub(crate) fn count_passed_in(cmd: &str, dir: &Path) -> usize {
    ...
    crate::agent::harness::parse_test_result(&combined).passed
}
    ...
    match workspace {
        Some(workspace) => {
            ...
            if let Some(session_id) = session_id.filter(|id| !id.trim().is_empty()) {
                let session_key = crate::knowledge::cut::sha256_hex(session_id.as_bytes());
...
pub fn save(st: &LoopState) {
    ...
    if path.exists() {
        let existing_matches = std::fs::read_to_string(&path)
            ...
            .is_some_and(|root| {
                let stored = crate::platform::workspace_store::repo_identity(&root);
...
fn persist_state_dir(st: &LoopState) {
    ...
    let dir = std::path::Path::new(&base)
        .join(&identity.key)
        .join("state");
    let Ok(directory) = crate::platform::workspace_store::private_io::PrivateDirectory::open(&dir)
    else {
        return;
    };
...
pub fn load_for(workspace: &Path) -> Option<LoopState> {
    let expected = crate::platform::workspace_store::repo_identity(workspace);
    let state = load_from_path(store_path_for(Some(workspace)))?;
    let stored_workspace = state.workspace.as_deref()?;
    let stored = crate::platform::workspace_store::repo_identity(stored_workspace);
    ...
    if st.status == LoopStatus::Baselining {
        ...
        let blocked = st.retry_after_error
            ...
            || st
                .last_error
                .as_deref()
                .is_some_and(crate::agent::club::error_requires_provider_action);
...
fn clear_file_for(workspace: Option<&Path>, session_id: Option<&str>) {
    ...
    let owned = std::fs::read_to_string(&path)
        ...
        .is_some_and(|root| {
            let stored = crate::platform::workspace_store::repo_identity(&root);
            crate::platform::workspace_store::matches_project(workspace, &stored.root, &stored.key)
        });
    if owned {
        let _ = crate::platform::workspace_store::remove_durable(&path);
    }
}
