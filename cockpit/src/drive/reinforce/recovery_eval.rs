...
pub(crate) fn run(request: RecoveryEvalRequest<'_>) -> Result<RecoveryEvalExecution, String> {
    ...
    let before = crate::agent::harness::workspace_evidence_sha256(&workspace)
    ...
    let shell_sha256 =
        crate::knowledge::cut::sha256_hex(&std::fs::read(&shell).map_err(|e| e.to_string())?);
    ...
    let mut process = crate::agent::sandbox::command(&shell, ["-c", request.command], &policy)
        .map_err(|e| e.to_string())?;
    process.current_dir(&workspace).env_clear();
    for (key, value) in &environment {
        process.env(key, value);
    }
    crate::agent::sandbox::set_helper_policy(&mut process, &policy).map_err(|e| e.to_string())?;
    if request.cancel.load(Ordering::Acquire) {
        return Err("recovery evaluator cancelled before dispatch".into());
    }
    ...
    let capture = crate::agent::harness::output_timed_captured_cancellable(
        process,
        request.timeout,
        Some(request.cancel),
    )?;
    let cancelled = capture.cancelled || request.cancel.load(Ordering::Acquire);
    let after = crate::agent::harness::workspace_evidence_sha256(&workspace)
    ...
    let evidence = EvaluatorEvidence::from_captured_output(
        ...
        CapturedEvaluatorExecution {
            ...
            execution_policy_sha256: crate::knowledge::cut::sha256_hex(&canonical),
