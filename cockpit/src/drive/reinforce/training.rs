...
pub(super) fn publish(
    ...
    let decision = Decision {
        ...
        subject_sha256,
        task_sha256: crate::knowledge::cut::sha256_hex(task.as_bytes()),
...
pub(crate) fn audit(root: &Path, input: &[u8]) -> Result<Value, String> {
    ...
    if crate::knowledge::cut::sha256_hex(&bytes) != request.decision_sha256 {
    ...
    if decision.schema != DECISION_SCHEMA
        ...
        || decision.subject_sha256
            != crate::knowledge::cut::sha256_hex(subject(&request.task, &request.answer).as_bytes())
        || decision.task_sha256 != crate::knowledge::cut::sha256_hex(request.task.as_bytes())
        || decision.answer_sha256 != crate::knowledge::cut::sha256_hex(request.answer.as_bytes())
    ...
    if crate::knowledge::cut::sha256_hex(&artifact) != decision.evaluator_artifact_sha256 {
    ...
    Ok(json!({
        "schema": RECEIPT_SCHEMA, "request_sha256": crate::knowledge::cut::sha256_hex(input),
