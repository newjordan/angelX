...
//!   [`crate::agent::harness::run_loop_experiment`], with the candidate policy note
...
//!   ([`crate::drive::reinforce::promotion::TechnicalReward::ObjectivePass`]);
...
//! * an approved release is installed through [`crate::drive::continual_harness`]
...
use crate::agent::club::{ChatMsg, Club, ClubReply, RouteIdentity};
use crate::drive::continual_harness::{EntryKind, RefinementEdit, Scope};
use crate::drive::reinforce::objective_case::ObjectiveCaseEvaluator;
use crate::drive::reinforce::promotion::{
...
/// [`crate::drive::continual_harness::context_block`].
...
fn configured_cases(workspace: &Path) -> Result<Vec<RlCase>, String> {
    ...
    let goal = crate::drive::goal::load_for(workspace).ok_or_else(|| MISSING.to_string())?;
    if goal.status != crate::drive::goal::GoalStatus::Active {
        return Err(format!(
...
impl CodingPolicyGenerator {
    ...
    fn attempt(&self, policy_note: &str, task: &str, version: u64) -> Candidate {
        ...
        let request = crate::agent::harness::LoopExperimentRequest {
        ...
        let measured = crate::agent::harness::run_loop_experiment(
            request,
            Arc::clone(&self.club),
        ...
        match measured {
            Ok(result) => {
                ...
                let output = format!(
                    ...
                    attempt.display(),
                    result.stop_reason,
                    crate::drive::reinforce::objective_case::attempt_evidence_pack(&attempt)
                );
...
fn rl_root() -> PathBuf {
    match std::env::var("ANGEL_RL_DIR") {
        ...
        _ => crate::platform::workspace_store::angel_subdir("rl"),
    }
}

fn workspace_run_root(workspace: &Path) -> PathBuf {
    rl_root().join(crate::platform::workspace_store::workspace_key(
...
fn run_plan(
    ...
    crate::agent::harness::freeze_active_source(workspace, &fixture_root, cancel)?;
    ...
    for case in plan.audit.iter() {
        ...
        crate::agent::harness::freeze_active_source(&source, &dest, cancel)?;
    ...
    let sink: crate::drive::reinforce::objective_case::MeasurementSink =
    ...
    let audit_cases: Vec<TechnicalHeldoutCase<'_>> = plan
        ...
        .collect();
    let promotion_evaluators: Vec<&dyn crate::drive::reinforce::evaluator::PolicyEvaluator> = plan
        .cases
        .iter()
        .map(|case| {
            evaluators
                .get(&case.task)
                .map(|evaluator| {
                    evaluator.as_ref() as &dyn crate::drive::reinforce::evaluator::PolicyEvaluator
                })
                .ok_or_else(|| format!("case {:?} has no evaluator", case.id))
        })
        .collect::<Result<Vec<_>, String>>()?;
    let audit_evaluators: Vec<&dyn crate::drive::reinforce::evaluator::PolicyEvaluator> = plan
        .audit
        .iter()
        .map(|case| {
            evaluators
    ...
    let promotion_manifest = CohortManifest::new_technical(
        &format!(
            "objective-{}",
            crate::knowledge::cut::sha256_hex(plan.cases[0].task.as_bytes())
        ),
    ...
    let audit_manifest = if audit_cases.is_empty() {
        ...
        Some(CohortManifest::new_technical(
            &format!(
                "audit-{}",
                crate::knowledge::cut::sha256_hex(plan.audit[0].task.as_bytes())
            ),
    ...
    for round in 0..plan.rounds {
        ...
        match audit_manifest.as_ref() {
            ...
            None => {
                ...
                let (rollouts, metrics) = crate::drive::reinforce::evaluate_batch(
                    &training_reward,
                    version,
                    &config,
                    candidates,
                ...
                let heldout = crate::drive::reinforce::promotion::technical_cases_as_heldout(
                    &promotion_cases,
                );
...
fn persist_policy_note(
    ...
    let existing = crate::drive::continual_harness::load_project(workspace)
        .entries
    ...
    crate::drive::continual_harness::apply_edits(
        workspace,
        Scope::Project,
        "rl campaign release",
        &evidence,
    ...
    let event = crate::drive::continual_harness::load_project(workspace)
        .refinements
        .last()
...
pub(crate) fn current_policy_note(workspace: &Path) -> Option<String> {
    crate::drive::continual_harness::load_project(workspace)
        .entries
...
impl From<&RlCase> for RecordedCase {
    fn from(case: &RlCase) -> Self {
        Self {
            ...
            verify_sha256: crate::knowledge::cut::sha256_hex(case.verify.as_bytes()),
        }
    }
}
...
pub(crate) fn authority_summary() -> Option<AuthoritySummary> {
    let root = crate::drive::reinforce::authority_root();
    let entries = std::fs::read_dir(root).ok()?;
