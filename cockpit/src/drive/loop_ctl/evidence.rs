...
pub(crate) fn loop_binary_identity() -> LoopBinaryIdentity {
    if let Some(id) = crate::agent::harness::run_identity::current() {
    ...
    LoopBinaryIdentity {
        sha256: crate::agent::harness::run_identity::source_sha256().to_string(),
        source_digest: crate::agent::harness::run_identity::source_sha256().to_string(),
        build_info: None,
    }
}
...
fn measured_row_from_receipt(st: &LoopState, receipt: &str) -> MeasuredCandidateRow {
    ...
    let command = receipt
        ...
        .unwrap_or(receipt)
        .to_string();
    let digest = crate::knowledge::cut::sha256_hex(command.as_bytes());
...
pub(crate) fn drain_submission_journal(st: &mut LoopState) {
    let pending = crate::agent::tools::submit_identity::drain_workspace_journal(
        st.workspace.as_deref(),
        &st.id,
    );
    if pending.is_empty() {
        return;
