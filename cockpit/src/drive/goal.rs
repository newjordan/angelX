...
//! command (a test/build/lint invocation) that the [`loop_ctl`](crate::drive::loop_ctl)
...
fn binding(workspace: &Path) -> crate::platform::workspace_store::RepoIdentity {
...
fn store_path_for(workspace: &Path) -> PathBuf {
    ...
    crate::platform::workspace_store::workspace_json_path_in(
        &crate::platform::workspace_store::angel_subdir("goals"),
        &identity.root,
    )
}
...
fn matches_workspace(goal: &Goal, workspace: &Path) -> bool {
    match (goal.workspace.as_deref(), goal.project_key.as_deref()) {
        (Some(root), Some(key)) => {
            crate::platform::workspace_store::matches_project(workspace, root, key)
        }
        _ => false,
    }
}
...
pub fn save_for(goal: &mut Goal, workspace: &Path) -> Result<(), String> {
    ...
    crate::platform::workspace_store::write_private_atomic(&path, &json)
}
...
pub fn clear_for(workspace: &Path) -> Result<(), String> {
    ...
    if owned {
        crate::platform::workspace_store::remove_durable(&path)
    } else if path.exists() {
    ...
    pub(crate) fn update_goal(&mut self, arg: Option<String>) -> String {
        ...
        match lower.as_str() {
            ...
            "go" | "run" | "drive" | "pursue" => {
                ...
                if self.loop_ctl.status == crate::drive::loop_ctl::LoopStatus::Paused {
        ...
        if let Some(rest) = strip_word(raw, &["cmd", "verify", "check"]) {
            ...
            if self.goal.is_some() {
                match self.repin_loop_accept_cmd(rest) {
                    ...
                    Some(false)
                        if self.loop_ctl.status == crate::drive::loop_ctl::LoopStatus::Baselining
        ...
        self.start_lifecycle_ceremony(crate::ui::viz::lifecycle_viz::CeremonyKind::GoalSet, raw);
        msg
    }
