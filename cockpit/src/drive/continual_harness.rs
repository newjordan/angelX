...
fn project_state_path(workspace: &Path) -> PathBuf {
    ...
    crate::platform::workspace_store::workspace_json_path_in(&root_dir(), &identity.root)
}
...
fn save_state(path: &Path, state: &HarnessState) -> Result<(), String> {
    ...
    let bytes = crate::platform::secrets::to_redacted_vec_pretty(state)
...
fn enabled() -> bool {
    crate::agent::harness::env_flag("ANGEL_CONTINUAL_HARNESS", true)
}
...
impl crate::agent::harness::Tool for ContinualHarnessTool {
    fn name(&self) -> &str {
        "continual_harness"
    }
