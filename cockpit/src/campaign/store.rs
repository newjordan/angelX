...
impl CampaignStore {
    pub(crate) fn for_workspace(workspace: &Path) -> Self {
        let root = std::env::var_os("ANGEL_CAMPAIGN_STATE_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| crate::workspace_store::angel_subdir("campaigns"));
    ...
    pub(crate) fn save(&self, record: &CampaignRecord) -> Result<(), String> {
        ...
        secure_dir(&self.project_dir)?;
        reject_symlink(&self.record_path)?;
        let body = crate::secrets::to_redacted_vec_pretty(record)
    ...
    pub(crate) fn append_event(
        ...
        let mut line = crate::secrets::to_redacted_vec(&event)
    ...
    pub(crate) fn save_proof_snapshot(
        ...
        let body = crate::secrets::to_redacted_vec_pretty(receipt)
        ...
        if path.exists() {
            ...
            return Ok((relative, crate::cut::sha256_hex(&body)));
        }
        ...
        Ok((relative, crate::cut::sha256_hex(&body)))
    }
    ...
    pub(crate) fn save_alignment_snapshot(
        ...
        let snapshot_sha256 = crate::cut::sha256_hex(&body);
        secure_dir(&self.root)?;
        secure_dir(&self.project_dir)?;
        let proofs = self.project_dir.join("proofs");
        secure_dir(&proofs)?;
...
fn compact_scrubbed(text: &str, max_chars: usize) -> String {
    let scrubbed = crate::experience::scrub_secrets(text);
